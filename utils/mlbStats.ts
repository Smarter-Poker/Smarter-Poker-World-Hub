...
=======
  if (useRpc) {
    const [rpcRes, riskRes, marketRes, baselineRes, gradeRes, equityRes] = await Promise.all([
      ...
      mlbDb.from('sim_bet_grade_summary').select('*'),
      mlbDb
        .from('sim_equity_daily')
        .select('day, bets, day_pnl, end_bankroll, drawdown')
...
>>>>>>> Stashed changes
    ...
    if (!rpcRes.error && rpcRes.data) {
      ...
      const gradeSummary = (gradeRes.data || [])
        .slice()
        .sort((a: any, b: any) => (TIER_ORDER[a.bet_tier] ?? 99) - (TIER_ORDER[b.bet_tier] ?? 99));
  ...
  while (hasMore) {
    let query = mlbDb
      .from('sim_bets')
      .select(
        'id, as_of_ts, pnl, result, stake, bankroll_after, market, selection, edge_pts, bet_score, bet_tier, game_pk'
      );
  ...
  for (const bet of bets) {
    ...
    if (!marketsMap[m])
      marketsMap[m] = { market: m, bets: 0, wins: 0, losses: 0, pnl: 0, staked: 0 };
    marketsMap[m].bets++;
    ...
    if (!gradesMap[t])
      gradesMap[t] = { bet_tier: t, bets: 0, wins: 0, losses: 0, pnl: 0, staked: 0 };
    gradesMap[t].bets++;
    gradesMap[t].pnl += pnl;
  ...
  const weeklyCurve = Object.values(weeksMap).sort((a: any, b: any) =>
    a.weekOf.localeCompare(b.weekOf)
  );
  const recentBets = bets.slice(-20).reverse();

  // Extras computation
  const equityDailyArr = Object.values(daysMap).sort((a: any, b: any) =>
    a.day.localeCompare(b.day)
  );
  const equityDaily = equityDailyArr.map((d) => ({
    ...d,
    drawdown:
      d.peak_bankroll > 0 ? ((d.peak_bankroll - d.end_bankroll) / d.peak_bankroll) * 100 : 0,
  }));

  const bestDayObj = equityDailyArr.reduce(
    (prev, current) => (prev.day_pnl > current.day_pnl ? prev : current),
    { day_pnl: 0 }
  );
  const worstDayObj = equityDailyArr.reduce(
    (prev, current) => (prev.day_pnl < current.day_pnl ? prev : current),
    { day_pnl: 0 }
  );
  ...
  const marketSummary = Object.values(marketsMap)
    .map((m) => ({
      ...m,
  ...
  const TIER_ORDER: Record<string, number> = { ELITE: 0, STRONG: 1, LEAN: 2, THIN: 3, PASS: 4 };
  const gradeSummary = Object.values(gradesMap)
    .map((g) => ({
      ...g,
