...
## PHASE 56 — Task #106: Drop 86 zero-scan indexes (2026-05-12)

Migration: `20260512_task106_drop_unused_indexes.sql`. Applied to production 2026-05-12.
86 non-unique, non-primary indexes with `idx_scan=0` across the full stats window
(stats_reset=NULL, since DB creation 2026-01-06, ~4.5 months). Phase 39 (2026-05-03)
dropped the prior cohort; this cohort covers newer indexes plus any that survived the last
soak. Freed ~303 MB. Excluded 4 indexes: 3 materialized-view refresh paths
(`mv_active_poker_locations_*`) + `autofix_attempts_status_next_retry_idx`.

Post-drop verification: 4 remaining zero-scan indexes — exactly the 4 excluded. Clean.

---

## PHASE 57 — Vercel Preview Auth Bypass + Workers Deploy Script (2026-05-13)

Three deferred items from 2026-05-12 closed out.

| Deliverable | Detail |
|---|---|
| `preview-signup-gate.yml` bypass fix | Added `VERCEL_AUTOMATION_BYPASS_SECRET` env var to both the curl health probe step and the Playwright step. Curl now sends `-H "x-vercel-protection-bypass: $VERCEL_AUTOMATION_BYPASS_SECRET"`. Commit `2dbf1471`. |
| `playwright.preview.config.ts` bypass | Added `extraHTTPHeaders: { 'x-vercel-protection-bypass': ... }` to the `use` block so every Playwright page navigation carries the header when running against a protected preview URL. Commit `2dbf1471`. |
| RULE 0 handoff: Vercel secret provisioning | `.agent/handoffs/2026-05-13-vercel-automation-bypass-secret.md` — step-by-step to generate the bypass secret in Vercel dashboard and add as GitHub repo secret. Commit `3bd76e43`. |
| `scripts/deploy-workers.sh` created | The script referenced in the build tracker but missing from disk. Syncs `scripts/yt-transcode-worker/index.js` to `reels-transcode-worker` (5.161.49.206, ash) and restarts `sp-yt-transcode.service`. Supports `--dry-run` and `--all` flags. Commit `b58a5ecf`. |
| RULE 0 handoff: P50 workers deploy | `.agent/handoffs/2026-05-13-deploy-workers-p50.md` — 7+ self-healer commits unreleased to VM. Run `bash scripts/deploy-workers.sh`. Commit `b58a5ecf`. |

**Root cause on preview gate 401s:** Vercel deployment protection was blocking CI runner requests to preview URLs. The `x-vercel-protection-bypass` header was simply missing from both the curl probe and the Playwright config. Code fix is shipped; secret value requires dashboard provisioning (see handoff).

---

## CURRENT STATE — 2026-05-13

Production is green. All SQL applied. No active incidents.

Shipped this session: Phase 49 (trivia timer hook), Phase 50 (10 notification/social SQL migrations), Phase 55 (poker engine crash + auth callback + home games PNM), Phase 56 (Task #106: 86 zero-scan indexes, 303 MB freed), Phase 57 (preview gate bypass fix + deploy-workers.sh).

**Pending (requires Mac/dashboard access — RULE 0 handoffs written and committed):**
...
| Mobile menu finish | The global trigger focus ring is contained inside the scaled header; Poker Near Me utility controls, section selectors, and destination tiles use continuous one-pixel frames and a restrained-blue selected state without decorative edge fragments. |
...
**Release state:** Complete and published. Core Phase 5 merged in PR #1141 as `24cae9ed`; PR #1150 merged the permanent WebKit harness as `e0373dd940`; PR #1153 merged the mobile command-frame correction as `e9e0b1ccfa`; PRs #1154 and #1155 synchronized the Title Case and Club Arena footer regression contracts as `b262854cdf` and `00507177f4`. Vercel deployment `dpl_EF7LQV2aoDGE1sCSqtfuQ7KKDvA8` is Ready on `smarter.poker` from main revision `7002d6f`. Verification passes 90/90 Poker Near Me contracts, 554/554 repository prebuild checks, 14/14 compiled Chromium desktop/mobile Phase 16+17 journeys, the exact webpack production build (403 static pages), 18/18 applicable live Chromium/mobile Chrome/desktop WebKit/iPhone WebKit Phase 17 checks with two expected WebKit forced-colors skips, and all 14 Chromium/WebKit global-footer route/geometry checks. Desktop 1440×1000 and mobile 390×844 visual audits confirmed zero overflow, fully contained continuous command borders, 44-pixel tabs, and working live Back/Forward restoration. Full evidence: `.agent/audits/2026-08-31-poker-near-me-phase-5-cross-engine-accessibility.md`.
...
**Renewed closure audit before Phase 7:** Thirteen connected Personal Assistant and Sandbox handlers now share the cached asymmetric-token verifier instead of repeatedly calling the remote Auth user endpoint. The permanent route contract blocks direct `auth.getUser` regressions, the durable App Router worker owns a JSON 503 boundary, all three formerly orphaned Personal Assistant contract suites are part of the engine release gate, and the production watchdog now verifies twelve authenticated read-only APIs across the Assistant and Sandbox. Verification passed 152/152 engine contracts, 20/20 focused hardening contracts, strict TypeScript, the exact 403-page production build and all five performance budgets, 49 applicable compiled desktop/mobile Chromium/Safari journeys with four intentional viewport skips, all twelve authenticated production reads, all four owner-isolation probes, and complete protected-account reconciliation. Two repeated broad CI runs then proved that four browser workers saturated the one local Next.js server and produced changing cross-surface failures, including intermittent local health failures; the unchanged desktop/mobile suite and its release contract now enforce two bounded workers for stable server capacity. The bounded run exposed and corrected a backward skip-link traversal check, an undersized degraded-map bootstrap allowance, a popup-action race, and invalid News feed count assumptions; the canonical wallet flow now opens from the global-header dialog. Pull requests #1207, #1227, #1235, #1389, and #1391 merged the closure work. Broad Actions run `33546158379` completed successfully with 687 passed journeys, 24 intentional skips, and zero final failures. The exact production revision `97b74d82` is a verified descendant of the functional closure merges and reports healthy database status; deployment watchdog run `33547596036` passed. The final Daniel-account audit retained 37 history rows without duplication, scanned 1,468 Club Arena records across eight batches, audited 303 eligible hands and 663 decisions, kept all 663 currently unsealed decisions explicitly unpriced, persisted 19 findings, and resolved all 32 corrective candidates without a broken destination. Pull request #1391 repaired the isolated publication helper with shell-safe merge-SHA parsing and a fail-closed contract; its own live run printed exact merge SHA `bca88a3edf830e848addcec3100798f8251c2278`. Exact evidence remains in the Phase 6 audit file.

**Final release-gate closure:** Pull request #1396 merged as `e22114113601cb12c3d6e22ea4f14163e181b0c6`, synchronizing the World Hub footer component and both regression oracles to Club Arena's live `clamp(44px, 12.326vw, 132px)` geometry. Global Footer run `34003392539` passed 16/16 Chromium and WebKit contracts. Production revision `c9e490ea` contains the correction and reports healthy database status; its deployment-triggered Personal Assistant watchdog passed in run `34003740109`. The live Personal Assistant browser matrix passed 49 applicable desktop/mobile Chromium/Safari journeys with four intentional viewport skips, and final Daniel-account reconciliation retained 37 history rows without duplication while covering all 32 corrective destinations. Phase 6 is complete and Phase 7 has not started.

**Deep reverification gate before Phase 7:** A source-to-persistence audit found and closed false exact-match risk, incomplete live bet and raise geometry, duplicate cross-schema hand pagination, heuristic source-example wiring, missing private-fact cascades, stale Daily Hand precedence, technical-copy corruption, expired shared-scenario silence, an invalid sign-in route, and incomplete keyboard and mobile range-grid behavior. The hardened matcher is now version 3 and fails closed unless the complete situation identity and trusted solver provenance match. The permanent engine gate now includes the new data-wiring and UI contracts. Local verification passed 164/164 Personal Assistant contracts, strict TypeScript, full lint across 3,938 files, the exact 403-page production build, and all five Personal Assistant performance budgets. Production cascade preflight and post-apply checks found zero orphan references, and both Supabase advisor classes contain no `ca_hand_facts` finding. Publication and live production evidence are recorded in `.agent/audits/2026-09-05-personal-assistant-phase-6-deep-reverification.md`. Phase 7 remains unstarted.

**Matcher freshness addendum:** The protected-account receipt proved that recent unpriced version 2 rows could bypass an immediate version 3 re-audit. Every verified, unpriced, and lookup-failed decision now carries matcher provenance, and any older matcher row is queued regardless of age. The regression gate covers both stale current-matcher rows and fresh legacy-matcher rows. The live copy audit was also corrected to inspect rendered text rather than CSS comments inside style elements.
