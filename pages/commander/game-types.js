...
import { Plus, Edit2, Trash2, Check, X, Loader2, Save,
...
export default function GameTypesPage() {
  ...
  function resetForm() {
    setForm({ name: '', short_code: '', stakes: '', min_buyin: 100, max_buyin: 0,
      max_players: 9, rake_type: 'pot', rake_percent: 5, rake_cap: 15,
      time_rate: 0, color: '#1877F2', notes: '' });
    setEditingId(null);
    setShowForm(false);
        ...
        <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {gameTypes.map(gt => (
                <div key={gt.id}
                  className={`bg-[#242526] rounded-2xl border overflow-hidden transition-all ${
                    gt.is_active ? 'border-[#3A3B3C]' : 'border-[#3A3B3C]/50 opacity-60'
                  }`}>
