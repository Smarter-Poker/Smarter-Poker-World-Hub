...
import {
  ArrowLeft, Plus, Trophy, CheckCircle2, Loader2, RefreshCw,
...
export default function HighHands() {
  ...
  return (
    <>
      <Head><title>High Hands | Club Commander</title></Head>
      <div className="min-h-screen bg-[#18191A] text-[#E4E6EB] font-['Inter']">
        <div className="bg-[#242526] border-b border-[#3A3B3C] px-4 py-3 flex items-center gap-3">
          <button onClick={() => router.push('/commander/dashboard')} style={{ background: 'none', border: '1px solid #444', borderRadius: 10, padding: '8px 14px', color: '#ccc', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600 }}>
                    ...
                    {HAND_RANKS.map(rank => (
                      <button key={rank} onClick={() => setForm({ ...form, hand_rank: rank })}
                        className={`py-2.5 rounded-xl text-xs font-semibold ${
                          form.hand_rank === rank ? 'bg-[#F59E0B] text-black' : 'bg-[#3A3B3C] text-[#B0B3B8] active:bg-[#4A4B4C]'
                        }`}>{rank}</button>
                    ))}
                  </div>
                </div>
