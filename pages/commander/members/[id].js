...
export default function MemberProfile() {
  ...
  const tierColor = tierColors[m.membership_tier] || '#B0B3B8';
  const statusColor = statusColors[m.membership_status] || '#B0B3B8';

  const timeHours = Math.floor((m.time_balance_minutes || 0) / 60);
    ...
    <>
      ...
      <div className="min-h-screen bg-[#18191A] text-[#E4E6EB] font-['Inter']">
        ...
        <div className="bg-[#242526] border-b border-[#3A3B3C] px-4 py-3 flex items-center gap-3">
          <button onClick={() => router.push('/commander/members')} style={{ background: 'none', border: '1px solid #444', borderRadius: 10, padding: '8px 14px', color: '#ccc', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600 }}>
