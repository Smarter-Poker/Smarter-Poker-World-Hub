...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      const [dailyRes, playersRes] = await Promise.all([
        fetch(`/api/club-commander/analytics/daily?venue_id=${venueId}&period=${period}`),
        fetch(`/api/club-commander/analytics/players?venue_id=${venueId}&period=${period}&limit=10`)
      ]);
  ...
  return (
    <>
      <Head>
        <title>Analytics | Club Commander</title>
