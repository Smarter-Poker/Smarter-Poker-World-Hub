/**
 * Club Commander Staff Dashboard - Main terminal view
...
import GameGrid from '../../src/components/club-commander/staff/GameGrid';
import WaitlistManager from '../../src/components/club-commander/staff/WaitlistManager';
import QuickActions from '../../src/components/club-commander/staff/QuickActions';
import ActivityFeed, { createActivity } from '../../src/components/club-commander/staff/ActivityFeed';
import OpenGameModal from '../../src/components/club-commander/modals/OpenGameModal';
import AddWalkInModal from '../../src/components/club-commander/modals/AddWalkInModal';
import SeatPlayerModal from '../../src/components/club-commander/modals/SeatPlayerModal';
import { useRealtimeUpdates } from '../../src/lib/club-commander/useRealtimeUpdates';

export default function ClubCommanderDashboard() {
  ...
  const navItems = [
    { href: '/club-commander/dashboard', label: 'Dashboard', icon: Home, active: true },
    { href: '/club-commander/tables', label: 'Tables', icon: Grid3X3 },
    { href: '/club-commander/tournaments', label: 'Tournaments', icon: Trophy },
    { href: '/club-commander/promotions', label: 'Promotions', icon: Gift },
    { href: '/club-commander/analytics', label: 'Analytics', icon: BarChart3 },
    { href: '/club-commander/qr-code', label: 'Check-In QR', icon: QrCode },
    { href: '/club-commander/staff', label: 'Staff', icon: Users },
    { href: '/club-commander/settings', label: 'Settings', icon: Settings },
  ];
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!staffData.venue_id) {
        localStorage.removeItem('club_commander_staff');
        router.push('/club-commander/login');
        return;
      }
  ...
  const fetchData = useCallback(async (showRefreshing = false) => {
    ...
    try {
      const [venueRes, gamesRes, waitlistRes, tablesRes] = await Promise.all([
        ...
        fetch(`/api/club-commander/tables/venue/${venueId}`)
      ]);
  ...
  async function handleCallPlayer(player) {
    try {
      const res = await fetch(`/api/club-commander/waitlist/${player.id}/call`, {
        method: 'POST',
  ...
  async function handleRemovePlayer(player) {
    ...
    try {
      const res = await fetch(`/api/club-commander/waitlist/${player.id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <>
      <Head>
        <title>{venue?.name || 'Dashboard'} | Smarter Club Commander</title>
        ...
        <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          <section>
            <QuickActions
              ...
              onManageTables={() => router.push('/club-commander/tables')}
              onSendAnnouncement={() => router.push('/club-commander/announcements')}
              onSettings={() => router.push('/club-commander/settings')}
