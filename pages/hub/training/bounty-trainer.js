...
const BOUNTY_FORMATS = {
    ko: {
        id: 'ko', name: 'Knockout (KO)', color: '#ef4444',
    ...
    pko: {
        id: 'pko', name: 'Progressive KO (PKO)', color: '#f59e0b',
    ...
    tko: {
        id: 'tko', name: 'Total Knockout (TKO)', color: '#a855f7',
    ...
    mystery: {
        id: 'mystery', name: 'Mystery Bounty', color: '#06b6d4',
...
function calculateBountyEquity(format, stacks, bounties, heroStack, heroBounty, villainStack, villainBounty, pot) {
    ...
    return {
        ...
        bountyValue: bountyEV.toFixed(0),
        adjustedEquity: Math.min(99, bountyAdjEq * 100).toFixed(1),
        callingThreshold: Math.max(25, (50 - bountyEV / 2)).toFixed(1),
    };
}
...
const DRILL_SCENARIOS = [
    {
        id: 1, title: 'Short Stack Shove vs Big Bounty',
        situation: 'Villain (5BB) shoves. Their bounty is worth 30% of your stack. You have A9o.',
        format: 'pko', heroStack: 25, villainStack: 5, bounty: 7.5, pot: 1.5,
        correctAction: 'call', gtoExplanation: 'Bounty equity widens your calling range significantly. A9o is a clear call.',
    },
    {
        id: 2, title: 'Bubble with Bounty Consideration',
        situation: 'ICM bubble. Villain (12BB) shoves. Bounty = $100. You have KQs (20BB).',
        format: 'ko', heroStack: 20, villainStack: 12, bounty: 100, pot: 1.5,
        correctAction: 'call', gtoExplanation: 'The bounty overcomes ICM pressure. KQs has enough equity + bounty value.',
    },
    {
        id: 3, title: 'Deep Stack PKO — Marginal Spot',
        situation: 'Effective 40BB. Villain 3-bet shoves with growing bounty. You have JTs.',
        format: 'pko', heroStack: 40, villainStack: 40, bounty: 20, pot: 6.5,
        correctAction: 'fold', gtoExplanation: 'Despite the bounty, risking 40BB with JTs is -EV. Bounty doesn\'t overcome the risk.',
    },
    {
        id: 4, title: 'Mystery Bounty — High Variance Call',
        situation: 'Medium stack. Villain shoves 8BB with a mystery bounty. You have 55.',
        format: 'mystery', heroStack: 30, villainStack: 8, bounty: 15, pot: 1.5,
        correctAction: 'call', gtoExplanation: 'Mystery bounty EV makes this a call. 55 has ~43% equity and the bounty adds significant value.',
    },
    {
        id: 5, title: 'TKO — Everyone is a Bounty',
        situation: 'All prize money is bounties. Villain (15BB) shoves. You have A5s (22BB).',
        format: 'tko', heroStack: 22, villainStack: 15, bounty: 15, pot: 1.5,
        correctAction: 'call', gtoExplanation: 'In TKO format, every knockout IS the prize pool. A5s is profitable against typical shoving range.',
    },
    {
        id: 6, title: 'Mini Bounty — Don\'t Overpay',
        situation: 'KO tournament. Villain bounty is only $5. You have 87o facing a 3-bet shove.',
        format: 'ko', heroStack: 30, villainStack: 25, bounty: 5, pot: 7,
        correctAction: 'fold', gtoExplanation: 'A $5 bounty does not justify calling with 87o. Don\'t chase micro-bounties.',
    },
];
    ...
    return (
        <>
            <Head>
                ...
                <meta name="description" content="Master KO, PKO, TKO, and Mystery Bounty tournament strategy" />
            </Head>
            ...
            <div style={{ minHeight: '100vh', background: '#18191a', color: '#e4e6eb', fontFamily: "'Inter', -apple-system, sans-serif" }}>
                ...
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #3a3b3c' }}>
                    ...
                    <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, fontFamily: "'Rajdhani', sans-serif" }}>Bounty Tournament Trainer</h1>
                    <p style={{ fontSize: 14, color: '#b0b3b8', margin: '2px 0 0' }}>Master KO, PKO, TKO, and Mystery Bounty strategy adjustments</p>
                </div>
                ...
                <div style={{ padding: '16px 20px', maxWidth: 900, margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: 3 }}>
                        {[{ id: 'learn', label: 'Learn' }, { id: 'drill', label: 'Drill' }, { id: 'calculator', label: 'Calculator' }].map(t => (
                            <button
                                key={t.id}
                                ...
                                style={{
                                    ...
                                    border: 'none', color: activeTab === t.id ? '#000' : '#b0b3b8',
                                }}
                            >
                                {t.label}
                            </button>
                    ...
                    {/* Learn Tab */}
                    {activeTab === 'learn' && (
                        <div style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${format.color}30`, borderRadius: 12, padding: 20 }}>
                            <h2 style={{ fontSize: 18, fontWeight: 700, color: format.color, margin: '0 0 12px', fontFamily: "'Rajdhani', sans-serif" }}>
                                {format.name}
                            </h2>
                            <p style={{ fontSize: 14, color: '#e4e6eb', lineHeight: 1.6, marginBottom: 16 }}>{format.desc}</p>
                            <div style={{ padding: '12px 16px', background: 'rgba(0,0,0,0.2)', borderRadius: 8, marginBottom: 12, borderLeft: `3px solid ${format.color}` }}>
                                <div style={{ fontSize: 11, fontWeight: 700, color: format.color, letterSpacing: '0.1em', marginBottom: 4 }}>EXAMPLE</div>
                                <div style={{ fontSize: 13, color: '#e4e6eb' }}>{format.example}</div>
                            </div>
                            <div style={{ padding: '12px 16px', background: 'rgba(0,0,0,0.2)', borderRadius: 8, borderLeft: `3px solid ${format.color}` }}>
                                <div style={{ fontSize: 11, fontWeight: 700, color: format.color, letterSpacing: '0.1em', marginBottom: 4 }}>BOUNTY SPLIT</div>
                                <div style={{ fontSize: 13, color: '#e4e6eb' }}>{format.splitRatio}</div>
                            </div>
                            <div style={{ marginTop: 20 }}>
                                <h3 style={{ fontSize: 14, fontWeight: 700, color: '#e4e6eb', marginBottom: 10 }}>Key Strategy Adjustments</h3>
                                <div style={{ display: 'grid', gap: 8 }}>
                                    <div style={{ padding: '10px 14px', background: 'rgba(34,197,94,0.06)', borderRadius: 8, borderLeft: '3px solid #4ade80' }}>
                                        <div style={{ fontSize: 12, fontWeight: 700, color: '#4ade80', marginBottom: 2 }}>CALL WIDER</div>
                                        <div style={{ fontSize: 13, color: '#e4e6eb' }}>Bounty equity expands your calling range, especially vs short stacks.</div>
                                    </div>
                                    <div style={{ padding: '10px 14px', background: 'rgba(245,158,11,0.06)', borderRadius: 8, borderLeft: '3px solid #f59e0b' }}>
                                        <div style={{ fontSize: 12, fontWeight: 700, color: '#f59e0b', marginBottom: 2 }}>COVER OPPONENTS</div>
                                        <div style={{ fontSize: 13, color: '#e4e6eb' }}>Having the biggest stack means you can collect bounties from everyone.</div>
                                    </div>
                                    <div style={{ padding: '10px 14px', background: 'rgba(99,102,241,0.06)', borderRadius: 8, borderLeft: '3px solid #818cf8' }}>
                                        <div style={{ fontSize: 12, fontWeight: 700, color: '#818cf8', marginBottom: 2 }}>ADJUST ICM</div>
                                        <div style={{ fontSize: 13, color: '#e4e6eb' }}>Traditional ICM undervalues chips in bounty events. Factor in knockout equity.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {activeTab === 'drill' && (
                        <div>
                            ...
                            <div style={{
                                ...
                                padding: '8px 16px', background: 'rgba(255,255,255,0.04)', borderRadius: 8, marginBottom: 12,
                            }}>
                                <span style={{ fontSize: 13, color: '#b0b3b8' }}>
                                    Scenario {(drillIndex % DRILL_SCENARIOS.length) + 1}/{DRILL_SCENARIOS.length}
                                </span>
                                <span style={{ fontSize: 14, fontWeight: 700, color: score > 0 ? '#4ade80' : '#b0b3b8' }}>
                                ...
                                <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                                    {[
                                        ...
                                        <div key={s.label} style={{
                                            ...
                                            <div style={{ fontSize: 10, color: '#b0b3b8', fontWeight: 600 }}>{s.label}</div>
                                            <div style={{ fontSize: 14, fontWeight: 700, color: '#e4e6eb' }}>{s.value}</div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                {!showResult && (
                                    <div style={{ display: 'flex', gap: 8 }}>
                                        <motion.button
                                            ...
                                            style={{
                                                flex: 1, padding: '14px', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: 'pointer',
                                                background: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.05))',
                                                border: '2px solid rgba(34,197,94,0.3)', color: '#4ade80',
                                        ...
                                        <motion.button
                                            ...
                                            style={{
                                                ...
                                                border: '2px solid rgba(239,68,68,0.3)', color: '#f87171',
                                                fontFamily: "'Rajdhani', sans-serif", letterSpacing: '0.08em',
                                            }}
                                        >
                                            FOLD
                                ...
                                <AnimatePresence>
                                    {showResult && (
                                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                                            <div style={{
                                                padding: '14px 16px', borderRadius: 10, marginTop: 12,
                                                background: selectedAnswer === scenario.correctAction ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)',
                                                ...
                                                <div style={{
                                                    ...
                                                    color: selectedAnswer === scenario.correctAction ? '#4ade80' : '#f87171',
                                                }}>
                                                    {selectedAnswer === scenario.correctAction ? 'CORRECT' : `INCORRECT — Correct: ${scenario.correctAction.toUpperCase()}`}
                                                </div>
                                            ...
                                            <button
                                                ...
                                                style={{
                                                    width: '100%', padding: '12px', borderRadius: 8, marginTop: 10, fontSize: 14, fontWeight: 700,
                                                    background: format.color, border: 'none', color: '#000', cursor: 'pointer',
                                                    fontFamily: "'Rajdhani', sans-serif", letterSpacing: '0.08em',
                                                }}
                                            >
                                                NEXT SCENARIO
                            ...
                            {totalAnswered >= DRILL_SCENARIOS.length && (
                                <button onClick={handleComplete} style={{
                                    width: '100%', padding: '14px', borderRadius: 10, marginTop: 12, fontSize: 15, fontWeight: 700,
                                    background: 'linear-gradient(135deg, #22c55e, #10b981)', border: 'none', color: '#000', cursor: 'pointer',
                                }}>
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 20 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                                {[
                                    ...
                                    { label: 'Villain Stack (BB)', value: calcVillainStack, setter: setCalcVillainStack },
                                    { label: 'Bounty Value ($)', value: calcBounty, setter: setCalcBounty },
                                    { label: 'Pot Size (BB)', value: calcPot, setter: setCalcPot },
                                ].map(f => (
                                    <div key={f.label}>
                                        <label style={{ fontSize: 11, fontWeight: 600, color: '#b0b3b8', display: 'block', marginBottom: 4 }}>{f.label}</label>
                                        <input
                                            type="number"
                                            value={f.value}
                                            onChange={e => {
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                                {[
                                    ...
                                    { label: 'Adjusted Equity', value: `${calcResult.adjustedEquity}%`, color: '#22c55e' },
                                    { label: 'Calling Threshold', value: `${calcResult.callingThreshold}%`, color: '#a855f7' },
                                ].map(r => (
                                    <div key={r.label} style={{
                                        padding: '12px 14px', background: `${r.color}08`, borderRadius: 8,
                                        border: `1px solid ${r.color}25`,
                                    }}>
                                        <div style={{ fontSize: 11, fontWeight: 700, color: r.color, letterSpacing: '0.08em', marginBottom: 4 }}>{r.label.toUpperCase()}</div>
