...
function getCall(position, action) {
  ...
  if (action === '3bet')
    return new Set(['99', '88', '77', 'AJs', 'ATs', 'KJs', 'JTs', 'T9s', '87s']);
  return new Set();
}

function handKey(r1, r2, suited) {
  const i = RANKS.indexOf(r1),
    j = RANKS.indexOf(r2);
  if (i === j) return r1 + r2;
...
const TIPS = {
  ...
  '3bet':
    '3-bet ranges use a polar strategy: strong value hands (AA-JJ, AK) + blocker bluffs (A5s, A4s, J9s). Avoid 3-bet/folding with medium hands.',
    ...
    for (let i = 0; i < 13; i++) {
      for (let j = 0; j < 13; j++) {
        ...
        out.push({
          key,
          act,
          label: isPair ? r1 + r2 : r1 + r2 + (isSuited ? 's' : 'o'),
          short: r1 + (r1 === r2 ? r2 : r2 + (isSuited ? 's' : 'o')),
        });
      }
    }
    return out;
          ...
          return (
            <motion.div
              ...
              style={{
                ...
                border: `1px solid ${hov ? '#fff' : c.border}`,
                borderRadius: 2,
                display: 'flex',
                alignItems: 'center',
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="GTO Wizard-style range advisor. See solver-recommended opening and 3-bet ranges by position."
        />
        <link
                ...
                {hovered.key && (
                  <motion.div
                    ...
                    style={{
                      marginTop: 10,
                      padding: '8px 14px',
                      background: ACTION_COLORS[hovered.act]?.bg || ACTION_COLORS.fold.bg,
                      border: `1px solid ${ACTION_COLORS[hovered.act]?.border || '#333'}`,
                      borderRadius: 8,
                      display: 'flex',
                      alignItems: 'center',
                      gap: 10,
