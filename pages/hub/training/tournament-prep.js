                    ...
                    <div style={styles.sidebar}>
                        ...
                        <div style={{ ...styles.card, marginTop: 16 }}>
                            ...
                            <div style={styles.statusRow}>
                                ...
                                <span style={styles.statusValue}>{currentBlindInfo.sb}/{currentBlindInfo.bb}</span>
                            </div>
                            <div style={styles.statusRow}>
                    ...
                    <div style={styles.main}>
                        <div style={styles.tabs}>
                            <button
                                ...
                                style={{ ...styles.tab, borderBottom: activeTab === 'structure' ? '2px solid #00d4ff' : '2px solid transparent', color: activeTab === 'structure' ? '#fff' : '#64748b' }}
                            >
                                Structure Flow
                            </button>
                            <button
                                onClick={() => setActiveTab('pushfold')}
                                style={{ ...styles.tab, borderBottom: activeTab === 'pushfold' ? '2px solid #00d4ff' : '2px solid transparent', color: activeTab === 'pushfold' ? '#fff' : '#64748b' }}
                            >
                                Push/Fold Engine
                            </button>
                        </div>
                                ...
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                    ...
                                    <div style={styles.table}>
                                        ...
                                        {bls.map((b) => {
                                            ...
                                            return (
                                                <div key={b.level} style={{ ...styles.tableRow, background: currentLevel === b.level ? 'rgba(0,212,255,0.1)' : 'transparent', borderLeft: currentLevel === b.level ? '3px solid #00d4ff' : '3px solid transparent' }}>
                                                    <div style={styles.cell}>{b.level}</div>
                                                    <div style={styles.cell}>{b.sb}/{b.bb} ({b.ante})</div>
                                                    <div style={styles.cell}>{Math.max(0, estStack)} ({Math.max(0, bbCount).toFixed(1)} BB)</div>
                                                    <div style={{ ...styles.cell, color: zColor, fontWeight: 'bold' }}>{estStack <= 0 ? 'BUSTED' : zone}</div>
                                                </div>
                                            )
                                        })}
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', gap: 16 }}>
                                        <div style={{ ...styles.card, flex: 1 }}>
                                            ...
                                            <p style={{ color: '#e2e8f0', fontSize: 13, lineHeight: 1.5 }}>
                                                Push: 77+, A9s+, AJo+, KTs+, KQo<br />
                                                <span style={{ color: '#ef4444' }}>Fold everything else.</span> The danger of calling off with medium stacks is immense.
                                            </p>
                                        </div>
                                        <div style={{ ...styles.card, flex: 1 }}>
                                            <h4 style={{ color: '#4ade80', margin: '0 0 12px 0' }}>BTN/Late Position</h4>
                                            <p style={{ color: '#e2e8f0', fontSize: 13, lineHeight: 1.5 }}>
                                                Push: 22+, A2s+, A2o+, K2s+, K8o+, Q8s+, QTo+, J8s+, T8s+, 98s<br />
                                                <span style={{ color: '#4ade80' }}>Expand shoving range</span> to exploit tight blinds.
                                            </p>
                                        </div>
                                    </div>
                                    <div style={{ marginTop: 20, padding: 16, background: 'rgba(0,212,255,0.05)', borderRadius: 12, border: '1px solid rgba(0,212,255,0.1)' }}>
                                        <h4 style={{ color: '#fff', margin: '0 0 8px 0' }}>💡 Strategy Tip (Level {currentLevel})</h4>
