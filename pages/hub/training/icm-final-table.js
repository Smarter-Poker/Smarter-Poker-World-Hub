  ...
  const stackDistributions = [
    {
      ...
      stacks: () =>
        Array.from({ length: players }, (_, i) =>
          i === 0 ? 45 : Math.max(5, 55 / (players - 1) - i * 2)
        ),
    },
    {
      label: 'Short Stack',
      stacks: () =>
        Array.from({ length: players }, (_, i) =>
          i === 0 ? 8 : Math.max(10, 92 / (players - 1) - i)
        ),
    },
    { label: 'Even Stacks', stacks: () => Array.from({ length: players }, () => 100 / players) },
    {
      label: 'Two Big / Rest Short',
      stacks: () =>
        Array.from({ length: players }, (_, i) => (i < 2 ? 30 : Math.max(5, 40 / (players - 2)))),
    },
  ];
    ...
    handCategories.forEach((hand, hi) => {
      positions.forEach((pos, pi) => {
        scenarios.push({
          ...
          stacks: stacks.map((s) => {
            const n = Number(s);
            return Number.isFinite(n) ? (Number.isFinite(Number(n)) ? Number(n) : 0).toFixed(1) : '0.0';
          }),
          heroStack: (() => {
            const n = Number(stacks[pi]);
            return Number.isFinite(n) ? (Number.isFinite(Number(n)) ? Number(n) : 0).toFixed(1) : '0.0';
          })(),
          heroPos: pos,
          hand: hand.label,
...
export default function ICMFinalTableLibrary() {
  ...
  const scenarios = useMemo(
    () => generateFTScenarios(structure, payoutType),
    [structure, payoutType]
  );
        ...
        <div style={{ padding: '16px 20px', maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            >
              ...
              {['UTG', 'MP', 'HJ', 'CO', 'BTN', 'SB', 'BB'].slice(0, structure.players).map((p) => (
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
            {(PAYOUT_STRUCTURES[payoutType] || PAYOUT_STRUCTURES.standard).payouts
              .slice(0, structure.players)
              .map((p, i) => (
                <div
                  key={i}
                  style={{
                    ...
                    setScenariosViewed((prev) => {
                      ...
                      if (next % 5 === 0) {
                        eventBus?.emit?.('training:session-complete', {
                          game_id: 'icm-final-table',
                          accuracy: 100,
                          correct_answers: next,
                          total_questions: next,
                          hands_played: next,
                        });
                      }
                      return next;
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: '#e4e6eb' }}>
                      {s.heroPos}
                    </span>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 16 }}>
              <button
                ...
                style={{
                  ...
                  color: page === 0 ? 'rgba(255,255,255,0.2)' : '#b0b3b8',
                }}
              >
                Prev
              </button>
              <span style={{ fontSize: 13, color: '#b0b3b8', padding: '6px 8px' }}>
                {page + 1} / {totalPages}
              </span>
              <button
                ...
                style={{
                  ...
                  background: 'rgba(255,255,255,0.04)',
                  border: '1px solid rgba(255,255,255,0.08)',
                  color: page >= totalPages - 1 ? 'rgba(255,255,255,0.2)' : '#b0b3b8',
                }}
              >
                Next
              </button>
