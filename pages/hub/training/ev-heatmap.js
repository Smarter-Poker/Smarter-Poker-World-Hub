...
const BASE_STATS = {
  ...
  'UTG-River': { ev: 0.1, sample: 10 },
  'HJ-River': { ev: 0.2, sample: 12 },
  'CO-River': { ev: 0.4, sample: 18 },
  ...
  const activeData = useMemo(() => {
    ...
    for (const [key, value] of Object.entries(BASE_STATS || {})) {
      ...
      if (activeFilter === 'Short Deck') {
        if (key.includes('Preflop') && (key.includes('SB') || key.includes('BB')))
          evMod += 0.3; // Flatter equity
        else evMod -= 0.2;
      }
      adjusted[key] = { ev: Number((Number.isFinite(Number(evMod)) ? Number(evMod) : 0).toFixed(2)), sample: value.sample };
    }
    return adjusted;
  }, [activeFilter]);
      ...
      >
        ...
        <div style={{ padding: '20px 16px', maxWidth: 800, margin: '0 auto' }}>
          ...
          >
            ...
            <div
              style={{ width: 24, height: 12, borderRadius: 2, background: getCellColor(-0.8) }}
            />
            <div
              style={{ width: 24, height: 12, borderRadius: 2, background: getCellColor(-0.3) }}
            />
            ...
            {selectedCell && (
              ...
              >
                ...
                >
                  ...
                  >
                    {activeData[selectedCell].sample} hands analyzed
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div
                    style={{
                      padding: '12px 20px',
                      borderRadius: 12,
                      background: getCellColor(activeData[selectedCell].ev),
                      color: '#fff',
                      fontSize: 28,
                      fontWeight: 900,
                    }}
