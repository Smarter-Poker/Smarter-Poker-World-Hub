...
export default function RiskAnalyzerPage() {
    ...
    const runMonteCarlo = async () => {
        ...
        try {
            ...
            eventBus.emit(EventType.SESSION_END, { accuracy: 100, questionsAnswered: 1, questionsCorrect: 1 }, 'risk-analyzer');
    ...
    return (
        <>
            ...
            <div style={{ minHeight: '100vh', background: '#0B0D11', color: '#e2e8f0', fontFamily: "'Inter', sans-serif", paddingBottom: 60 }}>
                {/* Header */}
                <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center' }}>
                    ...
                    <div><div style={{ fontSize: 16, fontWeight: 700 }}>Risk Analyzer</div><div style={{ fontSize: 11, color: '#64748b' }}>Monte Carlo Bankroll Simulator</div></div>
                </div>

                <div style={{ maxWidth: 1000, margin: '40px auto', padding: '0 20px', display: 'flex', flexWrap: 'wrap', gap: 32 }}>

                    {/* Controls */}
                    <div style={{ flex: '1 1 350px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, padding: 32 }}>
                        <div style={{ marginBottom: 24 }}>
                            ...
                            <input type="range" min="-5" max="25" step="0.5" value={winRate} onChange={e => setWinRate(e.target.value)} style={{ width: '100%', accentColor: '#4ade80' }} />
                        </div>
                        ...
                        <div style={{ marginBottom: 24 }}>
                            ...
                            <input type="range" min="40" max="180" step="5" value={stdDev} onChange={e => setStdDev(e.target.value)} style={{ width: '100%', accentColor: '#fbbf24' }} />
                        ...
                        <motion.button
                            ...
                            style={{ width: '100%', padding: '16px', background: 'linear-gradient(135deg, #3b82f6, #6366f1)', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 800, cursor: 'pointer', boxShadow: '0 8px 24px rgba(59,130,246,0.3)' }}
                        >
                    ...
                    <div style={{ flex: '2 1 450px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                        {riskOfRuin !== null && !simulating ? (
                            <AnimatePresence>
                                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ padding: 32, background: 'rgba(0,0,0,0.3)', borderRadius: 24, border: `1px solid ${riskOfRuin > 5 ? 'rgba(239,68,68,0.3)' : 'rgba(74,222,128,0.3)'}` }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                                        <div>
                                            <div style={{ fontSize: 12, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 2 }}>Risk of Ruin</div>
                                            <div style={{ fontSize: 56, fontWeight: 900, color: riskOfRuin > 5 ? '#ef4444' : '#4ade80', letterSpacing: '-1px', lineHeight: 1 }}>
                                                {riskOfRuin < 0.1 && riskOfRuin > 0 ? '< 0.1' : riskOfRuin.toFixed(1)}%
                                            </div>
                                        </div>
                                        <div style={{ width: '50%', fontSize: 13, color: '#cbd5e1', lineHeight: 1.5, background: 'rgba(255,255,255,0.03)', padding: 16, borderRadius: 12 }}>
                                            {riskOfRuin === 100 ? "Mathematical certainty of going broke. Your win rate must be positive." :
                                                riskOfRuin > 10 ? "Extremely dangerous bankroll management. Increase your bankroll or drop stakes immediately." :
                                                    riskOfRuin > 5 ? "Aggressive shot-taking. Prepare to move down if a downswing occurs." :
                                                        riskOfRuin > 1 ? "Standard professional risk tolerance. Bankroll is adequate for variance." :
                                                            "Ultra-safe bankroll. You are statistically bulletproof against standard downswings."}
                                        </div>
                                    </div>

                                    {/* Mini SVG Chart of standard paths */}
                                    <div style={{ height: 200, position: 'relative', borderBottom: '1px solid rgba(255,255,255,0.1)', borderLeft: '1px solid rgba(255,255,255,0.1)' }}>
                                        {/* Chart Zero Line */}
                                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, borderBottom: '1px dashed rgba(239,68,68,0.5)', zIndex: 0 }} />

                                        <svg width="100%" height="100%" preserveAspectRatio="none" viewBox={`0 0 100 100`} style={{ overflow: 'visible' }}>
                                            {chartData.map((pathObj) => {
                                                const maxBankroll = Math.max(parseFloat(bankroll) * 4, ...chartData.flatMap(p => p.data));
                                                // Generate points normalized 0-100
                                                const points = pathObj.data.map((val, idx) => {
                                        ...
                                        <div style={{ position: 'absolute', bottom: -20, right: 0, fontSize: 10, color: '#64748b' }}>10,000 Hands</div>
                                        <div style={{ position: 'absolute', top: -20, left: 0, fontSize: 10, color: '#64748b' }}>Bankroll</div>
                                    </div>
                                    <div style={{ fontSize: 11, color: '#64748b', textAlign: 'center', marginTop: 32 }}>Sample subset of 20 careers plotted out of 1,000 simulated.</div>
                                </motion.div>
                            </AnimatePresence>
                        ) : (
                            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: simulating ? 1 : 0.4, background: 'rgba(255,255,255,0.02)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
                                {simulating ? (
                                    <>
                                        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, ease: 'linear', duration: 1 }} style={{ width: 60, height: 60, border: '4px solid rgba(59,130,246,0.2)', borderTopColor: '#3b82f6', borderRadius: '50%', marginBottom: 24 }} />
                                        <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>Simulating 1,000 Careers...</div>
                                        <div style={{ fontSize: 14, color: '#94a3b8', marginTop: 8 }}>Rolling 10,000,000 hands of variance.</div>
                                    </>
                                ) : (
                                    <>
                                        <div style={{ fontSize: 50, marginBottom: 20 }}>📉</div>
                                        <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>Simulate Variance</div>
                                        <div style={{ fontSize: 14, color: '#94a3b8', marginTop: 8, textAlign: 'center' }}>Find out if your bankroll can survive<br />the mathematical realities of the game.</div>
                                    </>
                                )}
                            </div>
                        )}
