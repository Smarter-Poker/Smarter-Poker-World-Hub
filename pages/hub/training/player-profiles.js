...
const PROFILES = {
    gto: {
        id: 'gto', name: 'GTO Balanced', color: '#22c55e', border: 'rgba(34,197,94,0.3)',
        desc: 'Perfectly balanced, unexploitable baseline strategy',
        stats: { vpip: 24, pfr: 20, threeBet: 8.5, foldTo3Bet: 52, cBet: 67, foldToCBet: 38, wtsd: 28, aggFactor: 2.8 },
        weaknesses: [],
        exploits: ['None — this is the baseline strategy'],
    },
    nit: {
        id: 'nit', name: 'Ultra Nit', color: '#3b82f6', border: 'rgba(59,130,246,0.3)',
        desc: 'Plays extremely tight. Only enters pots with premium hands.',
        stats: { vpip: 12, pfr: 10, threeBet: 4, foldTo3Bet: 72, cBet: 75, foldToCBet: 45, wtsd: 22, aggFactor: 1.8 },
        weaknesses: ['Over-folds to 3-bets', 'Predictable ranges', 'Folds too much on later streets'],
        exploits: ['3-bet light frequently', 'Steal blinds aggressively', 'Float flop c-bets and bet turn'],
    },
    tag: {
        id: 'tag', name: 'Tight-Aggressive', color: '#06b6d4', border: 'rgba(6,182,212,0.3)',
        desc: 'Solid, tight-aggressive regular. Small pool of leaks.',
        stats: { vpip: 22, pfr: 18, threeBet: 7, foldTo3Bet: 55, cBet: 70, foldToCBet: 40, wtsd: 27, aggFactor: 2.5 },
        weaknesses: ['Slightly over-folds to 3-bets', 'Can be exploited in 4-bet pots'],
        exploits: ['4-bet bluff selectively', 'Attack their blind defense', 'Exploit predictable sizing tells'],
    },
    lag: {
        id: 'lag', name: 'Loose-Aggressive', color: '#f59e0b', border: 'rgba(245,158,11,0.3)',
        desc: 'Wide opening ranges with aggressive postflop play.',
        stats: { vpip: 32, pfr: 26, threeBet: 12, foldTo3Bet: 40, cBet: 78, foldToCBet: 32, wtsd: 30, aggFactor: 3.5 },
        weaknesses: ['Overbluffs in marginal spots', 'Wide ranges are capped on many boards', 'Susceptible to check-raises'],
        exploits: ['Trap with premium hands', 'Check-raise more frequently', 'Call down lighter against their bluffs'],
    },
    callingStation: {
        id: 'callingStation', name: 'Calling Station', color: '#a855f7', border: 'rgba(168,85,247,0.3)',
        desc: 'Passive player who calls too much and rarely raises.',
        stats: { vpip: 42, pfr: 8, threeBet: 2, foldTo3Bet: 30, cBet: 35, foldToCBet: 18, wtsd: 42, aggFactor: 0.8 },
        weaknesses: ['Calls with weak holdings', 'Never folds draws', 'Rarely raises for value'],
        exploits: ['Value bet thinner (top pair is good)', 'Never bluff', 'Bet large with strong hands for max value'],
    },
    maniac: {
        id: 'maniac', name: 'Maniac', color: '#ef4444', border: 'rgba(239,68,68,0.3)',
        desc: 'Hyper-aggressive. Raises and re-raises with a very wide range.',
        stats: { vpip: 55, pfr: 42, threeBet: 18, foldTo3Bet: 25, cBet: 88, foldToCBet: 50, wtsd: 35, aggFactor: 4.2 },
        weaknesses: ['Range is extremely wide and weak', 'Overbluffs constantly', 'Susceptible to 4-bet traps'],
        exploits: ['Flat call pre with premium hands', 'Let them hang themselves', 'Check-call more, let them barrel off'],
    },
    fish: {
        id: 'fish', name: 'Recreational Fish', color: '#f472b6', border: 'rgba(244,114,182,0.3)',
        desc: 'Inexperienced player. Random bet sizing, calls with anything.',
        stats: { vpip: 52, pfr: 10, threeBet: 3, foldTo3Bet: 60, cBet: 40, foldToCBet: 25, wtsd: 38, aggFactor: 0.5 },
        weaknesses: ['No concept of position', 'Chases draws at any price', 'Sizes bets randomly'],
        exploits: ['Isolate with wider range', 'Bet big for value', 'Avoid fancy plays — keep it simple'],
    },
};
...
function computeAdaptation(profileId) {
    ...
    const threeBetAdj = s.foldTo3Bet > 55 ? '+6% (3-bet light)' : s.threeBet > 12 ? '-4% (flat more)' : '+2%';
    const foldAdj = s.threeBet > 12 ? '+10% (fold marginal)' : s.threeBet < 5 ? '-8% (defend wider)' : '0%';

    // Postflop adjustments
    const cBetAdj = s.foldToCBet > 40 ? '+15% (c-bet more)' : s.foldToCBet < 25 ? '-20% (check more)' : '+5%';
    const valueAdj = s.wtsd > 35 ? '+12% (thinner value)' : s.wtsd < 25 ? '-8% (tighten value)' : '0%';
    const bluffAdj = s.foldToCBet > 40 ? '+10% (bluff more)' : s.wtsd > 35 ? '-15% (never bluff)' : '0%';
...
const SCENARIOS = [
    ...
    { id: 4, spot: 'Villain donk-bets 75% on K♣ T♠ 4♥', action: 'Facing Donk Bet', board: 'K♣ T♠ 4♥' },
    { id: 5, spot: 'Turn barrel: J♠ 8♥ 3♦ → 5♣', action: 'Multi-Street Barrel', board: 'J♠ 8♥ 3♦ 5♣' },
    { id: 6, spot: 'River decision: Q♥ 9♠ 6♦ 2♣ → K♦', action: 'River Value/Bluff', board: 'Q♥ 9♠ 6♦ 2♣ K♦' },
];
    ...
    return (
        <>
            <Head>
                ...
                <meta name="description" content="Apply exploitative opponent profiles and see how GTO strategy adapts" />
            </Head>
                ...
                <div style={{
                    ...
                    <div>
                        <button onClick={() => router.back()} style={{
                            background: 'none', border: 'none', color: '#b0b3b8', fontSize: 14, cursor: 'pointer', marginBottom: 4,
                        }}>Back to Training</button>
                        <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, fontFamily: "'Rajdhani', sans-serif" }}>
                ...
                <div style={{ padding: '16px 20px', maxWidth: 900, margin: '0 auto' }}>
                    ...
                    <div style={{
                        ...
                        {Object.values(PROFILES).map(p => (
                            ...
                            >
                                <div style={{ fontSize: 12, fontWeight: 700, color: p.color, fontFamily: "'Rajdhani', sans-serif", letterSpacing: '0.05em' }}>
                                    {p.name.toUpperCase()}
                                </div>
                                <div style={{ fontSize: 11, color: '#b0b3b8', textAlign: 'center', lineHeight: 1.3 }}>
                                    {p.desc.split('.')[0]}
                                </div>
                            </motion.button>
                        ))}
                    ...
                    <div style={{
                        background: 'rgba(255,255,255,0.03)', border: `1px solid ${profile.border}`,
                        borderRadius: 12, padding: 20, marginBottom: 16,
                    }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <h2 style={{ fontSize: 18, fontWeight: 700, color: profile.color, margin: 0, fontFamily: "'Rajdhani', sans-serif" }}>
                                {profile.name} — HUD Stats
                            </h2>
                            <div style={{
                                padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700,
                                background: `${profile.color}20`, color: profile.color, letterSpacing: '0.08em',
                            }}>
                                VPIP/PFR: {profile.stats.vpip}/{profile.stats.pfr}
                            </div>
                        </div>

                        <StatBar label="VPIP" value={profile.stats.vpip} gtoValue={gto.vpip} color={profile.color} />
                        <StatBar label="PFR" value={profile.stats.pfr} gtoValue={gto.pfr} color={profile.color} />
                        <StatBar label="3-Bet" value={profile.stats.threeBet} gtoValue={gto.threeBet} max={25} color={profile.color} />
                        <StatBar label="Fold to 3-Bet" value={profile.stats.foldTo3Bet} gtoValue={gto.foldTo3Bet} color={profile.color} />
                        <StatBar label="C-Bet" value={profile.stats.cBet} gtoValue={gto.cBet} color={profile.color} />
                        <StatBar label="Fold to C-Bet" value={profile.stats.foldToCBet} gtoValue={gto.foldToCBet} color={profile.color} />
                        <StatBar label="WTSD" value={profile.stats.wtsd} gtoValue={gto.wtsd} max={50} color={profile.color} />
                        <StatBar label="Aggression Factor" value={profile.stats.aggFactor * 10} gtoValue={gto.aggFactor * 10} max={50} color={profile.color} />
                    </div>
                    ...
                    {profile.id !== 'gto' && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                            <div style={{
                                ...
                                <h3 style={{ fontSize: 13, fontWeight: 700, color: '#f87171', margin: '0 0 10px', letterSpacing: '0.08em' }}>
                                    WEAKNESSES
                                </h3>
                                {profile.weaknesses.map((w, i) => (
                                    <div key={i} style={{ fontSize: 13, color: '#e4e6eb', marginBottom: 6, paddingLeft: 12, borderLeft: '2px solid rgba(239,68,68,0.3)' }}>
                                        {w}
                                    </div>
                                ))}
                            </div>
                            <div style={{
                                ...
                                <h3 style={{ fontSize: 13, fontWeight: 700, color: '#4ade80', margin: '0 0 10px', letterSpacing: '0.08em' }}>
                                    OPTIMAL EXPLOITS
                                </h3>
                                {profile.exploits.map((e, i) => (
                                    <div key={i} style={{ fontSize: 13, color: '#e4e6eb', marginBottom: 6, paddingLeft: 12, borderLeft: '2px solid rgba(34,197,94,0.3)' }}>
                                        {e}
                                    </div>
                                ))}
                            </div>
                    ...
                    <motion.button
                        ...
                        style={{
                            ...
                            border: '1px solid rgba(99,102,241,0.25)', display: 'flex',
                            justifyContent: 'space-between', alignItems: 'center', marginBottom: 16,
                        }}
                    >
                        <span style={{ fontSize: 14, fontWeight: 700, color: '#a5b4fc', fontFamily: "'Rajdhani', sans-serif", letterSpacing: '0.08em' }}>
                            GTO ADAPTATION ENGINE
                        </span>
                            ...
                            >
                                <div style={{
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                                        <div>
                                            <h4 style={{ fontSize: 12, fontWeight: 700, color: '#818cf8', margin: '0 0 10px', letterSpacing: '0.1em' }}>
                                                PREFLOP ADJUSTMENTS
                                            </h4>
                                            {Object.entries(adaptation.preflopAdj).map(([key, val]) => (
                                                <div key={key} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                                    <span style={{ fontSize: 12, color: '#b0b3b8' }}>{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                                                    <span style={{ fontSize: 13, fontWeight: 700, color: val.includes('+') ? '#4ade80' : val.includes('-') ? '#f87171' : '#b0b3b8' }}>
                                                        {val}
                                                    </span>
                                                </div>
                                            ))}
                                    ...
                                    <div style={{
                                        ...
                                        borderRadius: 8, fontSize: 13, color: '#e4e6eb', lineHeight: 1.5,
                            ...
                            {SCENARIOS.map((s, i) => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: activeScenario === i ? `${profile.color}20` : 'rgba(255,255,255,0.04)',
