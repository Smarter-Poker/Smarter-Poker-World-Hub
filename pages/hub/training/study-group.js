...
export default function StudyGroupRoom() {
    ...
    const handleCreateRoom = () => {
        ...
        setMessages([
            { id: 1, sender: 'System', text: `Room "${roomName}" created. Waiting for others to join...`, isSystem: true, time: new Date().toLocaleTimeString() }
        ]);
    };
    ...
    return (
        <PageTransition>
            ...
            <div style={styles.container}>
                ...
                {isCreating ? (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={styles.createBox}>
                        <h2>Create a New Study Room</h2>
                        <p style={{ color: '#94a3b8', marginBottom: 24 }}>Invite friends, upload hands, and analyze GTO lines together.</p>
                    ...
                    <div style={styles.roomLayout}>
                        ...
                        <div style={styles.mainCol}>
                            <div style={styles.participantsBar}>
                                <div style={{ fontWeight: 700, color: '#94a3b8', fontSize: 12, marginRight: 16 }}>ONLINE (3)</div>
                                {participants.map(p => (
                                    <div key={p.id} style={styles.participantChip}>
                                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#4ade80', marginRight: 8 }} />
                                        {p.name} {p.role === 'Admin' && <span style={{ color: '#fbbf24', marginLeft: 4 }}>👑</span>}
                                    </div>
                                ))}
                            ...
                            <div style={styles.handViewer}>
                                {activeHandInfo ? (
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={styles.activeHand}>
                                        <div style={styles.handHeader}>
                                            <h3 style={{ margin: 0 }}>{activeHandInfo.id} Overview</h3>
                                            <button style={styles.closeBtn} onClick={() => setActiveHandInfo(null)}>✕</button>
                                        </div>
                                    ...
                                    <div style={styles.emptyHandViewer}>
                                        ...
                                        <p style={{ color: '#94a3b8', margin: '0 0 24px 0' }}>Upload a hand history or load a saved bookmark to begin group analysis.</p>
                                        <button onClick={loadDemoHand} style={styles.loadBtn}>Load Demo Hand</button>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div style={styles.chatCol}>
                            ...
                            <div style={styles.chatMessages}>
                                {messages.map(m => (
                                    <div key={m.id} style={{ ...styles.messageWrapper, alignSelf: m.sender === 'You' ? 'flex-end' : 'flex-start' }}>
                                        {m.isSystem ? (
                                            <div style={styles.systemMsg}>{m.text}</div>
                                        ) : (
                                            <div style={{ ...styles.msgBubble, background: m.sender === 'You' ? 'rgba(0, 212, 255, 0.2)' : 'rgba(255,255,255,0.05)', border: m.sender === 'You' ? '1px solid rgba(0, 212, 255, 0.4)' : '1px solid rgba(255,255,255,0.1)' }}>
