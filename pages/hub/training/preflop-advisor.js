...
const RANGES = {
    UTG: { raise: new Set(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', 'AKs', 'AQs', 'AJs', 'ATs', 'A9s', 'KQs', 'KJs', 'QJs', 'AKo', 'AQo', 'AJo']), call: new Set([]) },
    HJ: { raise: new Set(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', 'AKs', 'AQs', 'AJs', 'ATs', 'A9s', 'A8s', 'KQs', 'KJs', 'KTs', 'QJs', 'QTs', 'JTs', 'AKo', 'AQo', 'AJo', 'ATo', 'KQo']), call: new Set([]) },
    CO: { raise: new Set(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', '66', 'AKs', 'AQs', 'AJs', 'ATs', 'A9s', 'A8s', 'A7s', 'A6s', 'A5s', 'A4s', 'A3s', 'A2s', 'KQs', 'KJs', 'KTs', 'K9s', 'QJs', 'QTs', 'Q9s', 'JTs', 'J9s', 'T9s', '98s', '87s', 'AKo', 'AQo', 'AJo', 'ATo', 'A9o', 'KQo', 'KJo', 'KTo', 'QJo']), call: new Set([]) },
    BTN: { raise: new Set(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', '66', '55', '44', '33', '22', 'AKs', 'AQs', 'AJs', 'ATs', 'A9s', 'A8s', 'A7s', 'A6s', 'A5s', 'A4s', 'A3s', 'A2s', 'KQs', 'KJs', 'KTs', 'K9s', 'K8s', 'K7s', 'K6s', 'K5s', 'QJs', 'QTs', 'Q9s', 'JTs', 'J9s', 'T9s', 'T8s', '98s', '97s', '87s', '86s', '76s', '75s', '65s', 'AKo', 'AQo', 'AJo', 'ATo', 'A9o', 'KQo', 'KJo', 'KTo', 'QJo', 'QTo', 'JTo']), call: new Set([]) },
    SB: { raise: new Set(['AA', 'KK', 'QQ', 'JJ', 'TT', '99', '88', '77', '66', '55', 'AKs', 'AQs', 'AJs', 'ATs', 'A9s', 'A8s', 'A7s', 'A6s', 'A5s', 'A4s', 'KQs', 'KJs', 'KTs', 'K9s', 'QJs', 'QTs', 'JTs', 'T9s', '98s', '87s', '76s', 'AKo', 'AQo', 'AJo', 'ATo', 'KQo', 'KJo', 'QJo']), call: new Set([]) },
    BB: { raise: new Set(['AA', 'KK', 'QQ', 'JJ', 'AKs', 'AQs', 'AKo']), call: new Set(['TT', '99', '88', '77', '66', '55', '44', '33', '22', 'AJs', 'ATs', 'A9s', 'A8s', 'A7s', 'A6s', 'A5s', 'A4s', 'A3s', 'A2s', 'KQs', 'KJs', 'KTs', 'K9s', 'K8s', 'QJs', 'QTs', 'Q9s', 'JTs', 'J9s', 'T9s', '98s', '87s', '76s', '65s', 'AJo', 'ATo', 'A9o', 'KQo', 'KJo', 'QJo']) },
};
...
export default function PreflopAdvisor() {
    ...
    useEffect(() => { setHand(dealHand()); }, []);

    const gtoAction = useMemo(() => hand ? getGTOAction(canonicalHand(hand.c1, hand.c2), hand.pos) : null, [hand]);
    ...
    const handleDecision = useCallback((chosen) => {
        ...
        setPosStats(prev => {
            ...
            return { ...prev, [hand.pos]: { correct: p.correct + (isCorrect ? 1 : 0), total: p.total + 1 } };
        });
    ...
    return (
        <>
            <Head>
                ...
                <meta name="description" content="Train GTO preflop decisions by position. Deal hole cards and choose Raise, Call, or Fold against canonical GTO ranges." />
            ...
            <div style={C.page}>
                <div style={{ maxWidth: 560, margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                        <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg,#22c55e20,#00d4ff20)', border: '1px solid rgba(34,197,94,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🃏</div>
                        <div>
                            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 900, ...C.orb, background: 'linear-gradient(135deg,#22c55e,#00d4ff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>PREFLOP ADVISOR</h1>
                            <p style={{ margin: 0, fontSize: 11, color: '#64748b', fontWeight: 600 }}>GTO Decision Trainer · 6-Max · 100BB</p>
                        </div>
                    </div>
                    ...
                    {last20.length > 0 && (
                        <div style={{ display: 'flex', gap: 4, marginBottom: 14, flexWrap: 'wrap' }}>
                            {last20.map((ok, i) => (
                                <div key={i} style={{ width: 10, height: 10, borderRadius: '50%', background: ok ? '#22c55e' : '#ef4444' }} />
                            ))}
                        </div>
                    )}
                    ...
                    {hand && (
                        <AnimatePresence mode="wait">
                            <motion.div key={hand.c1 + hand.c2 + hand.pos} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                                <div style={C.card}>
                                    {/* Position badge */}
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                                        <div style={{ background: 'rgba(168,85,247,0.12)', border: '1px solid rgba(168,85,247,0.3)', borderRadius: 20, padding: '4px 14px', fontSize: 12, fontWeight: 800, color: '#a855f7', ...C.orb }}>
                                            {hand.pos}
                                        </div>
                                        <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>6-Max · 100BB · No Limpers</div>
                                    </div>
                                    ...
                                    <p style={{ margin: 0, textAlign: 'center', fontSize: 14, fontWeight: 600, color: '#94a3b8' }}>
                                        Folded to you in <strong style={{ color: '#a855f7' }}>{hand.pos}</strong>. What is your action?
                                    </p>
                                </div>
                                ...
                                {!showResult && (
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
                                        {(['raise', 'call', 'fold']).map(a => (
                                            <button key={a} onClick={() => handleDecision(a)} style={{
                                                padding: '16px 8px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)',
                                                background: `rgba(${a === 'raise' ? '34,197,94' : a === 'call' ? '249,115,22' : '239,68,68'},0.12)`,
                                                color: ACTION_COLORS[a], fontWeight: 900, fontSize: 14, cursor: 'pointer', ...C.orb,
                                            }}>
                                                {ACTION_ICONS[a]}<br />{a.toUpperCase()}
                                            </button>
                                        ))}
                                    </div>
                                )}
                                ...
                                {showResult && (
                                    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}>
                                        <div style={{
                                            ...
                                            background: decision === gtoAction ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)',
                                        ...
                                        <div style={{ ...C.card, background: 'rgba(255,255,255,0.02)', fontSize: 13, color: '#94a3b8', lineHeight: 1.7 }}>
                                            ...
                                            <strong style={{ color: '#a855f7' }}>{canonical}</strong> — {getRangeNote(hand.pos, canonical)}
                                        </div>
                                                ...
                                                return (
                                                    <div key={p} style={{ marginBottom: 8 }}>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, color: '#64748b', marginBottom: 3 }}>
                                                            <span>{p}</span><span style={{ color: acc >= 70 ? '#22c55e' : '#f97316' }}>{acc}% ({ps.total} hands)</span>
                                                        </div>
                                                        <div style={{ height: 5, borderRadius: 3, background: 'rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                                                            <motion.div animate={{ width: `${acc}%` }} style={{ height: '100%', borderRadius: 3, background: acc >= 70 ? '#22c55e' : '#f97316' }} />
                                                        </div>
                                                    </div>
                                                );
                                            })}
                                        ...
                                        <button onClick={nextHand} style={{ width: '100%', padding: 14, borderRadius: 12, background: 'linear-gradient(135deg,#22c55e,#00d4ff)', border: 'none', color: '#000', fontWeight: 900, fontSize: 14, cursor: 'pointer', ...C.orb }}>
                                            NEXT HAND →
                                        </button>
                                    </motion.div>
                                )}
