...
function CardPicker({ selectedCards, onSelect, usedCards, label }) {
    ...
    return (
        <div style={{ position: 'relative' }}>
            <div style={{ fontSize: 9, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>
                {label}
            </div>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                {[0, 1].map(idx => {
                    ...
                    return (
                        <div
                            key={idx}
                            onClick={() => { setSlot(idx); setOpen(true); }}
                            style={{
                                ...
                                border: card ? '2px solid rgba(0,212,255,0.3)' : '2px dashed rgba(255,255,255,0.15)',
                                position: 'relative',
                            }}
                        >
                            {card ? (
                ...
                {open && (
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                            <span style={{ fontSize: 10, color: '#00d4ff', fontWeight: 700, fontFamily: "'Orbitron', monospace" }}>
                                SELECT CARD
                            </span>
                            <button
                                ...
                                {RANKS.map(rank => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={used}
                                            style={{
                                                width: 24, height: 28, borderRadius: 3, border: 'none',
                                                fontSize: 10, fontWeight: 700, cursor: used ? 'not-allowed' : 'pointer',
                                    ...
                                    return (
                                        <button
                                            ...
                                            style={{
                                                ...
                                                transition: 'all 0.1s',
                                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                flexDirection: 'column', lineHeight: 1, padding: 0,
                                            }}
                                        >
                                            <span style={{ fontSize: 10 }}>{rank}</span>
                                            <span style={{ fontSize: 8 }}>{suit.symbol}</span>
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                {results.map((r, i) => (
                    <div key={i} style={{ textAlign: i === 0 ? 'left' : i === results.length - 1 ? 'right' : 'center', flex: 1 }}>
                        <div style={{
                            fontSize: 22, fontWeight: 900, color: PLAYER_COLORS[i],
            ...
            {/* Bar */}
            <div style={{
                height: 28, borderRadius: 14, overflow: 'hidden', display: 'flex',
            ...
            <div style={{
                ...
                {results.map((r, i) => (
                    <div key={i} style={{
                        ...
                        padding: '8px 12px', flex: '1 1 120px',
                        border: `1px solid ${PLAYER_COLORS[i]}20`,
                    }}>
                        <div style={{
                            fontSize: 10, fontWeight: 700, color: PLAYER_COLORS[i],
                            fontFamily: "'Orbitron', monospace", marginBottom: 4,
                        }}>
                            {formatHand(r.hand)}
                        </div>
                        <div style={{ display: 'flex', gap: 12, fontSize: 10, color: '#94a3b8' }}>
                            <span>Win: <strong style={{ color: '#e2e8f0' }}>{r.wins}</strong></span>
                            <span>Tie: <strong style={{ color: '#e2e8f0' }}>{r.ties}</strong></span>
                        </div>
                    </div>
                ))}
            </div>
...
export default function EquityCalculatorPage() {
    ...
    const usedCards = useMemo(() => {
        const set = new Set();
        hands.forEach(h => h.forEach(c => { if (c) set.add(c); }));
        boardCards.forEach(c => { if (c) set.add(c); });
        return set;
    }, [hands, boardCards]);
    ...
    return (
        <>
            <Head>
                ...
                <meta name="description" content="Calculate hand vs hand equity with Monte Carlo simulation. Compare up to 4 hands pre-flop or on specific boards." />
                ...
                <div style={{ padding: '20px 24px', maxWidth: 700, margin: '0 auto' }}>
                    ...
                    <div style={{
                        ...
                        <p style={{ fontSize: 12, color: '#94a3b8', lineHeight: 1.6, margin: 0 }}>
                            ...
                            optionally add board cards to see how equity changes on different textures.
                            This is a core study tool for understanding pre-flop hand strength and
