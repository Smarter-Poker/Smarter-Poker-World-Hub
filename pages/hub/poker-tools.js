                        ...
                        {deadCards.map((c, i) => (
                            <div key={i} onClick={() => removeCard('dead', 0, i)} style={{ cursor: 'pointer' }}>
                                <img src={getCardImage(c.rank, c.suit)} alt=""
                                    style={{ width: 30, height: 42, borderRadius: 3, border: '1px solid #EF4444', opacity: 0.5 }} loading="lazy" decoding="async">
                            </div>
                        ))}
                    </div>
                )}
                                    ...
                                    return (
                                        <button key={`${rank}-${suit}`}
                                            ...
                                            <img src={imgSrc} alt={`${rank} of ${suit}`}
                                                style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} loading="lazy" decoding="async">
                                        </button>
                                    );
                                })
                            )}
