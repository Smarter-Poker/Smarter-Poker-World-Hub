...
export default function PlayerRewardsPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push('/login?redirect=/hub/club-commander/rewards');
      return;
    }
    fetchRewardsData();
  }, [router]);
  ...
  async function fetchRewardsData() {
    try {
      ...
      const [balanceRes, transactionsRes, ratesRes] = await Promise.all([
        fetch('/api/club-commander/comps/balances', { headers }),
        fetch('/api/club-commander/comps/transactions?limit=20', { headers }),
        fetch('/api/club-commander/comps/rates', { headers })
      ]);
