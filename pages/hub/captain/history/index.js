      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={
          session.status === 'active'
            ? 'cap-badge cap-badge-live'
            : 'cap-badge cap-badge-chrome'
        }>
...
export default function PlayerHistoryPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push('/login?redirect=/hub/captain/history');
      return;
    }
    fetchHistory();
  }, [router, filter]);
  ...
  async function fetchHistory() {
    ...
    try {
      const token = localStorage.getItem('smarter-poker-auth');
      const res = await fetch(`/api/captain/sessions?period=${filter}`, {
        ...
        <header className="cap-header-full text-white">
          <div className="max-w-lg mx-auto px-4 py-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="cap-inset rounded-lg p-3">
