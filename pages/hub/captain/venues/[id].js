      ...
      <button
        ...
        className={
          isFull
            ...
            : 'cap-btn cap-btn-primary px-4 py-2 text-sm transition-colors'
        }
      >
...
export default function VenueDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [venueRes, gamesRes, promosRes] = await Promise.all([
        ...
        fetch(`/api/captain/promotions?venue_id=${id}&active=true`)
      ]);
  ...
  function handleJoinWaitlist(game) {
    router.push(`/hub/captain/waitlist/${id}?game=${game.id}`);
  }

  function handleCheckIn() {
    router.push(`/hub/captain/check-in/${id}`);
  }
      ...
      <div className="cap-page">
        ...
        <main className="max-w-lg mx-auto px-4 py-6 space-y-6">
          ...
          <button
            ...
            className="cap-btn cap-btn-primary w-full h-14 text-lg flex items-center justify-center gap-2"
          >
