...
export default function PlayerCheckInPage() {
  ...
  async function fetchVenueData() {
    try {
      const [venueRes, gamesRes, promosRes] = await Promise.all([
        ...
        fetch(`/api/captain/promotions?venue_id=${venueId}&active=true`)
      ]);
  ...
  async function handleCheckIn() {
    ...
    if (!token) {
      router.push(`/login?redirect=/hub/captain/check-in/${venueId}`);
      return;
    }
    ...
    try {
      const res = await fetch('/api/captain/sessions', {
        method: 'POST',
        headers: {
  ...
  function handleJoinWaitlist() {
    router.push(`/hub/captain/waitlist/${venueId}`);
  }
