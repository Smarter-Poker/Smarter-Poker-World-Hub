...
const STAKES_OPTIONS = ['$0.25/$0.50', '$0.50/$1', '$1/$2', '$1/$3', '$2/$5', '$5/$10', 'Custom'];
...
export default function CreateHomeGamePage() {
  ...
  function getScheduleSummary() {
    ...
    const startStr = formData.start_time
      ? new Date(`2000-01-01T${formData.start_time}`).toLocaleTimeString([], {
          hour: 'numeric',
          minute: '2-digit',
        })
      : '';
    const endStr = formData.end_time
      ? new Date(`2000-01-01T${formData.end_time}`).toLocaleTimeString([], {
          hour: 'numeric',
          minute: '2-digit',
        })
      : '';
  ...
  function isStep2Valid() {
    ...
    if (!Array.isArray(formData.tables) || formData.tables.length !== formData.tables_count)
      return false;
    return formData.tables.every(
      (t) =>
        !!t &&
        !!t.game_type &&
        !!t.stakes &&
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const authUser = await ensureAuthReady(sb);
      console.log(
        '[home-games-create] ensureAuthReady returned:',
      ...
      if (!authUser) {
        ...
        try {
          ...
          console.log(
            '[home-games-create] localStorage fallback user:',
            lsUser ? `id ${lsUser.id}` : 'NULL'
          );
        } catch (lsErr) {
        ...
        if (!lsUser) {
          setError(
            'Cannot verify your session. Please sign in again and try once more. If this keeps happening, send a screenshot of DevTools console.'
          );
          setSubmitting(false);
          return;
        }
        console.warn(
          '[home-games-create] proceeding with localStorage user despite ensureAuthReady=null'
        );
      }
      ...
      if (!token) {
        ...
        console.warn(
          '[home-games-create] using sync getAccessToken fallback:',
          token ? 'ok' : 'NULL'
        );
      }
      if (!token) {
      ...
      const primaryCustomStakes = isMultiTable
        ? formData.tables[0].custom_stakes || ''
        : formData.custom_stakes;
      ...
      const payload = {
        ...
        typical_day:
          formData.recurring && formData.schedule_days.length > 0
      ...
      console.log('[home-games-create] About to POST to /api/commander/home-games/groups', {
      ...
      try {
        ...
        console.log('[home-games-create] fetch returned:', {
          status: res.status,
          ok: res.ok,
          url: res.url,
        });
            ...
            <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-[#64748B]">
                  {step <= 3 ? `Step ${step} of 3` : 'Schedule First Game'}
                </p>
              </div>
            </div>
          </header>
              ...
              <div className="cmd-panel p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <Lock
                        className={`w-5 h-5 mb-2 ${formData.visibility === 'private' ? 'text-[#22D3EE]' : 'text-[#64748B]'}`}
                      />
                    ...
                    >
                      <Globe
                        className={`w-5 h-5 mb-2 ${formData.visibility === 'public' ? 'text-[#22D3EE]' : 'text-[#64748B]'}`}
                      />
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[1, 2, 3, 4, 5, 6].map((n) => (
                      <button
                        ...
                        className={`flex-1 min-w-[44px] h-10 rounded-lg border font-medium transition-colors ${
                          formData.tables_count === n
                ...
                {formData.tables_count > 1 && (
                  <div className="space-y-3">
                    {formData.tables.map((tbl, idx) => (
                      <div
                        key={tbl.id || `tbl-fallback-${idx}`}
                        className="p-4 rounded-lg border border-[#4A5E78]/40 bg-[#0D192E] space-y-3"
                      >
                  ...
                  <div>
                    ...
                    {formData.neighborhood && (
                      <p className="text-sm text-[#22D3EE] mt-1 font-medium">
                        Selected Area: {formData.neighborhood}
                        ...
                        {DAYS_OF_WEEK.map(({ value, label }) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-lg border text-sm font-semibold transition-all ${
                              formData.schedule_days.includes(value)
              ...
              <div className="cmd-panel p-6 space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-[#0D192E] rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-[#64748B]">
                      Run Scheduled Tournaments At This Home Game
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full shadow transition-transform ${formData.schedules_tournaments ? 'translate-x-6' : 'translate-x-0.5'}`}
                    />
                  </button>
                </div>
                ...
                {formData.schedules_tournaments && Array.isArray(formData.tournaments) && (
                  <div className="space-y-4">
                    {formData.tournaments.map((t, idx) => (
                      <div
                        key={idx}
                        className="p-4 bg-[#0D192E] rounded-lg border border-[#4A5E78]/40 space-y-3"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {formData.tournaments.length > 1 && (
                            <button
                              ...
                              onClick={() => {
                                updateField(
                                  'tournaments',
                                  formData.tournaments.filter((_, i) => i !== idx)
                                );
                              }}
                        ...
                        <div className="flex items-center justify-between p-3 bg-[#101D33] rounded-lg">
                          ...
                          >
                            <div
                              className={`w-5 h-5 bg-white rounded-full shadow transition-transform ${t.recurring ? 'translate-x-6' : 'translate-x-0.5'}`}
                            />
                          </button>
                        </div>
                        ...
                        {t.recurring && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {DAYS_OF_WEEK.map(({ value, label }) => {
                                const sel =
                                  Array.isArray(t.recurring_days) &&
                                  t.recurring_days.includes(value);
                                return (
                                  <button
                                    key={value}
                                    type="button"
                                    onClick={() => {
                                      const arr = [...formData.tournaments];
                                      const cur = Array.isArray(arr[idx].recurring_days)
                                        ? arr[idx].recurring_days
                                        : [];
                                      arr[idx] = {
                                        ...arr[idx],
                                        recurring_days: sel
                  ...
                  const checks = [
                    {
                      ...
                      icon: FileText,
                      done: !!(sp.name && sp.description),
                    },
                    { label: 'Profile Photo', icon: ImageIcon, done: !!sp.avatar_url },
                    {
                      label: 'Cover Photo',
                      icon: ImageIcon,
                      done: !!(sp.cover_url || sp.metadata?.cover_photo_url),
                    },
                        ...
                        {checks.map((item, idx) => (
                          <div
                            ...
                            className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                              item.done
                                ? 'bg-[#10B981]/10'
                                : 'bg-[#0D192E] hover:bg-[#132240] cursor-pointer'
                            }`}
                            onClick={() => {
                          ...
                          >
                            ...
                            <span
                              className={`text-sm ${item.done ? 'text-[#10B981] line-through' : 'text-white'}`}
                            >
                              {item.label}
                            </span>
                            {!item.done && (
                              <ChevronRight className="w-3 h-3 text-[#64748B] ml-auto" />
                            )}
                          </div>
                        ))}
                      </div>
              ...
              {showFirstGameForm && (
                <CreateGameForm
                  ...
                  initialData={{
                    ...
                    game_type:
                      formData.game_type === 'tournament' ? '' : formData.game_type || 'nlhe',
                    ...
                    buy_in_min:
                      formData.min_buyin !== '' && formData.min_buyin != null
                        ? String(formData.min_buyin)
                        : '',
                    buy_in_max:
                      formData.max_buyin !== '' && formData.max_buyin != null
                        ? String(formData.max_buyin)
                        : '',
