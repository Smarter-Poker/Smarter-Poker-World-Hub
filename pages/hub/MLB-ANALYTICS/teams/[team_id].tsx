  ...
  if (error || data?.error) {
    return (
      <div className="min-h-screen bg-[#0a0a15] pb-20 font-sans w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
        ...
        <UniversalHeader
          pageDepth={2}
          onBackClick={() => router.push('/hub/MLB-ANALYTICS/teams')}
        />
        <MlbSubNav />
        <main className="max-w-7xl mx-auto px-4 py-12 flex justify-center items-center min-h-[50vh]">
          <div className="text-center bg-[#0d1117] p-8 rounded-xl border-[2px] border-[#00D4FF]/50 shadow-[0_0_20px_rgba(0,212,255,0.15),inset_0_1px_0_rgba(255,255,255,0.05)] relative overflow-hidden">
            ...
            <Link
              ...
              className="mt-6 inline-block bg-[#1a2332] text-white px-6 py-2 rounded-sm border border-[#3d4f5f] text-[10px] font-extrabold tracking-widest uppercase hover:bg-[#2a3a4a] relative z-10"
            >
              BACK TO TEAMS
            </Link>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a15] pb-20 font-sans w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <Link
          href="/hub/MLB-ANALYTICS/teams"
          className="inline-flex items-center text-sm font-bold tracking-wider text-[#94A3B8] hover:text-white transition-colors mb-6 uppercase"
        >
        ...
        {team && (
          <>
            {/* Header Profile */}
            <div
              className={`metal-panel mb-8 flex flex-col md:flex-row items-center md:items-start gap-6 ${hasEdge ? 'edge-glow' : ''}`}
            >
                ...
                {team.grade && (
                  <div className="flex items-center gap-2 mb-4 justify-center md:justify-start">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 tracking-widest uppercase">
                      {team.grade.edgeCount} active
                    </span>
                  </div>
                )}
                <div className="flex flex-wrap justify-center md:justify-start gap-4">
                  <div className="bg-[#000] px-4 py-2 rounded border border-[#3d4f5f]">
                    <div className="text-[10px] text-[#94A3B8] font-bold tracking-widest mb-1">
                      RECORD
                    </div>
                    <div className="text-xl text-white font-black">
                      {team.streaks?.record || '0-0'}
                    </div>
                  </div>
                  <div className="bg-[#000] px-4 py-2 rounded border border-[#3d4f5f]">
                    <div className="text-[10px] text-[#94A3B8] font-bold tracking-widest mb-1">
                    ...
                    <div className="text-xl text-white font-black">
                      {team.streaks?.last10_record || '0-0'}
                    </div>
                  </div>
                  <div className="bg-[#000] px-4 py-2 rounded border border-[#3d4f5f]">
                    <div className="text-[10px] text-[#94A3B8] font-bold tracking-widest mb-1">
            ...
            >
              <style
                dangerouslySetInnerHTML={{ __html: `div::-webkit-scrollbar { display: none; }` }}
              />
            ...
            {activeTab === 'OVERVIEW' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 md:px-0">
                ...
                <div className="metal-panel">
                  <div className="panel-title flex items-center">
                    <TrendingUp size={14} className="mr-2 text-[#00D4FF]" /> TEAM VALUE
                  </div>
                  <div className="stat-grid">
                    <div
                      className="stat-box"
                      style={{ background: 'rgba(0, 212, 255, 0.1)', borderColor: '#00D4FF' }}
                    >
                      <div className="stat-box-title text-[#00D4FF]">wRC+</div>
                      <div className="stat-box-value text-white">
                        {adv.wrc_plus != null ? Number(adv.wrc_plus).toFixed(0) : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">wOBA</div>
                      <div className="stat-box-value text-[#34D399]">
                        {adv.woba != null ? Number(adv.woba).toFixed(3).replace(/^0/, '') : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">OPS</div>
                      <div className="stat-box-value text-[#34D399]">
                        {adv.ops != null ? Number(adv.ops).toFixed(3).replace(/^0/, '') : '-'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="metal-panel">
                  <div className="panel-title flex items-center">
                    <Activity size={14} className="mr-2 text-[#60A5FA]" /> PITCHING METRICS
                  </div>
                  <div className="stat-grid">
                    <div className="stat-box">
                      <div className="stat-box-title">ERA</div>
                      <div className="stat-box-value">
                        {adv.era != null ? Number(adv.era).toFixed(2) : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">FIP</div>
                      <div className="stat-box-value">
                        {adv.fip != null ? Number(adv.fip).toFixed(2) : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">xFIP</div>
                      <div className="stat-box-value">
                        {adv.xfip != null ? Number(adv.xfip).toFixed(2) : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">SIERA</div>
                      <div className="stat-box-value text-[#FCD34D]">
                        {adv.siera != null ? Number(adv.siera).toFixed(2) : '-'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="metal-panel">
                  <div className="panel-title flex items-center">
                    <Activity size={14} className="mr-2 text-[#F472B6]" /> HITTING METRICS
                  </div>
                  <div className="stat-grid">
                    <div className="stat-box">
                      <div className="stat-box-title">AVG</div>
                      <div className="stat-box-value">
                        {adv.avg != null ? Number(adv.avg).toFixed(3).replace(/^0/, '') : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">OBP</div>
                      <div className="stat-box-value">
                        {adv.obp != null ? Number(adv.obp).toFixed(3).replace(/^0/, '') : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">SLG</div>
                      <div className="stat-box-value">
                        {adv.slg != null ? Number(adv.slg).toFixed(3).replace(/^0/, '') : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">HR</div>
                      <div className="stat-box-value">
                        {adv.hr != null ? Number(adv.hr).toFixed(0) : '-'}
                      </div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-box-title">SB</div>
                      <div className="stat-box-value">
                        {adv.sb != null ? Number(adv.sb).toFixed(0) : '-'}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === 'GAMES' && (
              <div className="metal-panel">
                <div className="panel-title flex items-center">
                  <Swords size={14} className="mr-2 text-[#00D4FF]" /> Recent & Upcoming Games
                </div>
                  ...
                  {games.length > 0 ? (
                    games.map((game: any) => {
                      ...
                      return (
                        <div
                          key={game.game_pk}
                          className="p-4 border border-[#3d4f5f] rounded bg-[rgba(0,0,0,0.3)] flex justify-between items-center gap-3 hover:bg-[rgba(255,255,255,0.05)] transition-colors"
                        >
                          <div className="flex flex-col gap-1 min-w-0">
                            <div className="text-[14px] font-bold text-white tracking-wider truncate">
                              <span className="text-slate-500">{game.is_home ? 'vs' : '@'}</span>{' '}
                          ...
                          <div className="text-right flex items-center gap-3 flex-shrink-0">
                            {game.final && game.team_score != null && game.opp_score != null ? (
                              <>
                                {game.result && (
                                  <span
                                    className={`text-[11px] font-black w-5 h-5 flex items-center justify-center rounded ${game.result === 'W' ? 'bg-[rgba(34,197,94,0.15)] text-[#22C55E]' : 'bg-[rgba(239,68,68,0.15)] text-[#EF4444]'}`}
                                  >
                                    {game.result}
                                  </span>
                                )}
                              ...
                              <span className="text-[11px] font-bold text-[#00D4FF] uppercase tracking-widest">
                    ...
                    <div className="p-6 text-center text-slate-500 text-[11px] font-bold uppercase tracking-widest">
                      No games found
                    </div>
                  )}
                </div>
            ...
            {activeTab === 'PROPS' && (
              <div className="metal-panel">
                <div className="panel-title flex items-center">
                  <Target size={14} className="mr-2 text-[#00D4FF]" /> Active Prop Edges
                </div>
                <div className="flex flex-col gap-2 mt-4">
                  {props.length > 0 ? (
                          ...
                          <div className="flex flex-col gap-1 min-w-0">
                            ...
                            {prop.ev_pct != null && (
                              <div className="text-[10px] font-bold text-slate-500 tracking-widest">
                                EV {prop.ev_pct > 0 ? '+' : ''}
                                {Number(prop.ev_pct).toFixed(1)}%
                                {prop.edge_pts != null
                                  ? ` · ${Number(prop.edge_pts).toFixed(1)} edge`
                                  : ''}
                              </div>
                            )}
                          </div>
