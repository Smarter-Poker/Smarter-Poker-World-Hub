...
const MLB_TEAMS: Record<number, string> = {
    ...
    119: 'Los Angeles Dodgers', 120: 'Washington Nationals', 121: 'New York Mets', 133: 'Athletics',
...
const MetricTile = ({ label, value, tip, accent }: { label: string; value: string; tip?: string; accent?: boolean }) => (
    <div className="bg-[#1a2332] border border-[#3d4f5f] rounded-lg p-3 shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)]" title={tip}>
        <div className="flex items-center gap-1 mb-1">
            <span className="text-[10px] font-extrabold text-slate-400 tracking-widest uppercase">{label}</span>
...
const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <h2 className="text-base font-extrabold text-white mb-4 uppercase tracking-widest pl-2 border-l-[3px] border-[#00D4FF]" style={{ fontFamily: '"Rajdhani", sans-serif' }}>
        {children}
    </h2>
);
...
export default function PlayerProfilePage() {
        ...
        return (
            <div className="min-h-screen bg-[#0a0a15] pb-[70px] w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
                ...
                <div className="p-8 text-center mt-10">
                    <div className="text-slate-300 font-extrabold text-2xl uppercase tracking-widest" style={{ fontFamily: '"Rajdhani", sans-serif' }}>
    ...
    const sim = profile?.sim_rates && typeof profile.sim_rates === 'object' ? profile.sim_rates : null;
               ...
               {isLoading || !profile ? (
                   ...
                   <>
                       ...
                       <div className="bg-[#0d1117] border-[3px] border-[#3d4f5f] rounded-xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] mb-6 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
                           ...
                           <div className="flex-1 text-center md:text-left z-10">
                               ...
                               <div className="flex items-center justify-center md:justify-start gap-2 mt-3 flex-wrap">
                                   {teamName && <span className="text-slate-300 font-bold text-[12px] tracking-widest uppercase">{teamName}</span>}
                       ...
                       <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                           {type === 'hitter' ? (
                               <>
                                   <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] relative overflow-hidden" title="Weighted Runs Created Plus — total offense vs league average (100 = average, higher is better).">
                                       ...
                                       <div className="text-3xl font-extrabold text-[#00D4FF]" style={{ fontFamily: '"Rajdhani", sans-serif', textShadow: '0 0 10px rgba(0,212,255,0.3)' }}>{dec0(profile.wrc_plus)}</div>
                                   </div>
                                   <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] relative overflow-hidden" title="Weighted On-Base Average — overall offensive value per plate appearance.">
                                       ...
                                       <div className="text-3xl font-extrabold text-[#00D4FF]" style={{ fontFamily: '"Rajdhani", sans-serif', textShadow: '0 0 10px rgba(0,212,255,0.3)' }}>{dec3(profile.woba)}</div>
                                   </div>
                                   <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] relative overflow-hidden" title="Plate appearances — sample size for the season.">
                                       ...
                                       <div className="text-3xl font-extrabold text-white" style={{ fontFamily: '"Rajdhani", sans-serif' }}>{profile.pa != null ? dec0(profile.pa) : '—'}</div>
                                   </div>
                               </>
                           ) : (
                               <>
                                   <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] relative overflow-hidden" title="Fielding Independent Pitching — ERA estimate from K, BB, HBP and HR only (lower is better).">
                                       ...
                                       <div className="text-3xl font-extrabold text-[#00D4FF]" style={{ fontFamily: '"Rajdhani", sans-serif', textShadow: '0 0 10px rgba(0,212,255,0.3)' }}>{dec2(profile.fip)}</div>
                                   </div>
                                   <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] relative overflow-hidden" title="Skill-Interactive ERA — ERA estimate accounting for batted-ball type (lower is better).">
                                       <div className="absolute top-0 right-0 p-2 opacity-10 text-[#00D4FF]"><Activity size={40} /></div>
                                       <div className="text-[10px] font-extrabold text-slate-400 tracking-widest mb-1 uppercase">SIERA</div>
                                       <div className="text-3xl font-extrabold text-[#00D4FF]" style={{ fontFamily: '"Rajdhani", sans-serif', textShadow: '0 0 10px rgba(0,212,255,0.3)' }}>{dec2(profile.siera)}</div>
                                   </div>
                                   <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] relative overflow-hidden" title="Batters faced — sample size for the season.">
                                       <div className="absolute top-0 right-0 p-2 opacity-10 text-slate-400"><User size={40} /></div>
                                       <div className="text-[10px] font-extrabold text-slate-400 tracking-widest mb-1 uppercase">Batters Faced</div>
                                       <div className="text-3xl font-extrabold text-white" style={{ fontFamily: '"Rajdhani", sans-serif' }}>{profile.bf != null ? dec0(profile.bf) : '—'}</div>
                                   </div>
                               </>
                           )}
                       </div>
                       ...
                       <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] mb-8">
                           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                               {type === 'hitter' ? (
                                   <>
                                       <MetricTile label="xwOBA" value={dec3(profile.xwoba)} tip="Expected wOBA from quality of contact (exit velocity & launch angle)." accent />
                                       <MetricTile label="ISO" value={dec3(profile.iso)} tip="Isolated Power — extra bases per at-bat (SLG minus AVG)." />
                                       <MetricTile label="Barrel%" value={pct1(profile.barrel_pct)} tip="Share of batted balls hit at the optimal exit velocity and launch angle." />
                                       <MetricTile label="Exit Velo" value={mph1(profile.ev)} tip="Average exit velocity off the bat." />
                                   </>
                               ) : (
                                   <>
                                       <MetricTile label="xFIP" value={dec2(profile.xfip)} tip="Expected FIP, normalizing home-run rate to league average (lower is better)." accent />
                                       <MetricTile label="Stuff+" value={dec0(profile.stuff_plus)} tip="Pitch-quality model (100 = average, higher is better)." />
                                       <MetricTile label="SIERA" value={dec2(profile.siera)} tip="Skill-Interactive ERA (lower is better)." />
                                       <MetricTile label="FIP" value={dec2(profile.fip)} tip="Fielding Independent Pitching (lower is better)." />
                                   </>
                               )}
                           </div>
                       </div>
                       ...
                       {sim && (
                           <>
                               <SectionTitle>Model Projection {type === 'pitcher' ? '(per batter faced)' : '(per plate appearance)'}</SectionTitle>
                               <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] mb-8">
                                   <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                       <MetricTile label="K Rate" value={pct1(sim.k)} tip="Projected strikeout rate from the simulation model." accent />
                                       <MetricTile label="BB Rate" value={pct1(sim.bb)} tip="Projected walk rate from the simulation model." />
                                       <MetricTile label="HR Rate" value={pct1(sim.hr)} tip="Projected home-run rate from the simulation model." />
                                       <MetricTile label="HBP Rate" value={pct1(sim.hbp)} tip="Projected hit-by-pitch rate." />
                                       <MetricTile label="BABIP" value={dec3(sim.babip)} tip="Projected batting average on balls in play." />
                                   </div>
                           ...
                           <>
                               ...
                               <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)] mb-8">
                                   ...
                                   <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                       {type === 'hitter' ? (
                                           <>
                                               <MetricTile label="Hit Streak" value={dec0(streaks.current_hitting_streak)} tip="Current consecutive games with a hit." />
                                               <MetricTile label="On-Base Streak" value={dec0(streaks.on_base_streak)} tip="Current consecutive games reaching base." />
                                               <MetricTile label="Multi-Hit Gms" value={dec0(streaks.multi_hit_games)} tip="Games this season with 2+ hits." />
                                               <MetricTile label="Games Since HR" value={dec0(streaks.games_since_hr)} tip="Games since the last home run." />
                                           </>
                                       ) : (
                                           <>
                                               <MetricTile label="QS Streak" value={dec0(streaks.quality_start_streak)} tip="Current consecutive quality starts." />
                                               ...
                                               <MetricTile label="High-K Gms" value={dec0(streaks.high_k_games)} tip="Games this season with a high strikeout total." />
                                               <MetricTile label="Gms Since HR" value={dec0(streaks.games_since_hr_allowed)} tip="Games since allowing a home run." />
                                           </>
                                       )}
                                   </div>
                                   ...
                                   {type === 'hitter' && last5.length > 0 && (
                                       <div className="mt-5">
                                           <div className="text-[10px] font-extrabold text-slate-500 tracking-widest uppercase mb-2">Last 5 Games</div>
                                               ...
                                               <table className="w-full text-left border-collapse">
                                                   ...
                                                   <tbody>
                                                       {last5.map((g: any, i: number) => (
                                                           <tr key={g.date || i} className="border-t border-[#1e2d3d] text-slate-300 text-[12px] font-bold">
                                                               ...
                                                               <td className="py-1.5 px-2 text-center text-[#00D4FF]">{g.HR ?? '—'}</td>
                                   ...
                                   {type === 'pitcher' && streaks.last_start_line && typeof streaks.last_start_line === 'object' && (
                                       <div className="mt-5">
                                           ...
                                           <div className="flex items-center gap-3 flex-wrap">
                                               <MetricTile label="IP" value={streaks.last_start_line.IP != null ? String(streaks.last_start_line.IP) : '—'} />
