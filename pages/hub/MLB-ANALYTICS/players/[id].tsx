...
import { Info, Swords, Activity, Target, Shield, Zap, TrendingUp, BarChart3, DollarSign, AlertTriangle } from 'lucide-react';
...
const PROP_LABELS: Record<string, string> = {
  total_bases: 'Total Bases', home_run: 'Home Run', hits: 'Hits', runs: 'Runs', rbi: 'RBI',
  walks: 'Walks', stolen_bases: 'Stolen Bases', hrr: 'Hits + Runs + RBI', earned_runs: 'Earned Runs',
  pitcher_strikeouts: 'Pitcher Strikeouts', pitcher_walks: 'Pitcher Walks',
};
const formatProp = (p?: string) => (p ? (PROP_LABELS[p] || p.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())) : '—');
...
const HITTER_GROUPS: { title: string; icon: any; stats: Stat[] }[] = [
  ...
  { title: 'Advanced', icon: Activity, stats: [
    ...
    ['BABIP', 'BABIP', 'rate'], ['BB%', 'BB%', 'pct', 'up'], ['K%', 'K%', 'pct', 'down'], ['WAR', 'WAR', 'n1', 'up'],
    ['Clutch', 'Clutch', 'n2'], ['BsR', 'wBsR', 'n1'], ['Spd', 'Spd', 'n1'] ] },
  { title: 'Statcast & Batted Ball', icon: Zap, stats: [
    ['EV', 'EV', 'n1', 'up'], ['maxEV', 'maxEV', 'n1'], ['Barrel%', 'Barrel%', 'pct', 'up'], ['HardHit%', 'HardHit%', 'pct', 'up'],
  ...
  { title: 'Plate Discipline', icon: Target, stats: [
    ['O-Swing%', 'O-Swing%', 'pct', 'down'], ['Z-Swing%', 'Z-Swing%', 'pct'], ['Swing%', 'Swing%', 'pct'],
    ['Contact%', 'Contact%', 'pct', 'up'], ['SwStr%', 'SwStr%', 'pct', 'down'], ['Zone%', 'Zone%', 'pct'], ['F-Strike%', 'F-Strike%', 'pct'] ] },
];
const PITCHER_GROUPS: { title: string; icon: any; stats: Stat[] }[] = [
  { title: 'Standard', icon: BarChart3, stats: [
    ...
    ['SO', 'SO', 'int'], ['ERA', 'ERA', 'n2', 'down'], ['WHIP', 'WHIP', 'n2', 'down'] ] },
  { title: 'Run Prevention', icon: Shield, stats: [
    ['FIP', 'FIP', 'n2', 'down'], ['xFIP', 'xFIP', 'n2', 'down'], ['SIERA', 'SIERA', 'n2', 'down'], ['xERA', 'xERA', 'n2', 'down'],
    ['K/9', 'K/9', 'n2', 'up'], ['BB/9', 'BB/9', 'n2', 'down'], ['HR/9', 'HR/9', 'n2', 'down'], ['K%', 'K%', 'pct', 'up'],
    ['BB%', 'BB%', 'pct', 'down'], ['K-BB%', 'K-BB%', 'pct', 'up'], ['LOB%', 'LOB%', 'pct', 'up'], ['BABIP', 'BABIP', 'rate'], ['WAR', 'WAR', 'n1', 'up'] ] },
  { title: 'Stuff & Batted Ball', icon: Zap, stats: [
    ['Stuff+', 'sp_stuff', 'int'], ['Location+', 'sp_location', 'int'], ['Pitching+', 'sp_pitching', 'int'],
    ['EV', 'EV', 'n1'], ['Barrel%', 'Barrel%', 'pct', 'down'], ['HardHit%', 'HardHit%', 'pct', 'down'],
    ['GB%', 'GB%', 'pct'], ['FB%', 'FB%', 'pct'], ['SwStr%', 'SwStr%', 'pct'], ['Contact%', 'Contact%', 'pct'] ] },
];

const MetricTile = ({ label, value, tip, accent, tone }: { label: string; value: string; tip?: string; accent?: boolean; tone?: 'good' | 'bad' }) => (
...
const StatGroup = ({ title, icon, stats, season, leagueAvg }: { title: string; icon: any; stats: Stat[]; season: any; leagueAvg?: Record<string, number> | null }) => (
  <div className="mb-8">
    ...
    <MetalFrame className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)]">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {stats.map((stat) => {
          ...
          return <MetricTile key={label} label={label} value={fmt(kind, sv)} tip={glossaryFor(label)} tone={tone} />;
        })}
      </div>
    </MetalFrame>
  </div>
...
export default function PlayerProfilePage() {
  ...
  const situational = data?.situational && typeof data.situational === 'object' ? data.situational : null;
  const tendencies = data?.tendencies && typeof data.tendencies === 'object' ? data.tendencies : null;
  const health = data?.health && typeof data.health === 'object' ? data.health : null;
  const props: any[] = Array.isArray(data?.props) ? data.props : [];
  const leagueAvg: Record<string, number> | null = leagueData ? (type === 'pitcher' ? leagueData.pitcher : leagueData.hitter) : null;
  ...
  const SIT_KEYS: [string, string][] = [
    ['High Leverage', 'high_leverage_woba'], ['Runners On', 'runners_on_woba'], ['Bases Empty', 'bases_empty_woba'],
    ['Low Leverage', 'low_leverage_woba'], ['Grass', 'grass_woba'], ['Turf', 'turf_woba'],
    ['This Month', 'month_woba_avg'], ['By Day Avg', 'dow_woba_avg'], ['Extra Innings', 'extra_innings_woba'],
  ];
  const TEND_KEYS: [string, string][] = [
    ['vs <90 mph', 'velo_under90_woba'], ['vs 90-95 mph', 'velo_90_95_woba'], ['vs 95+ mph', 'velo_95plus_woba'],
    ['vs Fastball-Heavy', 'fb_heavy_woba'], ['vs Offspeed-Heavy', 'offspeed_heavy_woba'],
    ['Best Park', 'best_park_woba'], ['Worst Park', 'worst_park_woba'],
  ];
        ...
        {isLoading || !player ? (
          <div className="animate-pulse">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (<div key={i} className="bg-[#1a2332] border border-[#3d4f5f] rounded-lg p-3 h-16" />))}
            </div>
          </div>
        ) : (
          <>
              ...
              <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-2 mt-3 flex-wrap">
                    ...
                    {player.position && (
                      <span className="bg-[#1a2332] border border-[#3d4f5f] px-2 py-0.5 rounded text-[13px] font-extrabold text-slate-300 tracking-widest" title="Field position">{player.position}</span>
                    )}
                    {roleLabel && (
                      <span className="bg-[#00D4FF]/10 border border-[#00D4FF]/40 px-2 py-0.5 rounded text-[13px] font-extrabold text-[#00D4FF] tracking-widest" title={type === 'pitcher' ? 'Pitching role' : 'Lineup role'}>{roleLabel}</span>
                    )}
                    ...
                    {age != null && (
                      <span className="bg-[#1a2332] border border-[#3d4f5f] px-2 py-0.5 rounded text-[13px] font-extrabold text-slate-400 tracking-widest">Age {age}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            {health && health.notes && (
              <MetalFrame className="p-4 mb-6">
                <div className="flex items-start gap-3">
                  <AlertTriangle size={20} className="text-[#FFB020] shrink-0 mt-0.5" aria-hidden="true" />
                  <div>
                    <div className="text-[#FFB020] font-extrabold text-[13px] tracking-widest" style={{ fontFamily: '"Rajdhani", sans-serif' }}>Injury Watch</div>
              ...
              <div className="mb-8">
                ...
                <MetalFrame className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)]">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="text-slate-500 text-[12px] font-extrabold tracking-widest">
                          ...
                          <th className="py-1 px-2 text-center" title="Sportsbook line">Line</th>
                          <th className="py-1 px-2 text-center" title="Model projected outcome">Proj</th>
                          <th className="py-1 px-2 text-center" title="Model edge vs the market price, in percentage points">Edge</th>
                      ...
                      <tbody>
                        {props.map((p: any, i: number) => {
                          ...
                          return (
                            <tr key={i} className="border-t border-[#1e2d3d] text-slate-300 text-[13px] font-bold">
                              <td className="py-1.5 pr-3 text-white">{formatProp(p.prop)}</td>
                              <td className="py-1.5 px-2 text-center">{p.line != null ? p.line : '—'}</td>
                              <td className="py-1.5 px-2 text-center">{p.proj != null ? Number(p.proj).toFixed(1) : '—'}</td>
                              <td className={`py-1.5 px-2 text-center font-extrabold ${edge != null && edge > 0 ? 'text-[#3ED598]' : 'text-slate-400'}`}>
                                {edge != null ? `${edge > 0 ? '+' : ''}${edge.toFixed(1)}` : '—'}
                              </td>
                              <td className="py-1.5 px-2 text-left text-slate-300">{simplifyRec(p.rec)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                  ...
                  <p className="text-slate-600 text-[12px] font-bold tracking-wide mt-3">
                    Edge is the model probability vs the market price (percentage points). Full analysis on the{' '}
                    <Link href="/hub/MLB-ANALYTICS/props" className="text-[#00D4FF] underline">Props page</Link>.
                  </p>
                </MetalFrame>
              </div>
            )}
            ...
            {groups.map((g) => (
              <StatGroup key={g.title} title={g.title} icon={g.icon} stats={g.stats} season={season} leagueAvg={leagueAvg} />
            ))}
            ...
            {sitTiles.length > 0 && (
              <div className="mb-8">
                ...
                <MetalFrame className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)]">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {sitTiles.map(([lbl, k]) => (
                      <MetricTile key={k} label={lbl} value={fmt('rate', situational[k])} tip={`${lbl}: weighted on-base average (wOBA) in this situation.`} />
                    ))}
                  </div>
                </MetalFrame>
              </div>
            ...
            {tendTiles.length > 0 && (
              <div className="mb-8">
                ...
                <MetalFrame className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 shadow-[0_4px_15px_rgba(0,0,0,0.3)]">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {tendTiles.map(([lbl, k]) => (
                      <MetricTile key={k} label={lbl} value={fmt('rate', tendencies[k])} tip={`${lbl}: weighted on-base average (wOBA) against this pitch profile.`} />
                    ))}
                  </div>
                </MetalFrame>
              </div>
