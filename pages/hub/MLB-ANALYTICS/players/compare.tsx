...
const PlayerPicker = ({
  ...
  return (
    <div className="flex-1 min-w-0">
      <div
        className="text-slate-400 text-[12px] font-extrabold tracking-widest mb-2"
      ...
      {selected ? (
        <div className="flex items-center gap-2 bg-[#1a2332] border border-[#3d4f5f] rounded-lg p-2">
          ...
          <button
            onClick={onClear}
            className="ml-auto text-slate-500 hover:text-[#00D4FF] min-h-[44px]"
...
export default function ComparePage() {
  ...
  const directory: DirPlayer[] = useMemo(() => {
    ...
    const pi = (pitData?.data || []).map((p: any) => ({
      player_id: p.player_id,
      full_name: p.full_name,
      team_id: p.team_id,
  ...
  return (
    <div className="min-h-screen bg-[#0a0a15] pb-[70px] font-sans w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
      <SEOHead
        title="Compare MLB Players — Side-by-Side Stats | Smarter.Poker"
        description="Compare any two MLB players side by side across advanced stats, with the better value highlighted."
        noindex={true}
      />
      <UniversalHeader
        pageDepth={2}
      ...
      <div className="p-4 w-full max-w-4xl mx-auto box-border">
        <Link
          ...
          className="inline-flex items-center gap-1 text-[#00D4FF] text-[13px] font-extrabold tracking-widest hover:text-white transition-colors mb-4"
        >
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1
            className="m-0 text-[31px] md:text-[39px] font-extrabold text-white tracking-wide"
            style={{ fontFamily: '"Rajdhani", sans-serif' }}
          >
            Compare Players
          </h1>
        ...
        <MetalFrame className="p-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            <PlayerPicker
              ...
              dir={directory}
              selectedId={a}
              onPick={(id) => setPick('a', id)}
              onClear={() => setPick('a', null)}
            />
            <PlayerPicker
              label="Player B"
              dir={directory}
              selectedId={b}
              onPick={(id) => setPick('b', id)}
              onClear={() => setPick('b', null)}
            />
          </div>
        </MetalFrame>
        ...
        {!bothLoaded ? (
          <div className="text-center text-slate-500 font-bold text-sm tracking-wide py-10">
            Pick two players above to compare their {type === 'pitcher' ? 'pitching' : ''} stat
            lines side by side.
          </div>
        ) : !sameType ? (
          ...
          <MetalFrame className="p-2 sm:p-4">
            <div className="grid grid-cols-3 items-center gap-2 pb-3 mb-2 border-b border-[#2a3a4a]">
              ...
              <div className="text-center text-slate-500 text-[11px] font-extrabold tracking-widest">
                STAT
              </div>
              <div
                className="text-center text-white font-extrabold text-[15px] "
            ...
            {rows.map(([label, key, kind, dir]) => {
              ...
              return (
                <div
                  key={label}
                  className="grid grid-cols-3 items-center gap-2 py-1.5 border-b border-[#1e2d3d] last:border-0"
                >
                  <div
                    className={`text-center text-[17px] font-extrabold ${aWin ? 'text-[#3ED598]' : 'text-slate-300'}`}
                    style={{ fontFamily: '"Rajdhani", sans-serif' }}
                  >
                    {fmt(kind, av)}
                  </div>
                  <div
                    className="text-center text-slate-500 text-[12px] font-extrabold tracking-widest"
                    title={glossaryFor(label)}
                  >
                    {label}
                  </div>
                  <div
                    className={`text-center text-[17px] font-extrabold ${bWin ? 'text-[#3ED598]' : 'text-slate-300'}`}
                    style={{ fontFamily: '"Rajdhani", sans-serif' }}
                  >
                    {fmt(kind, bv)}
                  </div>
