...
export default function GameMatchupDashboard() {
    ...
    const gameBets = useMemo(() => {
        ...
        const _matched = betsData.bets.filter((b: any) => {
            ...
            return (
                bMatchup.includes(h) || bMatchup.includes(a) ||
                bTeam === h || bTeam === a ||
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-20">
                        <p className="text-[#FF4444] font-['Rajdhani'] text-xl">Game not found on today's slate.</p>
                    </div>
                ) : (
                    <div className="space-y-6">
                            ...
                            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                                ...
                                <div className="flex flex-col items-center flex-1">
                                    <div className="w-16 h-16 mb-4 relative drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                                        ...
                                        <img src={teamLogo(game.awayId) || ''} alt={game.away} className="w-full h-full object-contain" />
                                    </div>
                                    ...
                                    {game.awayStarter && (
                                        <p className="text-sm font-['Rajdhani'] mt-2 text-[#00D4FF]">
                                            P: {game.awayStarter.name} {game.awayStarter.wins != null ? `(${game.awayStarter.wins}-${game.awayStarter.losses}, ${game.awayStarter.era})` : ''}
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    <div className="mt-4 px-3 py-1 bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/30 rounded text-xs font-bold tracking-widest font-['Rajdhani'] whitespace-nowrap">
                                        {game.firstPitch ? new Date(game.firstPitch).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZoneName: 'short' }) : 'TBD'}
                                    </div>
                                ...
                                <div className="flex flex-col items-center flex-1">
                                    <div className="w-16 h-16 mb-4 relative drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={teamLogo(game.homeId) || ''} alt={game.home} className="w-full h-full object-contain" />
                                    </div>
                                    <h2 className="text-2xl font-bold tracking-widest uppercase font-['Rajdhani'] text-white">
                                        {game.home}
                                    </h2>
                                    ...
                                    {game.homeStarter && (
                                        <p className="text-sm font-['Rajdhani'] mt-2 text-[#00D4FF]">
                                            P: {game.homeStarter.name} {game.homeStarter.wins != null ? `(${game.homeStarter.wins}-${game.homeStarter.losses}, ${game.homeStarter.era})` : ''}
                                        </p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-[#1A2436] grid grid-cols-3 gap-4">
                                <div className="bg-[#060B14] p-3 rounded text-center border border-[#1A2436]">
                                    ...
                                    <div className="flex justify-around">
                                        <span className="font-['Rajdhani'] text-lg font-bold text-white">{game.avgAwayLine ? (game.avgAwayLine > 0 ? `+${game.avgAwayLine}` : game.avgAwayLine) : '-'}</span>
                                        <span className="text-[#1A2436] mx-2">|</span>
                                        <span className="font-['Rajdhani'] text-lg font-bold text-white">{game.avgHomeLine ? (game.avgHomeLine > 0 ? `+${game.avgHomeLine}` : game.avgHomeLine) : '-'}</span>
                                    </div>
                                </div>
                                <div className="bg-[#060B14] p-3 rounded text-center border border-[#1A2436]">
                                    <p className="text-xs text-[#8BA4D5] uppercase tracking-widest mb-1">Spread</p>
                                    <div className="flex justify-around">
                                        <span className="font-['Rajdhani'] text-lg font-bold text-white">{game.avgAwaySpreadLine ? (game.avgAwaySpreadLine > 0 ? `+${game.avgAwaySpreadLine}` : game.avgAwaySpreadLine) : '-'}</span>
                                        <span className="text-[#1A2436] mx-2">|</span>
                                        <span className="font-['Rajdhani'] text-lg font-bold text-white">{game.avgHomeSpreadLine ? (game.avgHomeSpreadLine > 0 ? `+${game.avgHomeSpreadLine}` : game.avgHomeSpreadLine) : '-'}</span>
                                    </div>
                                </div>
                                <div className="bg-[#060B14] p-3 rounded text-center border border-[#1A2436]">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {gameBets.length === 0 ? (
                                    <div className="bg-[#0A101C] p-6 text-center rounded-lg border border-[#1A2436]">
                                        <p className="text-[#8BA4D5] font-['Rajdhani']">No active edges found for this matchup.</p>
                                    </div>
                                ) : (
                                    gameBets.slice(0, 10).map((bet: any, idx: number) => (
                                        <div key={idx} className="bg-[#0A101C] p-4 rounded-lg border border-[#1A2436] flex items-center justify-between hover:border-[#00D4FF]/30 transition-colors">
                                            <div className="flex flex-col">
                                                <span className="font-['Rajdhani'] text-lg font-bold text-white">
                                                    {bet.team_name || bet.selection} {bet.market && <span className="text-[#8BA4D5] text-sm ml-1">{bet.market.replace(/_/g, ' ').toUpperCase()}</span>}
                                                </span>
                                                <span className="text-xs text-[#8BA4D5] uppercase tracking-wider">{bet.bet_type}</span>
                                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {gameProps.length === 0 ? (
                                    <div className="bg-[#0A101C] p-6 text-center rounded-lg border border-[#1A2436]">
                                        <p className="text-[#8BA4D5] font-['Rajdhani']">No active prop edges found for this matchup.</p>
                                    </div>
                                ) : (
