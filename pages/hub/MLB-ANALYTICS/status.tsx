...
export default function StatusPage() {
  ...
  const tableCounts =
    typeof data?.tableCounts === 'object' && data.tableCounts !== null ? data.tableCounts : {};
  const stages = useMemo(() => (Array.isArray(data?.stages) ? data.stages : []), [data?.stages]);
  const latestRuns =
    typeof data?.latestRuns === 'object' && data.latestRuns !== null ? data.latestRuns : {};
    ...
    <div className="min-h-screen bg-[#0a0a15] text-slate-200 pb-[70px] font-sans w-full max-w-[100vw] overflow-x-hidden box-border">
      ...
      <div className="bg-gradient-to-b from-[#1a2332] to-[#0d1117] border-b-[2px] border-[#3d4f5f] p-4 flex justify-between items-center gap-3">
        ...
        <div className="text-right flex flex-col items-end shrink-0">
          <div className="text-[#00D4FF] text-xs font-bold tracking-widest uppercase">SYSTEM</div>
          <div
                    ...
                    {stages.length > 0 ? (
                      <ul className="flex flex-col m-0 p-0 list-none">
                        {stages.map((stage, idx) => {
                          ...
                          const statusColor = ['ERROR', 'FAILED', 'TIMEOUT', 'CRITICAL'].includes(
                            status
                          )
