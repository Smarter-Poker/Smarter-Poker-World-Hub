...
const marketLabel = (m: string) =>
  m === 'h2h' ? 'Moneyline' : m === 'total' ? 'Totals' : m || '—';
...
const tierColor = (tier: string) => {
  switch ((tier || '').toUpperCase()) {
    case 'ELITE':
      return { color: '#00D4FF', bg: 'rgba(0,212,255,0.12)', border: 'rgba(0,212,255,0.4)' };
    case 'STRONG':
      return { color: '#34D399', bg: 'rgba(52,211,153,0.12)', border: 'rgba(52,211,153,0.4)' };
    case 'LEAN':
      return { color: '#38BDF8', bg: 'rgba(56,189,248,0.10)', border: 'rgba(56,189,248,0.4)' };
    case 'THIN':
      return { color: '#F59E0B', bg: 'rgba(245,158,11,0.10)', border: 'rgba(245,158,11,0.4)' };
    default:
      return { color: '#64748B', bg: 'rgba(100,116,139,0.08)', border: 'rgba(100,116,139,0.3)' };
  }
};
    ...
    return (
      <div className="min-h-screen bg-[#0a0a15] pb-[70px] font-sans w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
        <SEOHead
          ...
          description="MLB betting portfolio tracker by Smarter.Poker."
          noindex={true}
        />
    ...
    <div className="bg-[#0a0a15] min-h-screen font-inter pb-[70px] w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
      ...
      <div className="edge-to-edge-container w-full max-w-[1000px] mx-auto px-0 md:px-4 py-6 relative">
        ...
        <div className="flex flex-wrap gap-4 justify-between items-start mb-6 relative z-10">
          <div>
            <h1
              className="m-0 text-[51px] font-extrabold text-white tracking-widest capitalize"
              style={{
                fontFamily: '"Rajdhani", sans-serif',
          ...
          <div className="flex gap-2 flex-wrap">
            <a
              href={csvHref}
              className="flex items-center gap-1 py-2 px-4 bg-[#00D4FF]/10 border border-[#00D4FF]/30 rounded-md text-[#00D4FF] text-[22px] font-bold cursor-pointer transition-all duration-200 hover:bg-[#00D4FF]/20"
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 min-h-[400px]">
            ...
            <div className="text-[#00D4FF] font-bold tracking-widest text-[23px] animate-pulse">
          ...
          <>
            ...
            <div className="flex flex-col lg:flex-row gap-4 mb-8 relative z-10">
              ...
              <div className="relative w-full lg:w-1/3 bg-[#0d1117] border-[3px] border-[#3d4f5f] rounded-xl p-6 flex flex-col justify-center items-center shadow-[0_4px_20px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] overflow-hidden">
                ...
                <div
                  className={`relative z-10 text-[22px] font-extrabold tracking-widest mt-2 capitalize ${totalPnl < 0 ? 'text-[#FF4444]' : 'text-[#00D4FF]'}`}
                  style={{
                    textShadow: `0 0 10px ${totalPnl < 0 ? 'rgba(255,68,68,0.5)' : 'rgba(0,212,255,0.5)'}`,
                  }}
                >
              ...
              <div className="mb-8 relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-4 md:px-0">
                  <div className="bg-[#0d1117] border-[2px] border-[#00D4FF]/40 rounded-xl p-5 shadow-[0_0_15px_rgba(0,212,255,0.1)]">
                    <div className="text-[17px] font-bold text-[#00D4FF] tracking-widest mb-2 capitalize">
                      Model (Kelly-Sized)
                    </div>
                    <div
                      ...
                      style={{ fontFamily: '"Rajdhani", sans-serif' }}
                    >
                      {formatCurrency(kellyFinal)}
                    </div>
                    <div className="text-[21px] font-bold text-[#00D4FF] mt-1 tracking-wide">
                      {formatPct(kellyGrowth || 0, true)} bankroll growth
                    </div>
                  </div>
                  <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-5">
                    <div className="text-[17px] font-bold text-slate-400 tracking-widest mb-2 capitalize">
                      Flat {formatCurrency(Number(baseline.unit_size))}/Unit
                    </div>
                    <div
                      className="text-[51px] font-extrabold text-slate-200"
                      style={{ fontFamily: '"Rajdhani", sans-serif' }}
                    >
                      {formatCurrency(flatFinal)}
                    </div>
                    <div className="text-[21px] font-bold text-slate-400 mt-1 tracking-wide">
                      {formatPct(flatGrowth || 0, true)} bankroll growth
                    </div>
                  </div>
                </div>
                {kellyMultiple != null && kellyMultiple > 0 && (
                  <p className="text-[18px] text-slate-500 mt-3 font-semibold tracking-wide px-4 md:px-0">
                    Same picks, same {formatCurrency(startBankroll)} start — Kelly-fractional sizing
                    produced {kellyMultiple.toFixed(1)}x the profit of flat{' '}
                    {formatCurrency(Number(baseline.unit_size))} units.
                  </p>
                )}
              </div>
            )}
            ...
            {risk && (
              <div className="mb-8 relative z-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 px-4 md:px-0">
                  <MetricBox
                    title="PROFIT FACTOR"
                    value={`${Number(risk.profit_factor || 0).toFixed(2)}x`}
                    sub="Gross won / lost"
                    valueColor={Number(risk.profit_factor || 0) >= 1 ? '#00D4FF' : '#FF0055'}
                  />
                  <MetricBox
                    title="EXPECTANCY / BET"
                    value={formatCurrency(Number(risk.expectancy || 0), true)}
                    sub="Avg profit per bet"
                    valueColor={Number(risk.expectancy || 0) >= 0 ? '#00D4FF' : '#FF0055'}
                  />
                  <MetricBox
                    title="AVG STAKE"
                    value={formatCurrency(Number(risk.avg_stake || 0))}
                    sub="Per bet"
                  />
                  <MetricBox
                    title="LONGEST WIN RUN"
                    value={`${risk.longest_win_streak || 0}`}
                    sub={`${risk.longest_loss_streak || 0} loss run`}
                    valueColor="#00D4FF"
                  />
                  <MetricBox
                    title="BEST DAY"
                    value={formatCurrency(Number(risk.best_day || 0), true)}
                    valueColor="#00D4FF"
                  />
                  <MetricBox
                    title="WORST DAY"
                    value={formatCurrency(Number(risk.worst_day || 0), true)}
                    valueColor="#FF0055"
                  />
                </div>
              </div>
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center text-[18px]">
                        ...
                        <span className="text-slate-200 font-bold">
                          {winPct(m.wins, m.losses).toFixed(1)}%
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-[18px]">
                        <span className="text-slate-400 font-semibold">P&amp;L</span>
                        <span
                          className={`font-bold ${Number(m.pnl) >= 0 ? 'text-[#00D4FF]' : 'text-[#FF0055]'}`}
                        >
                          {formatCurrency(Number(m.pnl), true)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-[18px]">
                    ...
                    <tbody>
                      {markets.map((m: MarketSummaryRow) => (
                        ...
                        >
                          ...
                          <td
                            className={`py-3 px-4 text-right font-semibold ${Number(m.pnl) >= 0 ? 'text-[#00D4FF]' : 'text-[#FF0055]'}`}
                          >
                            {formatCurrency(Number(m.pnl), true)}
                          </td>
                          <td
                            className={`py-3 px-4 text-right font-bold ${Number(m.roi) >= 0 ? 'text-[#00D4FF]' : 'text-[#FF0055]'}`}
                          >
                            {formatPct(Number(m.roi), true)}
                          </td>
                        </tr>
                  ...
                  {grades.map((g: GradeSummaryRow) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center border-b border-[#2a3a4a] pb-2">
                          <span
                            className="py-1 px-2 rounded text-[16px] font-extrabold tracking-wide"
                            style={{
                              color: tc.color,
                              background: tc.bg,
                        ...
                        <div className="flex justify-between items-center text-[18px]">
                          <span className="text-slate-400 font-semibold">Win Rate</span>
                          <span className="text-slate-200 font-bold">
                            {winPct(g.wins, g.losses).toFixed(1)}%
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-[18px]">
                          <span className="text-slate-400 font-semibold">P&amp;L</span>
                          <span
                            className={`font-bold ${Number(g.pnl) >= 0 ? 'text-[#00D4FF]' : 'text-[#FF0055]'}`}
                          >
                            {formatCurrency(Number(g.pnl), true)}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-[18px]">
                          <span className="text-slate-400 font-semibold">roi</span>
                        ...
                        return (
                          ...
                          >
                            <td className="py-3 px-4">
                              <span
                                className="py-1 px-2 rounded text-[18px] font-extrabold tracking-wide"
                                style={{
                                  color: tc.color,
                                  background: tc.bg,
                                  border: `1px solid ${tc.border}`,
                            ...
                            <td
                              className={`py-3 px-4 text-right font-semibold ${Number(g.pnl) >= 0 ? 'text-[#00D4FF]' : 'text-[#FF0055]'}`}
                            >
                              {formatCurrency(Number(g.pnl), true)}
                            </td>
                            <td
                              className={`py-3 px-4 text-right font-bold ${Number(g.roi) >= 0 ? 'text-[#00D4FF]' : 'text-[#FF0055]'}`}
                            >
                              {formatPct(Number(g.roi), true)}
                            </td>
                          </tr>
            ...
            {equityDaily.length > 0 && (
              <div className="mb-8 relative z-10">
                ...
                <div
                  className="bg-[#0d1117] border-[3px] border-[#3d4f5f] rounded-xl p-6 h-[260px] shadow-[0_4px_20px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] relative overflow-hidden"
                  role="img"
            ...
            <div className="mt-8 bg-[#0d1117] border border-[#2a3a4a] rounded-xl p-5 text-[21px] leading-relaxed text-slate-400 relative z-10 mx-4 md:mx-0">
              <div className="text-[17px] font-extrabold tracking-widest text-slate-500 capitalize mb-2">
                Methodology
              </div>
              <p className="m-0">
                A system {formatCurrency(startBankroll)} bankroll is staked on every model-graded
                MLB pick using Kelly-fractional sizing. Results are tracked at the model&apos;s
                graded price across moneyline and totals markets
                {dataWindow?.firstDay && dataWindow?.lastDay
                  ? ` from ${fmtDay(dataWindow.firstDay)} to ${fmtDay(dataWindow.lastDay)}`
                  : ''}
                . Past performance does not guarantee future results. Analysis only — not betting
                advice.
              </p>
            </div>
          </>
