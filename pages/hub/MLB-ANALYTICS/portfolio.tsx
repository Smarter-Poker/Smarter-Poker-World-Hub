...
const MetricBox = ({ title, value, sub, valueColor = '#FFFFFF', isLoading }: MetricBoxProps) => (
    <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-4 flex flex-col shadow-[inset_0_2px_4px_rgba(0,0,0,0.5),0_0_10px_rgba(0,0,0,0.5)] transition-all hover:border-[#00D4FF] hover:shadow-[inset_0_2px_4px_rgba(0,0,0,0.5),0_0_15px_rgba(0,212,255,0.2)]">
        <div className="text-[13px] font-bold text-slate-400 tracking-widest mb-2 capitalize">{title}</div>
        <div className="text-[31px] font-extrabold" style={{ color: isLoading ? '#00D4FF' : valueColor, textShadow: isLoading || valueColor !== '#FFFFFF' ? `0 0 10px ${isLoading ? '#00D4FF' : valueColor}80` : 'none', fontFamily: '"Rajdhani", sans-serif' }}>
            {isLoading ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-[#00D4FF]" /> : value}
        </div>
        {sub && <div className="text-[14px] text-slate-500 mt-1 font-bold tracking-widest capitalize">{isLoading ? '--' : sub}</div>}
    </div>
);

const SectionTitle = ({ children, tag }: { children: React.ReactNode; tag?: string }) => (
    <h2 className="text-[23px] font-extrabold text-white mb-4 flex items-center gap-2 capitalize tracking-widest relative z-10" style={{ fontFamily: '"Rajdhani", sans-serif' }}>
        <div className="w-1 h-[18px] bg-[#00D4FF] rounded-sm shadow-[0_0_8px_rgba(0,212,255,0.6)]" />
        {children}
        {tag && <span className="text-slate-400 font-bold tracking-widest text-[13px] border border-[#3d4f5f] rounded px-1.5 py-0.5">{tag}</span>}
    </h2>
);
        ...
        <div className="bg-[#0a0a15] min-h-screen font-inter pb-[70px] w-full max-w-[100vw] overflow-x-hidden box-border text-slate-200">
            ...
            <div className="edge-to-edge-container max-w-[1000px] mx-auto px-4 py-6 relative">
                ...
                <div className="flex flex-wrap gap-4 justify-between items-start mb-6 relative z-10">
                    <div>
                        <h1 className="m-0 text-[39px] font-extrabold text-white tracking-widest capitalize" style={{ fontFamily: '"Rajdhani", sans-serif', textShadow: '0 0 15px rgba(255,255,255,0.2)' }}>
                        ...
                        {dataWindow?.firstDay && dataWindow?.lastDay && (
                            <p className="m-0 mt-1 text-slate-500 font-semibold text-[14px] tracking-wide">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        <a
                            href={csvHref}
                            className="flex items-center gap-1 py-2 px-4 bg-[#00D4FF]/10 border border-[#00D4FF]/30 rounded-md text-[#00D4FF] text-[17px] font-bold cursor-pointer transition-all duration-200 hover:bg-[#00D4FF]/20"
                            aria-label="Download bet log as CSV"
                        >
                            <Download size={16} /> Csv
                        </a>
                        <Link href="/hub/MLB-ANALYTICS/backtest" passHref>
                            <button className="flex items-center gap-1 py-2 px-4 bg-[#1a2332] border border-[#3d4f5f] rounded-md text-slate-200 text-[17px] font-bold cursor-pointer transition-all duration-200 hover:bg-[#3d4f5f]">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 min-h-[400px]">
                        ...
                        <div className="text-[#00D4FF] font-bold tracking-widest text-[18px] animate-pulse">Calculating Matrices...</div>
                    </div>
                ) : (
                    <>
                        {/* Filter Bar */}
                        <div className="flex flex-wrap gap-4 mb-6 items-center justify-between bg-[#0d1117] p-4 rounded-xl border-[2px] border-[#3d4f5f] shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)] relative z-10">
                            <div className="flex gap-2 items-center" role="group" aria-label="Timeframe filter">
                                <span className="text-slate-400 text-[16px] font-semibold mr-2 capitalize tracking-wide">Timeframe:</span>
                                {[7, 14, 30].map(d => (
                                    <button
                                        key={d}
                                        onClick={() => setDaysFilter(d)}
                                        aria-pressed={daysFilter === d}
                                        className={`min-h-[36px] py-1.5 px-3 rounded-md text-[16px] font-bold cursor-pointer transition-all duration-200 ${daysFilter === d ? 'bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/30' : 'bg-transparent text-slate-500 border border-transparent hover:text-slate-300'}`}
                                    >
                                        {d} DAYS
                                    </button>
                                ))}
                                <button
                                    ...
                                    className={`min-h-[36px] py-1.5 px-3 rounded-md text-[16px] font-bold cursor-pointer transition-all duration-200 ${daysFilter === null ? 'bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/30' : 'bg-transparent text-slate-500 border border-transparent hover:text-slate-300'}`}
                                >
                                    Ytd
                                </button>
                            </div>
                            ...
                            <div className="flex gap-2 items-center flex-wrap" role="group" aria-label="Market filter">
                                <span className="text-slate-400 text-[16px] font-semibold mr-2 capitalize tracking-wide">Market:</span>
                                {MARKETS.map(m => (
                                    <button
                                        key={m.val}
                                        onClick={() => setMarketFilter(m.val)}
                                        aria-pressed={marketFilter === m.val}
                                        className={`min-h-[36px] py-1.5 px-3 rounded-md text-[16px] font-bold cursor-pointer transition-all duration-200 ${marketFilter === m.val ? 'bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/30' : 'bg-transparent text-slate-500 border border-transparent hover:text-slate-300'}`}
                                    >
                                        {m.short}
                                    </button>
                                ))}
                        ...
                        <div className="flex flex-col lg:flex-row gap-4 mb-8 relative z-10">
                            <div className="relative w-full lg:w-1/3 bg-[#0d1117] border-[3px] border-[#3d4f5f] rounded-xl p-6 flex flex-col justify-center items-center shadow-[0_4px_20px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] overflow-hidden">
                                ...
                                <div className="relative z-10 text-[14px] font-extrabold text-[#00D4FF] tracking-widest mb-2 capitalize">Current Bankroll</div>
                                ...
                                <div className={`relative z-10 text-[17px] font-extrabold tracking-widest mt-2 capitalize ${totalPnl < 0 ? 'text-[#FF4444]' : 'text-[#00D4FF]'}`} style={{ textShadow: `0 0 10px ${totalPnl < 0 ? 'rgba(255,68,68,0.5)' : 'rgba(0,212,255,0.5)'}` }}>
                                    {formatCurrency(totalPnl, true)} from start
                                </div>
                                <div className="relative z-10 text-[13px] text-slate-500 mt-1 font-bold tracking-widest capitalize">
                        ...
                        {(risk || baseline || markets.length > 0 || grades.length > 0) && (
                            <div className="mb-2 mt-2 flex items-center gap-3 relative z-10">
                                ...
                                <span className="text-[13px] font-extrabold tracking-[0.2em] text-slate-500 capitalize">Full Backtest Analytics</span>
                            ...
                            <div className="mb-8 relative z-10">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-4 md:px-0">
                                    <div className="bg-[#0d1117] border-[2px] border-[#00D4FF]/40 rounded-xl p-5 shadow-[0_0_15px_rgba(0,212,255,0.1)]">
                                        <div className="text-[13px] font-bold text-[#00D4FF] tracking-widest mb-2 capitalize">Model (Kelly-Sized)</div>
                                        <div className="text-[39px] font-extrabold text-white" style={{ fontFamily: '"Rajdhani", sans-serif' }}>{formatCurrency(kellyFinal)}</div>
                                        <div className="text-[16px] font-bold text-[#00D4FF] mt-1 tracking-wide">{formatPct(kellyGrowth || 0, true)} bankroll growth</div>
                                    </div>
                                    <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl p-5">
                                        <div className="text-[13px] font-bold text-slate-400 tracking-widest mb-2 capitalize">Flat {formatCurrency(Number(baseline.unit_size))}/Unit</div>
                                        <div className="text-[39px] font-extrabold text-slate-200" style={{ fontFamily: '"Rajdhani", sans-serif' }}>{formatCurrency(flatFinal)}</div>
                                        <div className="text-[16px] font-bold text-slate-400 mt-1 tracking-wide">{formatPct(flatGrowth || 0, true)} bankroll growth</div>
                                    </div>
                                </div>
                                {kellyMultiple != null && kellyMultiple > 0 && (
                                    <p className="text-[14px] text-slate-500 mt-3 font-semibold tracking-wide">
                        ...
                        {markets.length > 0 && (
                            <div className="mb-8 relative z-10">
                                ...
                                <div className="bg-[#0d1117] border-[2px] border-[#3d4f5f] rounded-xl overflow-x-auto shadow-[0_4px_20px_rgba(0,0,0,0.5)]">
                                    <table className="w-full min-w-[460px] border-collapse text-left text-[17px]">
                                        <thead>
                                            <tr className="border-b-[2px] border-[#3d4f5f] text-[#8b9bb4] bg-[#1a2332] capitalize tracking-widest text-[14px]" style={{ fontFamily: '"Rajdhani", sans-serif' }}>
                                                ...
                                                return (
                                                    <tr key={g.bet_tier} className="border-b border-[#2a3a4a] hover:bg-[#1a2332]/50 transition-colors">
                                                        <td className="py-3 px-4">
                                                            <span className="py-1 px-2 rounded text-[14px] font-extrabold tracking-wide" style={{ color: tc.color, background: tc.bg, border: `1px solid ${tc.border}` }}>
                                                                {g.bet_tier}
                                                            </span>
                                                        </td>
                        ...
                        <SectionTitle>
                            Recent Simulated Bets <span className="text-slate-400 font-bold tracking-widest text-[14px]">(LAST 20)</span>
                        </SectionTitle>
                        ...
                        <div className="mt-8 bg-[#0d1117] border border-[#2a3a4a] rounded-xl p-5 text-[16px] leading-relaxed text-slate-400 relative z-10">
                            <div className="text-[13px] font-extrabold tracking-widest text-slate-500 capitalize mb-2">Methodology</div>
                            <p className="m-0">
