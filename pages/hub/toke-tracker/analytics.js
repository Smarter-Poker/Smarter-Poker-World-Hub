...
export default function TokeAnalyticsPage() {
    ...
    const updatePref = useCallback(async (key, value) => {
        ...
        try {
            ...
            const { error: err_profiles_ee83d } = await supabase.from('profiles').update({ settings }).eq('id', userId);
            if (err_profiles_ee83d) console.warn('[Supabase] Silent mutation failed in profiles:', err_profiles_ee83d.message);
