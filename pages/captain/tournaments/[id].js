...
import EliminatePlayerModal from '../../../src/components/captain/modals/EliminatePlayerModal';
...
export default function TournamentDetailPage() {
  ...
  const fetchTournament = useCallback(async () => {
    ...
    try {
      const [tournamentRes, entriesRes] = await Promise.all([
        ...
        fetch(`/api/captain/tournaments/${id}/entries`)
      ]);
  ...
  async function handleClockAction(action) {
    try {
      const res = await fetch(`/api/captain/tournaments/${id}/clock`, {
        method: 'POST',
  ...
  async function handleStatusChange(newStatus) {
    try {
      const res = await fetch(`/api/captain/tournaments/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  function openPublicClock() {
    window.open(`/hub/captain/tournament/${id}/clock`, '_blank');
  }
  ...
  return (
    <>
      <Head>
        <title>{tournament.name} | Captain</title>
