...
export default async function handler(req, res) {
  ...
  try {
    ...
    const postsParams = new URLSearchParams({
      select:
        'id,content,content_type,media_urls,thumbnail_url,like_count,comment_count,share_count,view_count,visibility,created_at,author_id,link_url,link_title,link_description,link_image,link_site_name,metadata',
    ...
    postsParams.set(
      'select',
      'id,content,content_type,media_urls,thumbnail_url,like_count,comment_count,share_count,view_count,visibility,created_at,author_id,link_url,link_title,link_description,link_image,link_site_name,metadata,is_deleted'
    );
    ...
    const [profilesData, likesData, ownLikesData, bookmarksData] = await Promise.all([
      ...
      postIds.length > 0
        ? supaFetch(
            `/social_likes?post_id=in.(${postIds.join(',')})&select=post_id,user_id,reaction_type&limit=500`
          )
        : Promise.resolve([]),
      ...
      userId && postIds.length > 0
        ? supaFetch(
            `/social_likes?user_id=eq.${userId}&post_id=in.(${postIds.join(',')})&select=post_id`
          ).catch(() => [])
        : Promise.resolve([]),
