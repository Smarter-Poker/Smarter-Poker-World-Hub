/**
 * Club Commander Services API - GET/POST /api/club-commander/services
...
async function handleGet(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander services query error:', error);
    ...
    console.error('Club Commander services GET error:', error);
    return res.status(500).json({
      success: false,
...
async function handlePost(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander service create error:', error);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander services POST error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
