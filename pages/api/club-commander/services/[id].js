/**
 * Club Commander Service Request API - GET/PATCH /api/club-commander/services/:id
...
async function handleGet(req, res, requestId) {
  try {
    ...
    console.error('Club Commander service GET error:', error);
...
async function handlePatch(req, res, requestId) {
  try {
    ...
    if (updateError) {
      console.error('Club Commander service PATCH error:', updateError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander service PATCH error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
