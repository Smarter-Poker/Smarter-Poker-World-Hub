/**
 * Club Commander Games at Venue API - GET /api/club-commander/games/venue/:venueId [Public]
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (gamesError) {
      console.error('Club Commander venue games query error:', gamesError);
    ...
    if (tablesError) {
      console.error('Club Commander venue tables query error:', tablesError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander venue games API error:', error);
    return res.status(500).json({
      success: false,
