/**
 * Club Commander Session API - GET/PATCH /api/club-commander/sessions/:id
...
async function handleGet(req, res, sessionId) {
  try {
    ...
    console.error('Club Commander session GET error:', error);
...
async function handlePatch(req, res, sessionId) {
  try {
    ...
    if (updateError) {
      console.error('Club Commander session PATCH error:', updateError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander session PATCH error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
