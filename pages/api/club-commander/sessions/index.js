/**
 * Club Commander Sessions API - GET/POST /api/club-commander/sessions
...
async function handleGet(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander sessions query error:', error);
    ...
    console.error('Club Commander sessions GET error:', error);
    return res.status(500).json({
      success: false,
...
async function handlePost(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander session create error:', error);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander sessions POST error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
