/**
 * Club Commander Table API - GET/PATCH/DELETE /api/club-commander/tables/:id
...
async function handleGet(req, res, tableId) {
  try {
    ...
    console.error('Club Commander table GET error:', error);
...
async function handlePatch(req, res, tableId) {
  try {
    ...
    if (updateError) {
      console.error('Club Commander table PATCH error:', updateError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander table PATCH error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
...
async function handleDelete(req, res, tableId) {
  try {
    ...
    if (deleteError) {
      console.error('Club Commander table DELETE error:', deleteError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander table DELETE error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
