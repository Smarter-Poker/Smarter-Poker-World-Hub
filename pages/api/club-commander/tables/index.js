/**
 * Club Commander Tables API - GET/POST /api/club-commander/tables
...
async function handleGet(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander tables query error:', error);
    ...
    console.error('Club Commander tables GET error:', error);
    return res.status(500).json({
      success: false,
...
async function handlePost(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander table create error:', error);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander tables POST error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
