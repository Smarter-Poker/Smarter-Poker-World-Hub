/**
 * Club Commander Staff API - GET/POST /api/club-commander/staff
...
async function handleGet(req, res) {
  try {
    ...
    if (error) {
      console.error('Club Commander staff list error:', error);
    ...
    console.error('Club Commander staff GET error:', error);
    return res.status(500).json({
      success: false,
...
async function handlePost(req, res) {
  try {
    ...
    if (insertError) {
      console.error('Club Commander staff insert error:', insertError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Club Commander staff POST error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
