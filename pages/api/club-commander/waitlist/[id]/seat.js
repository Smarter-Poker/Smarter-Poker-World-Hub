/**
 * Club Commander Seat Player API - POST /api/club-commander/waitlist/:id/seat [Staff]
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (seatUpdateError) {
      console.error('Club Commander seat update error:', seatUpdateError);
    ...
    if (entryUpdateError) {
      console.error('Club Commander waitlist update error:', entryUpdateError);
    }
    ...
    console.error('Club Commander waitlist seat API error:', error);
    return res.status(500).json({
      success: false,
