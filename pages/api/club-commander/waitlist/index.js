/**
 * Club Commander Waitlist Join API - POST /api/club-commander/waitlist/join
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Verify venue exists and has Club Commander enabled
    ...
    if (!venue.club_commander_enabled) {
      return res.status(400).json({
        success: false,
        error: { code: 'VENUE_NOT_CLUB_COMMANDER', message: 'Venue is not using Club Commander' }
      });
    }
    ...
    if (insertError) {
      console.error('Club Commander waitlist insert error:', insertError);
    ...
    console.error('Club Commander waitlist API error:', error);
    return res.status(500).json({
      success: false,
