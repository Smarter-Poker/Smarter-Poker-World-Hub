...
async function edgeHandler(req: Request) {
  ...
  try {
    ...
    if (profRes.error) {
      console.warn(
        '[API/MLB/Teams] Error fetching v_team_profile (table may be missing):',
        profRes.error.message
      );
      return new Response(
        JSON.stringify({
          teams: [],
          globalEdgeActive: false,
          summary: { gradedCount: 0, eliteCount: 0, strongCount: 0 },
          slateDate,
        }),
        {
          status: 200,
    ...
    for (const p of propsData) {
      ...
      if (!existing || score > existing.score) {
        teamGrade.set(teamId, {
          ...
          edgeCount,
          topProp: p.prop ?? null,
          topLine: p.line != null ? Number(p.line) : null,
