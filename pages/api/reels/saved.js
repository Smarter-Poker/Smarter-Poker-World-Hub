 ...
 * GET /api/reels/saved
 *
 * Authenticated saved collection. Historical targets are returned only when
 * they still satisfy the canonical public Reel contract.
 */
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const result = await readSavedPokerReels({
            userId: user.id,
        ...
        console.warn('[api/reels/saved] failed:', error?.message || error);
        return res.status(503).json({
            success: false,
            error: 'Saved Reels are temporarily unavailable',
        });
    }
}
