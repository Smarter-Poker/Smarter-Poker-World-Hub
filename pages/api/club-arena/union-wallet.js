...
async function verifyUnionLead(token, unionId) {
  ...
  if (['admin', 'superadmin'].includes(profile?.role))
    return { user, isLead: true, isPlatformAdmin: true };
    ...
    try {
      ...
      if (action === 'get_balances') {
        ...
        const { data: wallet } = await supabaseAdmin
          .from('union_wallets')
          .select(
            'chip_balance, rake_wallet, bbj_wallet, promo_wallet, insurance_wallet, spin_reserve_wallet, total_rake_collected, total_settlements'
          )
          .eq('union_id', unionId)
          .maybeSingle();
        ...
        const { data: spinPools } = await supabaseAdmin
          ...
          .eq('is_active', true);
        const spinDeployed = (spinPools || []).reduce((sum, p) => sum + Number(p.balance || 0), 0);
        ...
        return res.json({
          ...
          spin_treasury: {
            ...
            pools: (spinPools || []).map((p) => ({
              id: p.id,
              owner_id: p.club_id,
              owner_kind: p.owner_kind,
              balance: Number(p.balance || 0),
              seeded_amount: Number(p.seeded_amount || 0),
              total_deposited: Number(p.total_deposited || 0),
              total_drawn: Number(p.total_drawn || 0),
            })),
          },
          bbj_pool: pool
            ? {
      ...
      if (!auth.isLead) {
        return res
          .status(403)
          .json({ success: false, error: 'Union Lead access required for fund transfers' });
      }
      ...
      if (action === 'send_to_club') {
        ...
        const txNote = (notes?.trim() || `Union transfer to ${club.name}`)
          .slice(0, 500)
          .replace(/[;'"\\]/g, '');
        const { data: sendRes, error: sendErr } = await supabaseAdmin.rpc(
        ...
        if (sendErr || sendRes?.success === false) {
          ...
          if (sendRes?.duplicate) {
            return res
              .status(409)
              .json({ success: false, error: 'This transfer was already processed' });
          }
          console.warn('[union-wallet] send_to_club failed:', msg);
          const status =
            String(msg).includes('insufficient') || String(msg).includes('not in this union')
              ? 400
              : 500;
      ...
      if (action === 'move_rake_to_chips') {
        ...
        const txNote = (
          notes?.trim() || `Moved ${amt.toLocaleString()} from rake wallet to chip balance`
        )
          .slice(0, 500)
          .replace(/[;'"\\]/g, '');
        const { data: moveRes, error: moveErr } = await supabaseAdmin.rpc(
          'fn_union_move_rake_to_chips_atomic',
          {
            p_union_id: unionId,
            p_amount: amt,
            p_notes: txNote,
        ...
        if (moveErr || moveRes?.success === false) {
          ...
          if (moveRes?.duplicate) {
            return res
              .status(409)
              .json({ success: false, error: 'This move was already processed' });
          }
      ...
      if (action === 'fund_spin_reserve') {
        ...
        if (
          !opId ||
          !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(opId)
        ) {
        ...
        if (fundErr || fundRes?.ok !== true) {
          if (fundRes?.duplicate) {
            return res
              .status(409)
              .json({ success: false, error: 'This funding was already processed' });
          }
      ...
      if (action === 'fund_bbj_pool') {
        ...
        const fundSplitOk =
          Number.isFinite(fMain) &&
          Number.isFinite(fBackup) &&
          Number.isFinite(fPromo) &&
          fMain >= 0 &&
          fBackup >= 0 &&
          fPromo >= 0 &&
          Math.round(fMain + fBackup + fPromo) === 100;

        const { data: fundRes, error: fundErr } = await supabaseAdmin.rpc(
          'fn_union_fund_bbj_pool',
          {
            p_union_id: unionId,
            p_amount: amount,
      ...
      // ── BBJ_BACKUP_TRANSFER — partial moves out of the backup bank ──────────
      //
      // Dan 2026-08-24: "funds are allowed to be moved to the promo fund or to
      // the main BBJ if the owner chooses to."
      //
      // Before this the ONLY route out of backup was
      // fn_bbj_reseed_main_from_backup, which moves the whole balance and only
      // when main is drained. Under that behaviour backup is not a reserve tier,
      // it is a lump sum that empties on drain. There was no route at all to
      // the promo wallet.
      //
      // The balance check, the FOR UPDATE lock and the club-membership check all
      // live in the RPC rather than here, on purpose: this handler is one caller
      // and the guarantee has to hold for every caller.
      if (action === 'bbj_backup_transfer') {
        const { amount, destination, notes } = payload;
        const { data: xfer, error: xferErr } = await supabaseAdmin.rpc(
          'fn_union_bbj_backup_transfer',
          {
            p_union_id: unionId,
            p_amount: amount,
        ...
        if (xferErr || xfer?.success === false) {
          const msg = xferErr?.message || xfer?.error || 'Backup transfer failed';
          if (xfer?.duplicate) {
            return res
              .status(409)
              .json({ success: false, error: 'This transfer was already processed' });
          }
          console.warn('[union-wallet] bbj_backup_transfer failed:', msg);
          const status = String(msg).includes('insufficient') ? 400 : 500;
          return res.status(status).json({ success: false, error: msg, available: xfer?.available });
        }
        return res.json({
          success: true,
          message:
            destination === 'main'
              ? `${Number(amount).toLocaleString()} moved from backup to the main jackpot`
              : `${Number(amount).toLocaleString()} moved from backup to the promo wallet`,
          destination,
          amount: xfer.amount,
          backupAfter: xfer.backup_after,
          mainAfter: xfer.main_after ?? null,
          promoAfter: xfer.promo_after ?? null,
        });
      }

      // ── PROMO_SEND — spend the promo wallet ─────────────────────────────────
      //
      ...
      if (action === 'promo_send') {
        const { amount, destination, clubId: promoClubId, notes } = payload;
        if (destination === 'club' && !promoClubId) {
          return res
            .status(400)
            .json({ success: false, error: 'clubId is required when sending to a club' });
        }
        const { data: sent, error: sendErr } = await supabaseAdmin.rpc('fn_union_promo_send', {
          p_union_id: unionId,
          p_amount: amount,
          p_destination: destination,
          p_club_id: destination === 'club' ? promoClubId : null,
          p_op_id: opId,
          p_created_by: auth.user.id,
          p_notes: notes || null,
        });
        if (sendErr || sent?.success === false) {
          const msg = sendErr?.message || sent?.error || 'Promo send failed';
          if (sent?.duplicate) {
            return res
              .status(409)
              .json({ success: false, error: 'This transfer was already processed' });
          }
          console.warn('[union-wallet] promo_send failed:', msg);
          const status =
            String(msg).includes('insufficient') || String(msg).includes('not in this union')
              ? 400
              : 500;
          return res.status(status).json({ success: false, error: msg, available: sent?.available });
        }
        return res.json({
          success: true,
          message:
      ...
      // ── GET_RAKE_DETAIL — everything behind the Rake Treasury row ───────────
      //
      // Dan 2026-08-24: "rake treasury should open up to see all the data for
      // all rake accumulated."
      //
      // Two independent readings of the same money, deliberately reported side
      // by side rather than reconciled into one number:
      //   ledger — union_rake_ledger_checkpoint, lifetime credits and debits
      //   wallet — union_wallets.rake_wallet, the live balance
      // They have drifted 757.18 historically (audit 2026-08-23) and showing one
      // figure would hide that. If they disagree the operator should see it, so
      // `drift` is computed here rather than swept up.
      //
      // The per-day and per-club breakdowns come from club_rake_daily_user, the
      // rollup the rest of the product already reads. The raw alternative,
      // rake_distribution_legs, holds 2.27M rows with no index on union_id: the
      // equivalent aggregate measured 7.7s of sequential scan on 2026-08-24 and
      // would have timed this route out every time the panel opened.
      if (action === 'get_rake_detail') {
        const days = 30;
        const [walletRow, ledger, byDay, byClub, recent] = await Promise.all([
          supabaseAdmin
            .from('union_wallets')
            .select('rake_wallet, chip_balance, updated_at')
            .eq('union_id', unionId)
            .maybeSingle(),
          supabaseAdmin.rpc('fn_union_rake_ledger_summary', { p_union_id: unionId }),
          supabaseAdmin.rpc('fn_union_rake_by_day', { p_union_id: unionId, p_days: days }),
          supabaseAdmin.rpc('fn_union_rake_by_club', { p_union_id: unionId, p_days: days }),
          supabaseAdmin
            .from('union_wallet_transactions')
            .select('id, direction, amount, tx_type, notes, created_at, club_id, clubs(name)')
            .eq('union_id', unionId)
            .eq('wallet', 'rake_wallet')
            .order('created_at', { ascending: false })
            .limit(50),
        ]);

        const balance = Number(walletRow.data?.rake_wallet || 0);
        const led = (ledger.data && ledger.data[0]) || null;
        const ledgerNet = led ? Number(led.net || 0) : null;

        return res.json({
          success: true,
          windowDays: days,
          balance,
          updatedAt: walletRow.data?.updated_at || null,
          ledger: led
            ? {
                credits: Number(led.credits || 0),
                debits: Number(led.debits || 0),
                net: ledgerNet,
                rowsSeen: Number(led.rows_seen || 0),
                asOf: led.as_of || null,
                lastVerifiedAt: led.last_verified_at || null,
                throughDay: led.through_day || null,
              }
            : null,
          // Positive means the wallet holds more than the ledger accounts for.
          // Rounded to cents so float noise is not reported as a discrepancy.
          drift: ledgerNet === null ? null : Math.round((balance - ledgerNet) * 100) / 100,
          byDay: byDay.data || [],
          byClub: byClub.data || [],
      ...
      // ── GET_BBJ_DETAIL — the three banks, plus who funded them ──────────────
      //
      // Dan 2026-08-24: "the same way we track 'rake credited to players' we
      // should be tracking the BBJ the same way so agents, clubs and union know
      // who accredited what to the BBJ, back up BBJ and promo fund."
      //
      // fn_bbj_credited_report is the per-player attribution shipped alongside
      // this; it reads bbj_attribution_daily_user, which splits each hand's drop
      // 50/25/25 per contributing player rather than per club. Per-CLUB
      // attribution is NOT derivable today: every hand on a union table is
      // stamped with the union's id as its club_id in both rake_records and
      // bbj_contributions, so the club that hosted the hand is not recorded.
      // Surfaced to the operator rather than faked here.
      if (action === 'get_bbj_detail') {
        const [pool, credited, byDay, recent] = await Promise.all([
          supabaseAdmin
            .from('bbj_pools')
            .select(
              'id, main_balance, backup_balance, promo_balance, pool_amount, ' +
                'alloc_cum_amount, updated_at, status'
            )
            .eq('union_id', unionId)
            .eq('status', 'active')
            .maybeSingle(),
          supabaseAdmin.rpc('fn_bbj_credited_report', { p_union_id: unionId, p_days: 30 }),
          supabaseAdmin.rpc('fn_bbj_contributions_by_day', { p_union_id: unionId, p_days: 30 }),
          supabaseAdmin
            .from('union_wallet_transactions')
            .select('id, wallet, direction, amount, tx_type, notes, created_at, clubs(name)')
            .eq('union_id', unionId)
            .in('wallet', ['bbj_wallet', 'promo_wallet'])
            .order('created_at', { ascending: false })
            .limit(50),
        ]);

        const p = pool.data || {};
        const main = Number(p.main_balance || 0);
        const backup = Number(p.backup_balance || 0);
        const promo = Number(p.promo_balance || 0);
        const banked = main + backup + promo;

        return res.json({
          success: true,
          poolId: p.id || null,
          banks: { main, backup, promo, total: banked },
          // The live split against the 50/25/25 house rule. Reported, not
          // asserted: the rule governs each ALLOCATION, and payouts come out of
          // main only, so a healthy pool drifts away from 50/25/25 as it pays.
          splitPct: banked > 0
            ? {
                main: Math.round((main / banked) * 1000) / 10,
                backup: Math.round((backup / banked) * 1000) / 10,
                promo: Math.round((promo / banked) * 1000) / 10,
              }
            : null,
          allocatedLifetime: Number(p.alloc_cum_amount || 0),
          updatedAt: p.updated_at || null,
          creditedBy: credited.data || [],
          byDay: byDay.data || [],
          transactions: recent.data || [],
        });
      }
