    ...
    if (vip) {
      ...
      for (const [id, entry, expected, expectedCard] of checks) {
        ...
        if (Number(entry.price) !== Number(expected)) {
          warnings.push(
            id + ': catalog says ' + expected + ', diamondStoreData says ' + entry.price
          );
        }
  ...
  try {
    ...
    if (warnings.length > 0) {
      ...
      if (strict) {
        return res.status(503).json({
          success: false,
          error: 'Current Marketplace Pricing Or Payment Capabilities Could Not Be Verified.',
          code: 'STORE_CATALOG_DRIFT',
        });
      }
    }
    ...
    try {
      ...
      /* Local error reporting is best-effort. */
    }
