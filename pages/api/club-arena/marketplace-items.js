...
export default async function handler(req, res) {
  try {
    ...
    const requestedClubId = Array.isArray(req.query.clubId)
      ? req.query.clubId[0]
      : req.query.clubId;
    if (requestedClubId && !isUUID(requestedClubId)) {
      return res.status(400).json({ error: 'Invalid clubId format' });
    }
    const requestedItemId = Array.isArray(req.query.itemId)
      ? req.query.itemId[0]
      : req.query.itemId;
    ...
    try {
      ...
      if (requestedClubId) {
        ...
        if (rows.length > 1) {
          const { data: stock, error: stockError } = await getSupabase()
            ...
            .select('club_id, name, category, item_type, grant_spec, is_active, stackable')
            .in(
              'club_id',
              rows.map((m) => m.club_id)
            )
            .eq('is_active', true)
            .limit(5000);
          if (stockError) throw stockError;
          const counts = new Map();
          for (const row of (stock || []).filter(isDeliverableShopItem)) {
      ...
      let itemsPromise = getSupabase()
        ...
        // Fetch one sentinel row beyond the supported storefront size.
        // This prevents an unbounded RPC fanout while still failing
        // closed instead of silently hiding sellable inventory.
        .limit(requestedItemId ? 1 : MAX_ACTIVE_SHOP_ITEMS + 1);
      ...
      if (requestedItemId) {
        ...
        [profileResult, itemsResult, countResult, mineResult, purchaseResult] = await Promise.all([
          ...
          getSupabase()
            .from('club_shop_purchases')
            .select(
              'id, item_id, price_paid, currency, created_at, refunded_at, club_shop_items(name, category)'
            )
            .eq('club_id', clubId)
            .eq('buyer_id', user.id)
            .eq('item_id', requestedItemId)
      ...
      if (!requestedItemId && (itemsResult.data || []).length > MAX_ACTIVE_SHOP_ITEMS) {
        return res.status(409).json({
          success: false,
          error: 'The Club Shop Catalog Exceeds The Verified Storefront Limit.',
          code: 'SHOP_CATALOG_LIMIT_EXCEEDED',
          maximumActiveItems: MAX_ACTIVE_SHOP_ITEMS,
        });
      }
      const profileRow = profileResult.data;
      // Do not advertise an active row that the currently installed
      // redemption authority cannot prove it will deliver. Historical and
      // disabled rows remain available through admin/receipt reads.
      const items = (itemsResult.data || []).filter(isDeliverableShopItem);
      ...
      const counts = {};
      (countResult.data || []).forEach((r) => {
        counts[r.item_id] = (counts[r.item_id] || 0) + 1;
      });
      const mine = {};
      (mineResult.data || []).forEach((r) => {
        mine[r.item_id] = (mine[r.item_id] || 0) + 1;
      });
      ...
      const [availabilityRows, packageResult] = await Promise.all([
        Promise.all(
          items.map(async (item) => {
            ...
            if (error) {
              console.warn(
                '[marketplace-items] availability check failed:',
                item.id,
                error.message
              );
            ...
            if (typeof data !== 'string')
              return [item.id, data || { ok: false, reason: 'verification_unavailable' }];
            try {
      ...
      const itemsWithCount = items.map((i) => {
        const availability = isDeliverableShopItem(i)
          ? availabilityByItem.get(i.id) || {
              ok: false,
        ...
        const effectivePrice =
          availability.ok === true && Number.isSafeInteger(rpcPrice) && rpcPrice >= 0
            ? rpcPrice
        ...
        const listPrice =
          ...
          listPriceValue >= effectivePrice
            ...
            : Number(i.price);
        const cardQuote =
          availability.ok === true && walletIsValid && walletBalance >= 0 && packageCatalog
        ...
        return {
          ...
          availability_reason:
            availability.ok === true ? null : availability.reason || 'unavailable',
          ...
          card_quote: cardQuote
            ? {
                ...
                unitPriceCents: cardQuote.unitPriceCents,
                baseDiamonds: cardQuote.baseDiamonds,
                bonusDiamonds: cardQuote.bonusDiamonds,
      ...
      try {
        ...
        console.warn(
          '[marketplace-items] FK join failed, falling back:',
          _joinErr?.message || _joinErr
        );
        ...
        const { data: purchases, error: purErr } = await fallbackQuery.order('created_at', {
          ascending: false,
        });
