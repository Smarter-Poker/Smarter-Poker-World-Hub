/**
 * 💬 STRATEGY COMMENT REWARD — Diamond Rewards Standard v2
 * ═══════════════════════════════════════════════════════════════════════════
 * Action key: strategy_comment. Amount, 3/day limit, caps, velocity and dedup
 * all live in award_diamonds_v2 — no local constants, no local cap math.
 *
 * SECURITY FIXES vs v1:
 *  - The claimer must be the AUTHOR of the comment (social_comments.author_id).
 *    Previously ANY authenticated user could claim on ANYONE's comment id.
 *  - reference_id is user-scoped: `strategy_comment_<userId>_<commentId>`
 *    (the old `comment_reward_<commentId>` collided across users).
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
const ACTION_KEY = 'strategy_comment';
...
export default async function handler(req, res) {
  try {
    ...
    try {
        ...
        // ── Eligibility 2: the comment exists AND the claimer authored it ──
        ...
        if (!comment) {
            return res.status(200).json({
                success: false, reason: 'not_eligible', awarded: 0, diamondsAwarded: 0,
                message: 'Comment not found for this account'
            });
        }
        ...
        const award = await awardDiamondsV2(supabase, {
            ...
            referenceId: `${ACTION_KEY}_${userId}_${commentId}`,
            targetId: commentId,
            metadata: { comment_id: commentId, content_length: (comment.content || '').trim().length }
        });

        return sendAwardResult(res, award, 'Comment Reward!');
