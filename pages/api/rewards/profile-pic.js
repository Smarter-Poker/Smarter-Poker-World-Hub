/**
 * 📸 PROFILE PIC REWARD — Diamond Rewards Standard v2
 * ═══════════════════════════════════════════════════════════════════════════
 * Action key: profile_pic (lifetime once, exempt from the daily cap).
 * Amount resolved server-side by award_diamonds_v2; no local constant, no
 * local daily-cap arithmetic. Dedup: reference_id `profile_pic_<userId>`.
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
export default async function handler(req, res) {
  try {
    ...
    try {
        // ── Eligibility: avatar actually present on the server-side profile ──
        ...
        if (!profile || !profile.avatar_url || profile.avatar_url.trim().length === 0) {
            return res.status(200).json({
                ...
                awarded: 0,
                diamondsAwarded: 0,
                message: 'Upload a profile picture to earn diamonds!'
            });
        }
        ...
        const award = await awardDiamondsV2(supabase, {
            ...
            targetId: null,
            metadata: { avatar_url: String(profile.avatar_url).substring(0, 100) }
        });

        return sendAwardResult(res, award, 'Profile Pic Uploaded!');
