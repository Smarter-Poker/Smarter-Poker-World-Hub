/**
 * ⭐ VIDEO FAVORITE REWARD — Diamond Rewards Standard v2
 * ═══════════════════════════════════════════════════════════════════════════
 * Action key: video_favorite. Amount, 3/day limit, caps, velocity and dedup
 * live in award_diamonds_v2. Eligibility owned here: the favorite row exists
 * for THIS user. reference_id: `video_favorite_<userId>_<videoId>`.
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
const ACTION_KEY = 'video_favorite';
  ...
  try {
    ...
    try {
        ...
        const award = await awardDiamondsV2(supabase, {
            ...
            metadata: { video_id: String(videoId) }
        });

        return sendAwardResult(res, award, 'Video Favorite!');
