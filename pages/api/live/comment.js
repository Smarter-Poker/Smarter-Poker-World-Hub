...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: ban } = await supabase
      ...
      .maybeSingle();
    if (ban)
      return res.status(403).json({ error: 'You are banned from commenting on this stream' });
    ...
    if (stream.slow_mode) {
      ...
      const delaySecs =
        typeof cfg === 'number' && cfg >= 1 && cfg <= 120 ? cfg : DEFAULT_SLOW_MODE_DELAY_SECS;
