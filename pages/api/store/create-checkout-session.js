 ...
 * Success: { success: true, duplicate?, data: { session_id, request_id, url, offer } }
 */
...
async function loadDiamondPackages({ requireCurrent = false } = {}) {
  ...
  try {
    ...
    console.warn(
      '[Checkout] current diamond_packages catalog unavailable:',
      error?.message || error
    );
...
function resolveCheckoutRedirect(rawUrl, baseUrl, fallbackPath) {
  ...
  try {
    ...
    return candidate.toString().replace(/%7BCHECKOUT_SESSION_ID%7D/gi, '{CHECKOUT_SESSION_ID}');
  } catch (_) {
    return fallbackUrl;
  }
}
...
function isConcurrentStripeReplay(error) {
  return error?.type === 'StripeIdempotencyError' || error?.code === 'idempotency_key_in_use';
}

function isAmbiguousStripeCreateFailure(error) {
  return (
    isConcurrentStripeReplay(error) ||
...
function vipSubscriptionClaimExpiry(expiresAtSeconds) {
  ...
  return new Date((stripeExpiry + VIP_SUBSCRIPTION_CLAIM_GRACE_SECONDS) * 1000).toISOString();
}
...
function exactJsonValueMatches(actual, expected) {
...
function isCheckoutOfferConfirmationShape(type, offer) {
  const validAccountId =
    typeof offer?.accountId === 'string' &&
    offer.accountId === offer.accountId.trim() &&
    offer.accountId.length > 0 &&
    offer.accountId.length <= 160;
  if (!validAccountId || offer.version !== 1 || offer.type !== type || offer.currency !== 'usd') {
    return false;
  }

  if (type === 'diamonds') {
    const diamondOfferKeys = [
      'version',
      'accountId',
      'type',
      'currency',
    ...
    const hasRedemption = Object.prototype.hasOwnProperty.call(offer, 'redemption');
    if (
      !hasExactObjectKeys(
        offer,
        hasRedemption ? [...diamondOfferKeys, 'redemption'] : diamondOfferKeys
      ) ||
      ...
      return false;
    if (hasRedemption) {
      const redemption = offer.redemption;
      if (
        !hasExactObjectKeys(redemption, [
          'kind',
          'clubId',
          'itemId',
          'itemPriceDiamonds',
        ...
        redemption.kind !== 'club_shop' ||
        !UUID_RE.test(String(redemption.clubId || '')) ||
        !UUID_RE.test(String(redemption.itemId || '')) ||
        !Number.isSafeInteger(redemption.itemPriceDiamonds) ||
        redemption.itemPriceDiamonds < 0 ||
        !Number.isSafeInteger(redemption.cardChargeCents) ||
        redemption.cardChargeCents < 1
      )
        return false;
    }
    let totalCents = 0n;
    ...
    for (const item of offer.items) {
      if (
        ...
        !/^[a-z0-9][a-z0-9_-]{0,63}$/i.test(item.packageId) ||
        (previousPackageId && previousPackageId.localeCompare(item.packageId) >= 0) ||
        !Number.isSafeInteger(item.quantity) ||
        item.quantity < 1 ||
        item.quantity > MAX_DIAMOND_QUANTITY_PER_PACKAGE ||
    ...
    return (
      ...
      offer.totalCents === Number(totalCents) &&
      offer.totalDiamonds === Number(totalDiamonds) &&
      offer.totalBonus === Number(totalBonus) &&
      (!hasRedemption || offer.redemption.cardChargeCents === offer.totalCents)
    );
  }

  if (type === 'subscription') {
    return (
      hasExactObjectKeys(offer, [
        'version',
        'accountId',
        'type',
        'currency',
      ...
      ['monthly', 'yearly'].includes(offer.plan) &&
      offer.interval === (offer.plan === 'monthly' ? 'month' : 'year') &&
      offer.quantity === 1 &&
      Number.isSafeInteger(offer.unitCents) &&
      offer.unitCents > 0 &&
      offer.totalCents === offer.unitCents
    );
  }
  ...
  if (type === 'merchandise') {
    ...
    for (const item of offer.items) {
      if (
        !hasExactObjectKeys(item, ['id', 'variantId', 'quantity', 'unitCents']) ||
        ...
        (item.variantId !== null &&
          (typeof item.variantId !== 'string' ||
            item.variantId !== item.variantId.trim() ||
        ...
        return false;
      const identity = `${item.id}\u0000${item.variantId || ''}`;
      if (previousIdentity && previousIdentity.localeCompare(identity) >= 0) return false;
      previousIdentity = identity;
      totalCents += BigInt(item.unitCents) * BigInt(item.quantity);
    }
    return totalCents <= BigInt(Number.MAX_SAFE_INTEGER) && offer.totalCents === Number(totalCents);
  }
...
/**
 * Serialize the exact authenticated, server-priced offer used for Checkout.
 * The client sends this object back as `offerConfirmation`; every field must
 * match so an account switch, option change, or catalog reprice cannot
 * silently change terms.
 */
function buildCheckoutOffer(type, preparedCheckout, accountId) {
  const normalizedAccountId = typeof accountId === 'string' ? accountId.trim() : '';
  if (!normalizedAccountId) return null;

  if (type === 'diamonds') {
    const items = preparedCheckout?.resolvedPackages
      ?.map((pkg) => ({
        packageId: pkg.key,
        quantity: pkg.quantity,
        unitCents: pkg.priceCents,
        diamonds: pkg.diamonds,
        bonus: pkg.bonus,
      }))
      .sort((a, b) => a.packageId.localeCompare(b.packageId));
    const totals = preparedCheckout?.checkoutTotals;
    if (
      !Array.isArray(items) ||
      items.length === 0 ||
      items.some(
      ...
      !Number.isSafeInteger(totals?.cardChargeCents) ||
      totals.cardChargeCents < 1 ||
      !Number.isSafeInteger(totals?.diamonds) ||
      totals.diamonds < 1 ||
      !Number.isSafeInteger(totals?.bonus) ||
      totals.bonus < 0
    )
      return null;
    const offer = {
      ...
      type: 'diamonds',
      currency: 'usd',
      totalCents: totals.cardChargeCents,
      totalDiamonds: totals.diamonds,
      totalBonus: totals.bonus,
      items,
    };
    return isCheckoutOfferConfirmationShape(type, offer) ? offer : null;
  }

  if (type === 'subscription') {
    const plan = preparedCheckout?.plan;
    if (
      typeof plan?.key !== 'string' ||
      !['monthly', 'yearly'].includes(plan.key) ||
      !['month', 'year'].includes(plan.interval) ||
      !Number.isSafeInteger(plan.unitAmount) ||
      plan.unitAmount < 1
    )
      return null;
    const offer = {
      ...
      interval: plan.interval,
      quantity: 1,
      unitCents: plan.unitAmount,
      totalCents: plan.unitAmount,
    };
    return isCheckoutOfferConfirmationShape(type, offer) ? offer : null;
  }

  if (type === 'vip_lifetime') {
    const unitCents = preparedCheckout?.unitAmount;
    if (!Number.isSafeInteger(unitCents) || unitCents < 1) return null;
    const offer = {
      version: 1,
      accountId: normalizedAccountId,
      type: 'vip_lifetime',
  ...
  if (type === 'merchandise') {
    const items = preparedCheckout?.resolvedItems
      ?.map((item) => ({
        id: item.id,
    ...
    const totalCents = preparedCheckout?.totalCents;
    const offer = {
      version: 1,
      accountId: normalizedAccountId,
      type: 'merchandise',
...
function computeCheckoutIntentHash(type, preparedCheckout, redemptionIntent = null) {
  ...
  if (type === 'diamonds') {
    ...
    intent = preparedCheckout.resolvedItems
      .map(({ id, variantId, quantity, priceCents, providerVariant }) => ({
        id,
  ...
  const redemptionIdentity =
    redemptionIntent?.kind === 'club_shop'
      ? {
...
function normalizeRedemptionIntent(type, raw) {
  ...
  if (type !== 'diamonds' || typeof raw !== 'object' || Array.isArray(raw)) {
    throw new CheckoutInputError(
      'INVALID_REDEMPTION_INTENT',
      'Invalid card-funded redemption target'
    );
  }
  ...
  if (
    ...
    if (
      typeof expectedCardChargeCents !== 'number' ||
...
const CLUB_SHOP_AVAILABILITY_ERRORS = Object.freeze({
  ...
  not_yet_available: [
    'CLUB_ITEM_NOT_YET_AVAILABLE',
    'That Club Shop Item Is Not On Sale Yet.',
    409,
  ],
  ...
  limit_reached: [
    'CLUB_ITEM_LIMIT_REACHED',
    'You Have Reached The Purchase Limit For That Item.',
    409,
  ],
});
...
async function preflightClubShopCardRedemption(userId, intent, preparedCheckout, profile) {
  ...
  if (!availability?.ok) {
    const [code, message, status] = CLUB_SHOP_AVAILABILITY_ERRORS[availability?.reason] || [
      'CLUB_ITEM_UNAVAILABLE',
      'That Club Shop Item Is Not Available.',
      409,
    ];
  ...
  if (!isDeliverableShopItem(itemResult.data)) {
    throw new CheckoutInputError(
      'CLUB_ITEM_FULFILLMENT_UNAVAILABLE',
      'That Club Shop Item Does Not Have A Verified Digital Delivery.',
      409
    );
  }
  ...
  const exactQuote =
    ...
    (selected[0].diamonds + selected[0].bonus) * selected[0].quantity === quote.diamondsPurchased;
  if (!exactQuote) {
    throw new CheckoutInputError(
      'CARD_QUOTE_CHANGED',
...
async function prepareCheckout(type, items, { requireCurrentDiamondCatalog = false } = {}) {
  if (type === 'diamonds') {
    const packageCatalog = await loadDiamondPackages({
      requireCurrent: requireCurrentDiamondCatalog,
    });
  ...
  if (type === 'vip_lifetime') {
    /* Lifetime is $499 and the amount lives HERE, on the server, exactly
           ...
           src/data/diamondStoreData.js, which is display-only. */
    const item = items[0];
    const key = String(item?.plan ?? item?.planId ?? item?.id ?? '')
      .replace(/^vip-/, '')
      .toLowerCase();
    if (key !== 'lifetime') {
    ...
    if (priceId) {
      ...
      try {
        ...
        console.warn(
          '[Checkout] STRIPE_VIP_LIFETIME_PRICE_ID points at an unknown Stripe price:',
          priceId,
          error?.message
        );
        throw new CheckoutInputError(
      ...
      if (
        ...
        console.warn(
          '[Checkout] STRIPE_VIP_LIFETIME_PRICE_ID is not the fixed $499 USD one-time price:',
          priceId
        );
        throw new CheckoutInputError(
          'LIFETIME_NOT_CONFIGURED',
  ...
  if (type === 'subscription') {
    ...
    if (plan.priceId) {
      try {
        ...
        console.warn(
          `[Checkout] ${plan.envVar} points at an unknown Stripe price:`,
          plan.priceId,
          error?.message
        );
        throw new CheckoutInputError(
  ...
  const MAX_ORDER_TOTAL_CENTS = 200000;
  ...
  if (catalogError) {
    ...
    throw new CheckoutInputError(
      'CATALOG_UNAVAILABLE',
      'Could not verify the merchandise catalog',
      503
    );
  }
  ...
  const lineIdentities = stockCheckLines.map((line) => `${line.id}\u0000${line.variant_id || ''}`);
  ...
  const { data: stockRaw, error: stockError } = await getSupabase().rpc('reserve_merch_order', {
    p_items: stockCheckLines,
    p_dry_run: true,
  });
  if (stockError) {
  ...
  const selectedVariantIds = [
    ...new Set(stockCheckLines.map((line) => line.variant_id).filter(Boolean)),
  ];
  ...
  const resolvedItems = items.map((item) => {
    ...
    const pricedLine = pricedLines.find(
      (line) => line?.id === item.id && String(line?.variant_id || '') === String(variantId || '')
    );
    const itemMetadata =
      catalog.metadata && typeof catalog.metadata === 'object' ? catalog.metadata : {};
    ...
    const rawPrice = Number(pricedLine?.price_usd ?? catalog.price_usd);
    const priceCents = Math.round(rawPrice * 100);
    if (
      !Number.isFinite(rawPrice) ||
      rawPrice <= 0 ||
      !Number.isSafeInteger(priceCents) ||
      Math.abs(rawPrice * 100 - priceCents) > 1e-8
    ) {
      throw new CheckoutInputError(
        'UNPRICED_ITEM',
        'That Merchandise Option Does Not Have A Verified Card Price.'
      );
    }
    return {
      id: item.id,
      ...
      providerVariant,
      madeToOrder:
        itemMetadata.made_to_order === true ||
        provider === 'printful' ||
        provider === 'provider_pending',
    };
  });
  const totalCentsBigInt = resolvedItems.reduce(
    (sum, item) => sum + BigInt(item.priceCents) * BigInt(item.quantity),
    0n
  );
  if (totalCentsBigInt > BigInt(MAX_ORDER_TOTAL_CENTS)) {
    throw new CheckoutInputError(
      'ORDER_TOO_LARGE',
      `Maximum Order Total Is $${(MAX_ORDER_TOTAL_CENTS / 100).toLocaleString('en-US')}`
    );
  }
  ...
  const providerNames = [
    ...new Set(resolvedItems.map((item) => item.fulfillmentProvider || 'manual')),
  ];
  const automaticFulfillment =
    isPrintfulReady() &&
...
function linkedCheckoutSessionMatches(type, userId, row, session, { requestId, intentHash }) {
  const metadata = session?.metadata || {};
  const recordMetadataKey = type === 'merchandise' ? 'order_id' : 'purchase_id';
  return Boolean(
    typeof requestId === 'string' &&
    /^[a-z0-9][a-z0-9_-]{11,127}$/i.test(requestId) &&
    typeof intentHash === 'string' &&
    /^[a-f0-9]{64}$/i.test(intentHash) &&
    session?.id === row?.stripe_checkout_session_id &&
    session?.mode === 'payment' &&
    metadata.type === type &&
    metadata.user_id === userId &&
    metadata.checkout_request_id === requestId &&
    metadata.checkout_intent_hash === intentHash &&
    metadata[recordMetadataKey] === row?.id
  );
}

async function inspectLinkedCheckout(type, userId, row, identity) {
  ...
  try {
    ...
    if (!linkedCheckoutSessionMatches(type, userId, row, session, identity)) {
      console.warn('[Checkout] Stored checkout session identity could not be verified.');
...
function classifyDurableSubscriptionClaimSession(
  session,
  claim,
  { customerId, userId, requestId, intentHash }
) {
...
async function inspectDurableSubscriptionClaim(claim, identity) {
  ...
  try {
    ...
    console.warn('[Checkout] Durable subscription session lookup failed:', error?.message || error);
...
function classifyStoredCheckout(row, intentHash, linkedInspection = null) {
  ...
  const terminalOrRefunded =
    ['refunded', 'canceled', 'cancelled'].includes(row.status) ||
...
async function findExistingCheckout(type, userId, checkoutRequestId, intentHash) {
  if (!checkoutRequestId || !['diamonds', 'merchandise', 'vip_lifetime'].includes(type))
    return null;
  const table =
    type === 'diamonds'
      ? 'diamond_purchases'
  ...
  if (!data.metadata?.checkout_intent_hash || data.metadata.checkout_intent_hash !== intentHash)
...
async function findActiveLifetimeCheckout(userId) {
  ...
  return inspectLinkedCheckout('vip_lifetime', userId, data, {
    requestId: data.metadata?.checkout_request_id,
    intentHash: data.metadata?.checkout_intent_hash,
  });
}
  ...
  try {
    ...
    try {
      ...
      const {
        ...
        successUrl,
        cancelUrl,
        redemptionIntent: rawRedemptionIntent,
        offerConfirmation: rawOfferConfirmation,
      } = req.body || {};
      ...
      const offerConfirmationProvided = Object.prototype.hasOwnProperty.call(
        req.body || {},
        'offerConfirmation'
      );
      if (!offerConfirmationProvided) {
        return res.status(409).json({
          success: false,
          error: {
            code: 'OFFER_CONFIRMATION_REQUIRED',
            message: 'Review The Current Checkout Offer And Confirm Again.',
          },
        });
      }
      if (!isCheckoutOfferConfirmationShape(type, rawOfferConfirmation)) {
        return res.status(409).json({
          ...
          error: {
            ...
            message: 'The Checkout Offer Changed. Review Current Terms And Confirm Again.',
          },
        });
      }
      ...
      const rawCheckoutRequestId =
        req.headers['x-checkout-request-id'] || req.headers['x-idempotency-key'];
      ...
      if (type === 'subscription' && !checkoutRequestId) {
        return res.status(400).json({
          ...
          error: {
            code: 'REQUEST_ID_REQUIRED',
            message: 'A checkout request identifier is required',
          },
        });
      }
      ...
      let checkoutOffer = buildCheckoutOffer(type, preparedCheckout, user.id);
      if (!checkoutOffer) {
        return res.status(503).json({
          success: false,
          error: {
            code: 'CHECKOUT_OFFER_UNAVAILABLE',
            message: 'The Current Checkout Offer Could Not Be Verified. Please Try Again.',
          },
        });
      }
      ...
      if (type === 'diamonds' && redemptionIntent?.kind === 'club_shop') {
        checkoutOffer = {
          ...checkoutOffer,
          redemption: {
            kind: 'club_shop',
            clubId: redemptionIntent.club_id,
            itemId: redemptionIntent.item_id,
            itemPriceDiamonds: redemptionIntent.expected_price,
            cardChargeCents: redemptionIntent.expected_card_charge_cents,
          },
        };
      }
      if (
        !isCheckoutOfferConfirmationShape(type, checkoutOffer) ||
      ...
      const checkoutIntentHash = computeCheckoutIntentHash(
        type,
        preparedCheckout,
        redemptionIntent
      );
      ...
      if (type === 'subscription' || type === 'vip_lifetime') {
        ...
        if (profile?.stripe_customer_id) {
          ...
          const knownVipPriceIds = Object.values(VIP_SUBSCRIPTION_PLANS).map(
            (plan) => process.env[plan.envVar]
          );
      ...
      if (existingCheckout?.url) {
        return res.status(200).json({
          ...
          data: {
            ...
            request_id: existingCheckout.requestId,
      ...
      if (existingCheckout?.initializing) {
        return res.status(503).json({
          ...
          error: {
            ...
            message:
              'Your existing checkout is still being recovered. Retry this same request shortly.',
          },
        });
      }
        ...
        if (activeLifetimeCheckout?.url) {
          return res.status(200).json({
            ...
            data: {
              ...
              request_id: activeLifetimeCheckout.requestId,
        ...
        if (activeLifetimeCheckout?.initializing) {
          return res.status(503).json({
            ...
            error: {
              code: 'CHECKOUT_RECOVERY_PENDING',
              message:
                'Your existing Lifetime VIP checkout is still being recovered. Retry shortly.',
            },
          });
        }
        ...
        try {
          ...
          if (err_profiles_epk3c) {
            console.error(
              ...
              user.id,
              '- every future subscription webhook for this customer will match zero rows:',
              err_profiles_epk3c.message
            );
            throw new Error(
              'Could not link your account to the payment provider. No charge was made. Please try again.'
            );
          }
          if (!linkedRows || linkedRows.length === 0) {
            console.error(
              '[Checkout] stripe_customer_id write MATCHED ZERO ROWS for user',
              user.id,
              '- profile missing or not visible to this client.'
            );
            throw new Error(
              'Could not link your account to the payment provider. No charge was made. Please try again.'
            );
      ...
      if (type === 'subscription') {
        ...
        if (
          sessionAuthorities.some(({ authority }) => authority === STRIPE_VIP_AUTHORITY.UNKNOWN)
        ) {
          const recoveryError = new Error(
        ...
        const openSubscriptionSessions = sessionAuthorities
          ...
          .map(({ entry }) => entry);
        const matchingSession = openSubscriptionSessions.find(
          (entry) =>
        ...
        if (
          openSubscriptionSessions.length > 1 ||
          ...
          if (claim?.state === 'vip_entitlement_active') {
            ...
            return res.status(409).json({
              ...
              error: {
                ...
                message:
                  'Your Current VIP Term Is Already Active. Card Checkout Becomes Available After It Ends.',
              },
            });
          }
        ...
        if (matchingSession?.url) {
          ...
          if (recoveryError || recovered !== true) {
            const error =
              recoveryError || new Error('Subscription checkout claim could not be recovered');
      ...
      const returnRoute =
        type === 'subscription' || type === 'vip_lifetime'
          ? '/hub/vip-membership'
          : type === 'merchandise'
            ? '/hub/merch-store'
            : '/hub/diamond-store';
      ...
      let sessionConfig = {
        ...
        ...(type === 'subscription'
          ? {
              ...
              expires_at: Math.floor(Date.now() / 1000) + VIP_SUBSCRIPTION_CHECKOUT_TTL_SECONDS,
            }
          : {}),
      ...
      if (type === 'diamonds') {
        ...
        if (purchaseInsertErr?.code === '23505' && checkoutRequestId) {
          const recovered = await findExistingCheckout(
            type,
            user.id,
            checkoutRequestId,
            checkoutIntentHash
          ...
          if (recovered?.url) {
            return res.status(200).json({
              ...
              data: {
                ...
                request_id: recovered.requestId,
                url: recovered.url,
                offer: checkoutOffer,
              },
            });
        ...
        if (purchaseInsertErr || !purchase) {
          console.warn(
            '[Checkout] Failed to create pending diamond purchase:',
            purchaseInsertErr?.message
          );
        ...
        const lifetimeRecordId = deriveCheckoutRecordId(type, user.id, checkoutRequestId);
        ...
        if (lifetimeInsertErr?.code === '23505') {
          ...
          return res.status(503).json({
            ...
            error: {
              code: 'CHECKOUT_RECOVERY_PENDING',
              message:
                'Your Lifetime VIP checkout is being initialized. Retry this same request shortly.',
            },
          });
        }
        ...
        if (lifetimeInsertErr || !lifetimeRow) {
          console.warn(
            '[Checkout] Failed to create pending lifetime purchase:',
        ...
        const { resolvedItems, totalUsd, fulfillmentMode, catalogProvider } = preparedCheckout;
              ...
              .update({
                ...
                metadata: {
                  ...
                  fulfillment_provider:
                    fulfillmentMode === 'automatic' ? catalogProvider : 'manual',
        ...
        if (orderInsertErr || !order) {
          console.warn(
            '[Checkout] Failed to create pending merchandise order:',
            orderInsertErr?.message
          );
          return res.status(500).json({
            success: false,
            error: {
              code: 'ORDER_RECORD_FAILED',
              message: 'Could not initialize order. Please try again.',
            },
          });
        }
        ...
        return res.status(400).json({
          ...
          error: {
            code: 'INVALID_TYPE',
            message: 'type must be diamonds, subscription, or merchandise',
          },
        });
      }
      ...
      try {
        ...
        const stripeRequestOptions = checkoutRequestId ? { idempotencyKey: checkoutRequestId } : {};
        stripeRequestOptions.idempotencyKey =
          type === 'subscription'
            ? `commerce:vip-subscription:${user.id}:${checkoutRequestId}`
            : checkoutRequestId
              ? `commerce:${type}:${user.id}:${checkoutRequestId}`
              : undefined;
        ...
        if (type === 'vip_lifetime' && sessionConfig.metadata.purchase_id) {
          if (isAmbiguousStripeCreateFailure(sessionError)) {
            ...
            if (cleanupError) {
              console.error(
                '[Checkout] Pending Lifetime VIP cleanup failed:',
                cleanupError.message
              );
            } else if (!cleanedRows?.length) {
              console.info(
                '[Checkout] Pending Lifetime VIP purchase was already linked or terminal'
              );
            }
          }
        }
      ...
      if (type === 'subscription' && subscriptionClaim) {
        ...
        if (completionError || completed !== true) {
          try {
            ...
            const recoveryError =
              completionError || new Error('Subscription checkout claim could not be finalized');
            recoveryError.checkoutRetryable = true;
            recoveryError.preserveSubscriptionClaim = true;
            throw recoveryError;
          }
