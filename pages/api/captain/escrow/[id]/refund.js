/**
 * Escrow Refund API
 * POST /api/captain/escrow/[id]/refund - Refund escrowed funds to player
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: escrow, error: escrowError } = await supabase
      ...
      .select(`
        *,
        captain_home_games:home_game_id (id, host_id, status)
      `)
      .eq('id', id)
      .single();
    ...
    const isHost = escrow.captain_home_games?.host_id === user.id;
    const isPlayer = escrow.player_id === user.id;
    const gameStatus = escrow.captain_home_games?.status;
