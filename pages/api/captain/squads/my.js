/**
 * My Squads API
 * GET /api/captain/squads/my - Get player's squads and invitations
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    memberships?.forEach(m => {
      if (!m.captain_squads) return;

      const squad = {
        id: m.captain_squads.id,
        name: m.captain_squads.name,
        game_type: m.captain_squads.game_type,
        stakes: m.captain_squads.stakes,
        squad_status: m.captain_squads.status,
        prefer_same_table: m.captain_squads.prefer_same_table,
        venue: m.captain_squads.poker_venues,
        leader: m.captain_squads.profiles,
        member_role: m.role,
