/**
 * Join Squad API
 * POST /api/captain/squads/:id/join
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: squad, error: squadError } = await supabase
      ...
      .select(`
        *,
        captain_waitlist_group_members (id, player_id)
      `)
      .eq('id', id)
      .single();
    ...
    const memberCount = squad.captain_waitlist_group_members?.length || 0;
    ...
    const alreadyMember = squad.captain_waitlist_group_members?.some(
