/**
 * Captain Services API - GET/POST /api/captain/services
...
async function handleGet(req, res) {
  try {
    ...
    if (error) {
      console.error('Captain services query error:', error);
    ...
    console.error('Captain services GET error:', error);
    return res.status(500).json({
      success: false,
...
async function handlePost(req, res) {
  try {
    ...
    if (error) {
      console.error('Captain service create error:', error);
      return res.status(500).json({
        success: false,
    ...
    console.error('Captain services POST error:', error);
    return res.status(500).json({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: 'Internal server error' }
    });
