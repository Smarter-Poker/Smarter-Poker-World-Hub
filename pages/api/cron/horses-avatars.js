...
const ATTIRE = {
    male: ['dark gray hoodie', 'black t-shirt', 'navy blue sweater', 'white polo shirt', 'charcoal hoodie', 'olive green hoodie'],
    female: ['gray hoodie', 'burgundy hoodie', 'black t-shirt', 'lavender sweater', 'casual white top', 'olive hoodie']
};

const ACCESSORIES = ['', 'dark sunglasses', 'black baseball cap', 'clear-frame glasses'];
const EXPRESSIONS = ['confident expression', 'friendly smile', 'poker face', 'slight smirk', 'focused look'];
const FEMALE_NAMES = ['Sarah', 'Jennifer', 'Amanda', 'Rachel', 'Emily', 'Ashley', 'Jessica', 'Olivia', 'Sophia', 'Megan', 'Lauren', 'Christina', 'Angela', 'Diana', 'Monica', 'Stephanie', 'Rebecca', 'Nicole', 'Vanessa', 'Maria', 'Lisa'];
...
async function generateAndUploadAvatar(horse) {
    ...
    try {
        ...
        const { error: authorError } = await supabase
            ...
            .eq('id', horse.id);
        if (authorError) console.error(`   content_authors update failed: ${authorError.message}`);
        if (horse.profile_id) {
            const { error: profileError } = await supabase
                .from('profiles')
                .update({ avatar_url: publicUrl })
                .eq('id', horse.profile_id);
            if (profileError) console.error(`   profiles update failed: ${profileError.message}`);
        }
        console.log(`   ✅ Avatar saved: ${publicUrl}`);
        return { success: true, horse_id: horse.id, horse_name: horse.name, avatar_url: publicUrl };
    } catch (error) {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!horses?.length) {
            return res.status(200).json({ success: true, message: 'All horses already have avatars!', generated: 0 });
        }
