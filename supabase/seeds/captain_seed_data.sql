-- =====================================================
-- SMARTER CAPTAIN - SEED DATA FOR TESTING
-- =====================================================
...
ON CONFLICT (id) DO UPDATE SET
  ...
  captain_tier = 'pro',
  captain_activated_at = COALESCE(poker_venues.captain_activated_at, now()),
...
INSERT INTO captain_tables (id, venue_id, table_number, table_name, max_seats, status, features) VALUES
...
BEGIN
  -- Get table IDs
  SELECT id INTO t1_id FROM captain_tables WHERE venue_id = 1 AND table_number = 1 LIMIT 1;
  SELECT id INTO t2_id FROM captain_tables WHERE venue_id = 1 AND table_number = 2 LIMIT 1;
  SELECT id INTO t3_id FROM captain_tables WHERE venue_id = 1 AND table_number = 3 LIMIT 1;
  SELECT id INTO t4_id FROM captain_tables WHERE venue_id = 1 AND table_number = 7 LIMIT 1;

  -- Create games
  INSERT INTO captain_games (id, venue_id, table_id, game_type, stakes, min_buyin, max_buyin, current_players, max_players, status, started_at)
  ...
  UPDATE captain_tables SET current_game_id = g1_id, status = 'in_use' WHERE id = t1_id;
  UPDATE captain_tables SET current_game_id = g2_id, status = 'in_use' WHERE id = t2_id;
  UPDATE captain_tables SET current_game_id = g3_id, status = 'in_use' WHERE id = t3_id;
  UPDATE captain_tables SET current_game_id = g4_id, status = 'in_use' WHERE id = t4_id;

  -- Create seats for Game 1 (9/9 full)
  INSERT INTO captain_seats (game_id, seat_number, player_name, status, buyin_amount, seated_at) VALUES
  ...
  INSERT INTO captain_waitlist (venue_id, game_type, stakes, player_name, player_phone, position, signup_method, status, created_at, estimated_wait_minutes) VALUES
...
BEGIN
  ...
  IF profile_count > 0 THEN
    ...
    INSERT INTO captain_staff (venue_id, user_id, role, permissions, pin_code, is_active)
    SELECT
      1,
      id,
      'manager',
...
INSERT INTO captain_tournaments (
  venue_id,
  name,
  description,
  tournament_type,
...
INSERT INTO captain_promotions (
  venue_id,
  name,
  description,
  promotion_type,
...
-- SELECT * FROM poker_venues WHERE captain_enabled = true;
-- SELECT * FROM captain_tables WHERE venue_id = 1;
-- SELECT * FROM captain_games WHERE venue_id = 1 AND status = 'running';
-- SELECT * FROM captain_waitlist WHERE venue_id = 1 AND status = 'waiting';
-- SELECT * FROM captain_tournaments WHERE venue_id = 1;
-- SELECT * FROM captain_promotions WHERE venue_id = 1;
