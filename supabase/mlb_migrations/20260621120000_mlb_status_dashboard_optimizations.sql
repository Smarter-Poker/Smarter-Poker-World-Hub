-- Migration: Optimize get_status_dashboard() RPC and v_model_health
-- 
-- 1. Fix pg_class count queries to use schema-scoped oid ('public.tablename'::regclass).
-- 2. Switch pipeline_runs from global LIMIT 12 to DISTINCT ON (stage) to prevent rare stage truncation.
-- 3. Fix v_model_health is_stale check: only mark stale if we actually have games today.
...
runs AS (
  ...
  FROM (
    SELECT DISTINCT ON (coalesce(stage, step)) id, run_ts, stage, step, status, duration_sec, rows_written, notes
    FROM pipeline_runs ORDER BY coalesce(stage, step), run_ts DESC NULLS LAST LIMIT 12
  ) r
),
alerts AS (
...
SELECT jsonb_build_object(
  ...
  'table_counts', jsonb_build_object(
    'pred_market_output', (SELECT COALESCE(reltuples::bigint, 0) FROM pg_class WHERE oid = 'public.pred_market_output'::regclass),
    'pred_props',         (SELECT COALESCE(reltuples::bigint, 0) FROM pg_class WHERE oid = 'public.pred_props'::regclass),
    'pred_best_bets',     (SELECT COALESCE(reltuples::bigint, 0) FROM pg_class WHERE oid = 'public.pred_best_bets'::regclass),
    'agg_market',         (SELECT COALESCE(reltuples::bigint, 0) FROM pg_class WHERE oid = 'public.agg_market'::regclass),
    'snapshots',          (SELECT COALESCE(reltuples::bigint, 0) FROM pg_class WHERE oid = 'public.snapshots'::regclass)
  ),
