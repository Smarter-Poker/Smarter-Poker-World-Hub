-- get_mlb_team_detail: single-call team detail payload (team + stats + games + matchup + props).
-- This revision LEFT JOINs v_mlb_team_defense_bullpen to populate bullpen ERA/WHIP/K9/xFIP and
-- team OAA/Def/Fld for every team. drs/uzr remain null (engine does not currently populate them).
-- Bet Score math stays in the API (TS); this returns raw inputs only.
