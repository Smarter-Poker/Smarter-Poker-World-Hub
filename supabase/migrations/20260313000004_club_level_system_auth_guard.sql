-- =========================================================================
-- MIGRATION: 50-LEVEL CLUB CAPACITY SYSTEM AUTH GUARD HOTFIX 
-- =========================================================================
-- Purpose: Hardens the RPC against Adversarial Threat Models (Compute
-- Exhaustion / DoS). Since the function is SECURITY DEFINER, it bypasses RLS.
-- This explicit auth.uid() guard prevents random users from spamming the
-- heavy COUNT(*) operations and UPDATE locks.
...
DECLARE
    ...
    L INTEGER;
BEGIN
    -- 0. ADVERSARIAL THREAT GUARD: Authorized Personnel Only
    -- Prevent compute exhaustion DoS by blocking non-managers from triggering heavy recounts
    IF NOT EXISTS (
