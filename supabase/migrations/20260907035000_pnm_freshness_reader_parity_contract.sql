-- ============================================================================
-- Poker Near Me freshness checks must match public reader contracts
-- Migration: 20260907035000_pnm_freshness_reader_parity_contract.sql
-- ============================================================================
-- The prior freshness RPC excluded stale quality but could still count an
-- undated/unsafe tour row or a series parent without source-owned provenance.
-- This append-only replacement aligns tour and series health with their public
-- reader contracts while preserving the private eight-check RPC surface.
...
begin
  ...
  if v_definition not like '%tour_stops_scraped_4d%'
     or v_definition not like '%series_scraped_4d%'
     or v_definition not like '%servable_tours%'
     or v_definition not like '%servable_series%' then
  ...
  servable_tours as materialized (
    select *
     ...
     where data_quality in (
       ...
       and btrim(coalesce(tour_code, '')) <> ''
       and btrim(coalesce(stop_name, '')) <> ''
       and btrim(coalesce(event_name, '')) <> ''
       and left(ltrim(event_name), 1) not in ('@', '{', '[')
       and left(ltrim(stop_name), 1) not in ('@', '{', '[')
       and event_name !~* '(<|>|-->|(^|[[:space:]])(class|href|src|style|charset|content|data-[a-z0-9_-]+)[[:space:]]*=)'
       and stop_name !~* '(<|>|-->|(^|[[:space:]])(class|href|src|style|charset|content|data-[a-z0-9_-]+)[[:space:]]*=)'
       and event_name !~* '&(#[x]?[0-9a-f]+|[a-z][a-z0-9]+)[[:space:]]*$'
       and stop_name !~* '&(#[x]?[0-9a-f]+|[a-z][a-z0-9]+)[[:space:]]*$'
       and (start_date is not null or stop_start_date is not null)
       and (
         stop_start_date is null
         or stop_end_date is null
         or stop_end_date >= stop_start_date
       )
       and (
         start_date is null
         or stop_start_date is null
         or start_date >= stop_start_date
       )
       and (
         start_date is null
         or stop_end_date is null
         or start_date <= stop_end_date
       )
  ),
  servable_series as materialized (
    select *
     ...
     where data_quality in (
       ...
       and scrape_timestamp <= now() + interval '5 minutes'
       and (
         case
       ...
       and (
         (
           ...
           and lower(regexp_replace(
             btrim(coalesce(source, '')), '[[:space:]-]+', '_', 'g'
           )) in (
...
comment on function public.pnm_freshness_invariants() is
  'Service-role PNM freshness checks aligned with public daily, tour, series and live reader contracts.';
...
begin
  ...
  if v_definition not like '%servable_daily%'
     ...
     or v_definition not like '%servable_tours%'
     or v_definition not like '%start_date is not null or stop_start_date is not null%'
     or v_definition not like '%stop_end_date >= stop_start_date%'
     or v_definition not like '%start_date >= stop_start_date%'
     or v_definition not like '%start_date <= stop_end_date%'
     or v_definition not like '%servable_series%'
     or v_definition not like '%scrape_html_hash !~ ''^0{64}$''%'
     or v_definition not like '%scrape_batch_id is not null%'
     or v_definition not like '%data_quality = ''manual_research''%'
     or v_definition not like '%servable_live%'
     or v_definition not like '%community_verified%'
     or v_definition not like '%manual_verified%'
     or v_definition not like '%operator_feed%'
     or v_definition not like '%venue_reported%'
...
-- ROLLBACK:
-- Restore a prior pnm_freshness_invariants() definition only if public reader
-- servability contracts intentionally change.
