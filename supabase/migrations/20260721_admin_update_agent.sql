-- Agent field updates (status/role/credit_limit/commission_rate/player_rakeback
-- _rate) from the SPA were writing directly to `agents`, which is fully RLS-
-- locked from the browser: SELECT policy = own row only (auth.uid()=user_id),
-- writes = service_role only. So a club owner could neither read nor write their
-- agents' rows — the update methods silently no-op'd and returned false success,
-- and the read-before-write (parent-limit check, membership lookup) failed too.
--
-- This SECURITY DEFINER RPC does it all server-side: authorize the caller as the
-- agent's club owner/admin, apply the field updates, sync club_members.role on a
-- role change, enforce the sub-agent parent credit-limit rule, and audit credit
-- changes into credit_assignments. AgentService.{updateAgentStatus, updateAgent
-- Role, setCreditLimit, updateRates} now route through it. Final applied version
-- (supersedes the intermediate admin_update_agent_* migrations from the same day).
DROP FUNCTION IF EXISTS public.fn_admin_update_agent(uuid, text, text, numeric, numeric);
DROP FUNCTION IF EXISTS public.fn_admin_update_agent(uuid, text, text, numeric, numeric, numeric);
