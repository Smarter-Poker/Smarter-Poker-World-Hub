-- Migration to add filters to get_portfolio_stats RPC for MLB Analytics
-- Replaces existing RPC to accept optional p_days and p_market filters
...
BEGIN
    ...
    FROM sim_bets
    WHERE (p_days IS NULL OR as_of_ts >= NOW() - INTERVAL '1 day' * p_days)
    ...
    FROM (
        ...
        WHERE as_of_ts IS NOT NULL
          AND (p_days IS NULL OR as_of_ts >= NOW() - INTERVAL '1 day' * p_days)
          AND (p_market IS NULL OR market = p_market)
