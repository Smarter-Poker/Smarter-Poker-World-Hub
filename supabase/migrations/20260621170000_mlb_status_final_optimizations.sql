-- Migration: 20260621170000_mlb_status_final_optimizations.sql
-- Add missing B-Tree indexes to fix massive sequential scans on the MLB status dashboard.
...
-- Update RPC to eliminate pg_class duplicate relname crash risks (scope to public tables only)
...
tier_dist AS (
  ...
  FROM (
    ...
    END AS g
    FROM pred_best_bets
    WHERE as_of_ts = (SELECT ts FROM bb_latest)   -- latest slate only (matches Best Bets count)
  ) x
),
runs AS (
...
sources AS (
  ...
  FROM (
    ...
    WHERE source IN ('daily_predict','odds_api','mlb_api','daily_ingest','fangraphs','fangraphs_splits','statcast','injuries','weather','umpire_scorecards')
  ) s
)
SELECT jsonb_build_object(
  'server_now', to_char(now(), 'YYYY-MM-DD"T"HH24:MI:SS"Z"'),
  ...
  'table_counts', jsonb_build_object(
    'pred_market_output', (SELECT reltuples::bigint FROM pg_class WHERE relname = 'pred_market_output' AND relnamespace = 'public'::regnamespace AND relkind = 'r'),
    'pred_props',         (SELECT reltuples::bigint FROM pg_class WHERE relname = 'pred_props'         AND relnamespace = 'public'::regnamespace AND relkind = 'r'),
    'pred_best_bets',     (SELECT reltuples::bigint FROM pg_class WHERE relname = 'pred_best_bets'     AND relnamespace = 'public'::regnamespace AND relkind = 'r'),
    'agg_market',         (SELECT reltuples::bigint FROM pg_class WHERE relname = 'agg_market'         AND relnamespace = 'public'::regnamespace AND relkind = 'r'),
    'snapshots',          (SELECT reltuples::bigint FROM pg_class WHERE relname = 'snapshots'          AND relnamespace = 'public'::regnamespace AND relkind = 'r')
  ),
