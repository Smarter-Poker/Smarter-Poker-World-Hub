-- ════════════════════════════════════════════════════════════════════════════════
-- Migration: VIP Paywall Audit - 30 Day Trial (Tier 2.2)
-- Updates handle_new_user trigger to explicitly grant 30-day VIP access
-- Combines the March 24 first_name/last_name split with the hardened profile init
-- ════════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- Extract first/last name natively for March 24 compatibility
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        last_active = NOW(),
        is_online = true,
        -- Preserve existing names if they weren't blank
        full_name = CASE
        ...
        -- Preserve existing balances and VIP status safely
        diamonds = CASE
