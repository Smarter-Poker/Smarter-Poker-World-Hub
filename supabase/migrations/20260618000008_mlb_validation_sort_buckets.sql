-- Fix get_mlb_validation_stats RPC to use float casting and monotonic edge bucket ordering
...
BEGIN
    ...
    WHERE actual_result IS NOT NULL
      AND (cutoff IS NULL OR as_of_ts::timestamp >= cutoff);
    ...
    -- Using ::float explicitly prevents arbitrary-precision numeric values from serializing as strings
    RETURN json_build_object(
        'all', json_build_object(
            'count', all_count,
            'modelBrier', COALESCE(all_model_brier, 0)::float,
            'mktBrier', all_mkt_brier::float
        ),
        'flagged', json_build_object(
            'count', flagged_count,
            'winRate', flagged_win_rate::float,
            'roi', flagged_roi::float,
            'modelBrier', COALESCE(flagged_model_brier, 0)::float,
            'mktBrier', flagged_mkt_brier::float
        ),
        'edgeData', json_build_array(
            json_build_object(
                'edge', '3-5 pts',
                'n', COALESCE(edge_3_5_n, 0),
                'winPct', CASE WHEN edge_3_5_n > 0 THEN ROUND((edge_3_5_wins::numeric / edge_3_5_n) * 100)::float ELSE 0::float END,
                'roi', CASE WHEN edge_3_5_n > 0 THEN ((edge_3_5_profit / edge_3_5_n) * 100)::float ELSE 0::float END
            ),
            json_build_object(
                'edge', '5-7 pts',
                'n', COALESCE(edge_5_7_n, 0),
                'winPct', CASE WHEN edge_5_7_n > 0 THEN ROUND((edge_5_7_wins::numeric / edge_5_7_n) * 100)::float ELSE 0::float END,
                'roi', CASE WHEN edge_5_7_n > 0 THEN ((edge_5_7_profit / edge_5_7_n) * 100)::float ELSE 0::float END
            ),
            json_build_object(
                'edge', '7-10 pts',
                'n', COALESCE(edge_7_10_n, 0),
                'winPct', CASE WHEN edge_7_10_n > 0 THEN ROUND((edge_7_10_wins::numeric / edge_7_10_n) * 100)::float ELSE 0::float END,
                'roi', CASE WHEN edge_7_10_n > 0 THEN ((edge_7_10_profit / edge_7_10_n) * 100)::float ELSE 0::float END
            ),
            json_build_object(
                'edge', '10+ pts',
                'n', COALESCE(edge_10_plus_n, 0),
                'winPct', CASE WHEN edge_10_plus_n > 0 THEN ROUND((edge_10_plus_wins::numeric / edge_10_plus_n) * 100)::float ELSE 0::float END,
                'roi', CASE WHEN edge_10_plus_n > 0 THEN ((edge_10_plus_profit / edge_10_plus_n) * 100)::float ELSE 0::float END
            )
        )
    );
END;
