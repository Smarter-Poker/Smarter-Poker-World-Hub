...
-- 20260914113439, which is exactly the version recorded for it in
...
-- An operator policy and its exact before/after receipt form one transaction.
BEGIN;
...
DECLARE v_hash text;
BEGIN
 SELECT md5(pg_get_functiondef('public.fn_ca_fleet_set_policy(text,uuid,jsonb,uuid,text)'::regprocedure)) INTO v_hash;
 IF v_hash NOT IN ('93b73f047fc0aac6d7945ad317c626da','264052c785ef4e479a23b8240a93b2e1') THEN
  RAISE EXCEPTION 'Fleet policy definition drift: %',v_hash;
 END IF;
 IF md5(pg_get_functiondef('public.fn_ca_fleet_policy_effective(uuid)'::regprocedure)) IS DISTINCT FROM '305a6ebb95d3eb4981afc40130f9ee43' THEN
  RAISE EXCEPTION 'Fleet policy dependency drift: fn_ca_fleet_policy_effective(uuid)';
 END IF;
 IF md5(pg_get_functiondef('public.fn_log_admin_action(uuid,text,text,text,jsonb,jsonb,jsonb,text,text,text)'::regprocedure)) IS DISTINCT FROM '1a48ee19daabe9b8b031a09934f1aaaa' THEN
  RAISE EXCEPTION 'Fleet policy dependency drift: fn_log_admin_action(uuid,text,text,text,jsonb,jsonb,jsonb,text,text,text)';
 END IF;
END $preflight$;
...
begin
  ...
  -- Serialize this exact scope before reading its before-state.
  -- The key also protects a scope whose row does not exist yet.
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    'fleet-policy:' || p_scope || ':' || coalesce(p_scope_id::text,'global'),0));
  ...
  -- A policy and its exact audit receipt commit together or neither does.
  v_audit_id := public.fn_log_admin_action(
  ...
  if v_audit_id is null or not exists (
    select 1 from public.admin_audit_log a
    where a.id=v_audit_id and a.admin_user_id=p_updated_by
      and a.action='fleet.set_policy' and a.target_type='fleet_policy'
      and a.target_id=case when p_scope='global' then 'global'
                          else p_scope||':'||p_scope_id::text end
      and a.before_state is not distinct from case when v_created then null else to_jsonb(v_before) end
      and a.after_state=to_jsonb(v_after)
  ) then
    raise exception 'Fleet policy audit did not return its exact stored receipt';
  end if;
