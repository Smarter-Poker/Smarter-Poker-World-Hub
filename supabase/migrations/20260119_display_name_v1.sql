-- ═══════════════════════════════════════════════════════════════════════════
-- DISPLAY NAME PREFERENCE FIX V1
-- Adds full_name and display_name_preference to social feed RPC
-- Ensures posts display correct name based on user preference
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. UPDATE FEED FUNCTION to include display name fields
...
RETURNS TABLE (
    ...
    author_full_name TEXT,               -- NEW: Full name for display preference
    author_display_name_preference TEXT, -- NEW: User's display preference
    author_avatar TEXT,
...
BEGIN
    ...
    SELECT 
        ...
        udp.full_name::TEXT AS author_full_name,                             -- NEW
        COALESCE(udp.display_name_preference, 'full_name')::TEXT AS author_display_name_preference, -- NEW
