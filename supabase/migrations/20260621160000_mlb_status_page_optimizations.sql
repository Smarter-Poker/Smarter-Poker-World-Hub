-- Add missing index for status page model freshness check
CREATE INDEX IF NOT EXISTS idx_raw_odds_as_of_ts ON public.raw_odds (as_of_ts DESC);

-- Update RPC to track 'daily_ingest' which acts as the canonical scheduled update for lineups/API
...
SELECT jsonb_build_object(
  ...
  'table_counts', jsonb_build_object(
    'pred_market_output', (SELECT reltuples::bigint FROM pg_class WHERE relname = 'pred_market_output'),
    'pred_props',         (SELECT reltuples::bigint FROM pg_class WHERE relname = 'pred_props'),
    'pred_best_bets',     (SELECT reltuples::bigint FROM pg_class WHERE relname = 'pred_best_bets'),
    'agg_market',         (SELECT reltuples::bigint FROM pg_class WHERE relname = 'agg_market'),
    'snapshots',          (SELECT reltuples::bigint FROM pg_class WHERE relname = 'snapshots')
  ),
