...
def process_modal(content, modal_var, component_name, props_list):
    ...
    # For PushPrompt, it includes `!pushSubscribed &&` inside the condition string
    # We will just replace the exact matching part.
    if modal_var == 'PushPrompt':
        replacement = f"            {{showPushPrompt && !pushSubscribed && (\n                <{component_name}\n{props_str}                />\n            )}}"
    else:
    ...
    if modal_content.startswith(f"{{show{modal_var}"):
        match = re.match(r'^\{show[a-zA-Z0-9_]+\s*(?:&&\s*![a-zA-Z0-9_]+)?\s*&&\s*\((.*)$', modal_content, re.DOTALL)
        if match:
...
"""
    os.makedirs('src/components/messenger/modals', exist_ok=True)
    with open(f'src/components/messenger/modals/{component_name}.js', 'w') as f:
        f.write(new_component.strip() + "\n")
        
    print(f"Successfully extracted {component_name}.")
    
    # Add dynamic import
    import_stmt = f"const {component_name} = dynamic(() => import('../../../src/components/messenger/modals/{component_name}'), {{ ssr: false }});\n"
    if import_stmt not in new_content:
        # insert near dynamic imports
        dyn_idx = new_content.find("const BlockUserModal = dynamic")
        if dyn_idx != -1:
...
content, ok1 = process_modal(content, 'PushPrompt', 'PushPromptModal', [
