...
async function run() {
  ...
  const { data, error } = await sb.from('pred_market_output').select('selection, market').eq('market', 'team_total').gte('as_of_ts', '2026-06-25T00:00:00Z').limit(50);
  console.log('team_totals:', Array.from(new Set(data.map(d => d.selection))));
}
run();
