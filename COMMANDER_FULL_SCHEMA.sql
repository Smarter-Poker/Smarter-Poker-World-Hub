...
-- =====================================================
-- CLUB COMMANDER - PHASE 3 DATABASE MIGRATION
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_commander_tournaments_venue ON commander_tournaments(venue_id, status);
CREATE INDEX IF NOT EXISTS idx_commander_tournaments_schedule ON commander_tournaments(scheduled_start);
CREATE INDEX IF NOT EXISTS idx_commander_tournaments_status ON commander_tournaments(status);
CREATE INDEX IF NOT EXISTS idx_commander_tournaments_series ON commander_tournaments(series_id);

-- ===================
-- TABLE 2: commander_tournament_entries
-- ===================

CREATE TABLE IF NOT EXISTS commander_tournament_entries (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  eliminated_by UUID REFERENCES commander_tournament_entries(id),
...
CREATE INDEX IF NOT EXISTS idx_commander_entries_tournament ON commander_tournament_entries(tournament_id, status);
CREATE INDEX IF NOT EXISTS idx_commander_entries_player ON commander_tournament_entries(player_id);
CREATE INDEX IF NOT EXISTS idx_commander_entries_position ON commander_tournament_entries(tournament_id, finish_position);
...
ALTER TABLE commander_tournament_entries ENABLE ROW LEVEL SECURITY;

-- Tournaments: Public can view, staff can manage
CREATE POLICY commander_tournaments_select ON commander_tournaments
  FOR SELECT USING (true);

CREATE POLICY commander_tournaments_insert ON commander_tournaments
  FOR INSERT WITH CHECK (
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );

CREATE POLICY commander_tournaments_update ON commander_tournaments
  FOR UPDATE USING (
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );

CREATE POLICY commander_tournaments_delete ON commander_tournaments
  FOR DELETE USING (
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND role IN ('owner', 'manager') AND is_active = true)
  );

-- Entries: Public can view, players can register self, staff can manage all
CREATE POLICY commander_entries_select ON commander_tournament_entries
  FOR SELECT USING (true);

CREATE POLICY commander_entries_insert ON commander_tournament_entries
  FOR INSERT WITH CHECK (
    player_id = auth.uid() OR
    player_id IS NULL OR
    tournament_id IN (
      SELECT id FROM commander_tournaments WHERE venue_id IN (
        SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true
      )
    )
  );

CREATE POLICY commander_entries_update ON commander_tournament_entries
  FOR UPDATE USING (
    player_id = auth.uid() OR
    tournament_id IN (
      SELECT id FROM commander_tournaments WHERE venue_id IN (
...
CREATE POLICY commander_entries_delete ON commander_tournament_entries
  FOR DELETE USING (
...
DROP TRIGGER IF EXISTS tournament_stats_trigger ON commander_tournament_entries;
CREATE TRIGGER tournament_stats_trigger
AFTER INSERT OR UPDATE OR DELETE ON commander_tournament_entries
...
BEGIN
  ...
  FROM commander_tournaments WHERE id = NEW.tournament_id;
...
DROP TRIGGER IF EXISTS entry_investment_trigger ON commander_tournament_entries;
CREATE TRIGGER entry_investment_trigger
BEFORE INSERT OR UPDATE ON commander_tournament_entries
FOR EACH ROW EXECUTE FUNCTION calculate_entry_investment();
-- =====================================================
-- CLUB COMMANDER - PHASE 4 DATABASE MIGRATION
-- =====================================================
...
-- ===================
-- TABLE 1: commander_home_groups
-- ===================
-- Private poker groups/clubs that can host home games

CREATE TABLE IF NOT EXISTS commander_home_groups (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
...
CREATE INDEX IF NOT EXISTS idx_home_groups_owner ON commander_home_groups(owner_id);
CREATE INDEX IF NOT EXISTS idx_home_groups_location ON commander_home_groups(city, state);
CREATE INDEX IF NOT EXISTS idx_home_groups_invite ON commander_home_groups(invite_code);
CREATE INDEX IF NOT EXISTS idx_home_groups_club_code ON commander_home_groups(club_code);

-- ===================
-- TABLE 2: commander_home_members
-- ===================
-- Membership in home game groups

CREATE TABLE IF NOT EXISTS commander_home_members (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id UUID REFERENCES commander_home_groups(id) ON DELETE CASCADE,
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_home_members_group ON commander_home_members(group_id, status);
CREATE INDEX IF NOT EXISTS idx_home_members_user ON commander_home_members(user_id, status);

-- ===================
-- TABLE 3: commander_home_games
-- ===================
-- Scheduled home game events

CREATE TABLE IF NOT EXISTS commander_home_games (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id UUID REFERENCES commander_home_groups(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_home_games_group ON commander_home_games(group_id, status);
CREATE INDEX IF NOT EXISTS idx_home_games_date ON commander_home_games(scheduled_date);
CREATE INDEX IF NOT EXISTS idx_home_games_host ON commander_home_games(host_id);

-- ===================
-- TABLE 4: commander_home_rsvps
-- ===================
-- RSVPs for home games

CREATE TABLE IF NOT EXISTS commander_home_rsvps (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_home_rsvps_game ON commander_home_rsvps(game_id, response);
CREATE INDEX IF NOT EXISTS idx_home_rsvps_user ON commander_home_rsvps(user_id);

-- ===================
-- TABLE 5: commander_club_announcements
-- ===================
-- Club announcements and messages to members

CREATE TABLE IF NOT EXISTS commander_club_announcements (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id UUID REFERENCES commander_home_groups(id) ON DELETE CASCADE,
  ...
  -- Related content
  related_game_id UUID REFERENCES commander_home_games(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_club_announcements_group ON commander_club_announcements(group_id, status);
CREATE INDEX IF NOT EXISTS idx_club_announcements_scheduled ON commander_club_announcements(scheduled_for) WHERE status = 'scheduled';

-- ===================
-- TABLE 6: commander_push_subscriptions
-- ===================
-- Push notification device tokens per user per club

CREATE TABLE IF NOT EXISTS commander_push_subscriptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  group_id UUID REFERENCES commander_home_groups(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_push_subs_user ON commander_push_subscriptions(user_id, is_active);
CREATE INDEX IF NOT EXISTS idx_push_subs_group ON commander_push_subscriptions(group_id, is_active);

-- ===================
-- TABLE 7: commander_notification_log
-- ===================
-- Log of sent notifications for tracking/debugging

CREATE TABLE IF NOT EXISTS commander_notification_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),

  -- Target
  user_id UUID REFERENCES profiles(id) ON DELETE SET NULL,
  group_id UUID REFERENCES commander_home_groups(id) ON DELETE SET NULL,
  announcement_id UUID REFERENCES commander_club_announcements(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_notification_log_user ON commander_notification_log(user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_notification_log_group ON commander_notification_log(group_id, created_at);
...
ALTER TABLE commander_home_members ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_home_games ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_home_rsvps ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_club_announcements ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_push_subscriptions ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_notification_log ENABLE ROW LEVEL SECURITY;

-- Groups: Public groups visible to all, private to members only
CREATE POLICY home_groups_select ON commander_home_groups
  FOR SELECT USING (
    NOT is_private OR
    owner_id = auth.uid() OR
    id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND status = 'approved')
  );

CREATE POLICY home_groups_insert ON commander_home_groups
  FOR INSERT WITH CHECK (owner_id = auth.uid());

CREATE POLICY home_groups_update ON commander_home_groups
  FOR UPDATE USING (
    owner_id = auth.uid() OR
    id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND role IN ('owner', 'admin') AND status = 'approved')
  );

CREATE POLICY home_groups_delete ON commander_home_groups
  FOR DELETE USING (owner_id = auth.uid());

-- Members: Visible to group members
CREATE POLICY home_members_select ON commander_home_members
  FOR SELECT USING (
    user_id = auth.uid() OR
    group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND status = 'approved')
  );

CREATE POLICY home_members_insert ON commander_home_members
  FOR INSERT WITH CHECK (
    user_id = auth.uid() OR
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid()) OR
    group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND role IN ('owner', 'admin') AND status = 'approved')
  );
...
CREATE POLICY home_games_select ON commander_home_games
  FOR SELECT USING (
    group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND status = 'approved') OR
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid())
  );
...
CREATE POLICY home_games_update ON commander_home_games
  FOR UPDATE USING (
    host_id = auth.uid() OR
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid()) OR
    group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND role IN ('owner', 'admin') AND status = 'approved')
...
CREATE POLICY home_games_delete ON commander_home_games
  FOR DELETE USING (
    host_id = auth.uid() OR
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid())
  );
...
CREATE POLICY home_rsvps_select ON commander_home_rsvps
  FOR SELECT USING (
    user_id = auth.uid() OR
    game_id IN (SELECT id FROM commander_home_games WHERE host_id = auth.uid()) OR
    game_id IN (SELECT id FROM commander_home_games WHERE group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND status = 'approved'))
  );

CREATE POLICY home_rsvps_insert ON commander_home_rsvps
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY home_rsvps_update ON commander_home_rsvps
  FOR UPDATE USING (
    user_id = auth.uid() OR
    game_id IN (SELECT id FROM commander_home_games WHERE host_id = auth.uid())
  );

CREATE POLICY home_rsvps_delete ON commander_home_rsvps
  FOR DELETE USING (user_id = auth.uid());

-- Announcements: Members can view, admins can create/edit
CREATE POLICY club_announcements_select ON commander_club_announcements
  FOR SELECT USING (
    group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND status = 'approved')
  );

CREATE POLICY club_announcements_insert ON commander_club_announcements
  FOR INSERT WITH CHECK (
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid()) OR
    group_id IN (SELECT group_id FROM commander_home_members WHERE user_id = auth.uid() AND role IN ('owner', 'admin') AND status = 'approved')
  );

CREATE POLICY club_announcements_update ON commander_club_announcements
  FOR UPDATE USING (
    author_id = auth.uid() OR
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid())
  );

CREATE POLICY club_announcements_delete ON commander_club_announcements
  FOR DELETE USING (
    author_id = auth.uid() OR
    group_id IN (SELECT id FROM commander_home_groups WHERE owner_id = auth.uid())
  );
...
CREATE POLICY push_subs_select ON commander_push_subscriptions
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY push_subs_insert ON commander_push_subscriptions
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY push_subs_update ON commander_push_subscriptions
  FOR UPDATE USING (user_id = auth.uid());

CREATE POLICY push_subs_delete ON commander_push_subscriptions
  FOR DELETE USING (user_id = auth.uid());

-- Notification log: Users can see their own notifications
CREATE POLICY notification_log_select ON commander_notification_log
  FOR SELECT USING (user_id = auth.uid());
...
DROP TRIGGER IF EXISTS home_member_count_trigger ON commander_home_members;
...
BEGIN
  ...
  SET
    rsvp_yes = (SELECT COUNT(*) FROM commander_home_rsvps WHERE game_id = COALESCE(NEW.game_id, OLD.game_id) AND response = 'yes'),
    rsvp_maybe = (SELECT COUNT(*) FROM commander_home_rsvps WHERE game_id = COALESCE(NEW.game_id, OLD.game_id) AND response = 'maybe'),
    rsvp_no = (SELECT COUNT(*) FROM commander_home_rsvps WHERE game_id = COALESCE(NEW.game_id, OLD.game_id) AND response = 'no'),
    waitlist_count = (SELECT COUNT(*) FROM commander_home_rsvps WHERE game_id = COALESCE(NEW.game_id, OLD.game_id) AND response = 'waitlist'),
    updated_at = now()
  WHERE id = COALESCE(NEW.game_id, OLD.game_id);
...
DROP TRIGGER IF EXISTS home_rsvp_count_trigger ON commander_home_rsvps;
CREATE TRIGGER home_rsvp_count_trigger
AFTER INSERT OR UPDATE OR DELETE ON commander_home_rsvps
...
BEGIN
  ...
  IF NEW.club_code IS NULL THEN
    LOOP
      ...
      -- Check if it exists
      SELECT EXISTS(SELECT 1 FROM commander_home_groups WHERE club_code = new_club_code) INTO code_exists;
...
DROP TRIGGER IF EXISTS home_group_invite_code_trigger ON commander_home_groups;
DROP TRIGGER IF EXISTS home_group_codes_trigger ON commander_home_groups;
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO commander_home_members (group_id, user_id, role, status, joined_at)
...
DROP TRIGGER IF EXISTS home_group_owner_trigger ON commander_home_groups;
...
$$ LANGUAGE plpgsql;
-- =====================================================
-- CLUB COMMANDER - PHASE 5 COMPS SYSTEM
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_comp_rates_venue ON commander_comp_rates(venue_id, is_active);

-- ===================
-- TABLE 2: commander_comp_balances
-- ===================
...
CREATE TABLE IF NOT EXISTS commander_comp_balances (
  ...
  frozen_by UUID REFERENCES commander_staff(id),
...
CREATE INDEX IF NOT EXISTS idx_comp_balances_venue ON commander_comp_balances(venue_id);
CREATE INDEX IF NOT EXISTS idx_comp_balances_player ON commander_comp_balances(player_id);
CREATE INDEX IF NOT EXISTS idx_comp_balances_balance ON commander_comp_balances(venue_id, current_balance DESC);

-- ===================
-- TABLE 3: commander_comp_transactions
-- ===================
...
CREATE TABLE IF NOT EXISTS commander_comp_transactions (
  ...
  rate_id UUID REFERENCES commander_comp_rates(id),
...
CREATE INDEX IF NOT EXISTS idx_comp_transactions_balance ON commander_comp_transactions(balance_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_comp_transactions_player ON commander_comp_transactions(player_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_comp_transactions_venue ON commander_comp_transactions(venue_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_comp_transactions_type ON commander_comp_transactions(transaction_type, created_at DESC);

-- ===================
-- TABLE 4: commander_comp_redemptions
-- ===================
...
CREATE TABLE IF NOT EXISTS commander_comp_redemptions (
  ...
  -- Processing
  processed_by UUID REFERENCES commander_staff(id),
...
CREATE INDEX IF NOT EXISTS idx_comp_redemptions_venue ON commander_comp_redemptions(venue_id, status);
CREATE INDEX IF NOT EXISTS idx_comp_redemptions_player ON commander_comp_redemptions(player_id);
...
ALTER TABLE commander_comp_redemptions ENABLE ROW LEVEL SECURITY;

-- Comp rates: Staff can manage, public can view active
CREATE POLICY comp_rates_select ON commander_comp_rates
  FOR SELECT USING (
    is_active = true OR
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );

CREATE POLICY comp_rates_insert ON commander_comp_rates
  FOR INSERT WITH CHECK (
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND role IN ('owner', 'manager') AND is_active = true)
  );

CREATE POLICY comp_rates_update ON commander_comp_rates
  FOR UPDATE USING (
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND role IN ('owner', 'manager') AND is_active = true)
  );

-- Comp balances: Players see their own, staff see venue balances
CREATE POLICY comp_balances_select ON commander_comp_balances
  FOR SELECT USING (
    player_id = auth.uid() OR
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );
...
CREATE POLICY comp_transactions_select ON commander_comp_transactions
  FOR SELECT USING (
    player_id = auth.uid() OR
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );

CREATE POLICY comp_transactions_insert ON commander_comp_transactions
  FOR INSERT WITH CHECK (
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );

-- Redemptions: Players see their own, staff see venue redemptions
CREATE POLICY comp_redemptions_select ON commander_comp_redemptions
  FOR SELECT USING (
    player_id = auth.uid() OR
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );

CREATE POLICY comp_redemptions_insert ON commander_comp_redemptions
  FOR INSERT WITH CHECK (
    player_id = auth.uid() OR
    venue_id IN (SELECT venue_id FROM commander_staff WHERE user_id = auth.uid() AND is_active = true)
  );
...
BEGIN
  ...
  INSERT INTO commander_comp_balances (venue_id, player_id)
  ...
  FROM commander_comp_balances WHERE id = v_balance_id;
...
DROP TRIGGER IF EXISTS session_comps_trigger ON commander_player_sessions;
...
-- =====================================================
-- CLUB COMMANDER - SOCIAL PAGES MIGRATION
-- =====================================================
...
-- ===================
-- TABLE 1: commander_venue_posts
-- ===================
-- Announcements/posts for venue pages

CREATE TABLE IF NOT EXISTS commander_venue_posts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  venue_id INTEGER REFERENCES poker_venues(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_venue_posts_venue ON commander_venue_posts(venue_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_venue_posts_pinned ON commander_venue_posts(venue_id, is_pinned);

-- ===================
-- TABLE 2: commander_home_posts
-- ===================
...
-- ===================
-- TABLE 3: commander_venue_photos
-- ===================
-- Photo gallery for venues

CREATE TABLE IF NOT EXISTS commander_venue_photos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  venue_id INTEGER REFERENCES poker_venues(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_venue_photos_venue ON commander_venue_photos(venue_id, display_order);

-- ===================
-- TABLE 4: commander_venue_reviews
-- ===================
-- Player reviews for venues

CREATE TABLE IF NOT EXISTS commander_venue_reviews (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  venue_id INTEGER REFERENCES poker_venues(id) ON DELETE CASCADE,
  reviewer_id UUID REFERENCES profiles(id),
...
CREATE INDEX IF NOT EXISTS idx_venue_reviews_venue ON commander_venue_reviews(venue_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_venue_reviews_rating ON commander_venue_reviews(venue_id, overall_rating);

-- ===================
-- TABLE 5: commander_venue_followers
-- ===================
-- Users following venues for updates

CREATE TABLE IF NOT EXISTS commander_venue_followers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  venue_id INTEGER REFERENCES poker_venues(id) ON DELETE CASCADE,
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_venue_followers_venue ON commander_venue_followers(venue_id);
CREATE INDEX IF NOT EXISTS idx_venue_followers_user ON commander_venue_followers(user_id);

-- ===================
-- TABLE 6: commander_post_likes
-- ===================
-- Likes on venue/home posts

CREATE TABLE IF NOT EXISTS commander_post_likes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_post_likes_post ON commander_post_likes(post_id);

-- ===================
-- TABLE 7: commander_post_comments
-- ===================
-- Comments on posts

CREATE TABLE IF NOT EXISTS commander_post_comments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  post_id UUID NOT NULL,
  post_type TEXT NOT NULL CHECK (post_type IN ('venue', 'home')),
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  ...
  -- Reply threading
  parent_comment_id UUID REFERENCES commander_post_comments(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_post_comments_post ON commander_post_comments(post_id, created_at);

-- ===================
-- ENABLE RLS
-- ===================

ALTER TABLE commander_venue_posts ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_home_posts ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_venue_photos ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_venue_reviews ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_venue_followers ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_post_likes ENABLE ROW LEVEL SECURITY;
ALTER TABLE commander_post_comments ENABLE ROW LEVEL SECURITY;

-- Public read policies for venue content
CREATE POLICY "Public read venue posts" ON commander_venue_posts
  FOR SELECT USING (is_published = true);

CREATE POLICY "Public read venue photos" ON commander_venue_photos
  FOR SELECT USING (true);

CREATE POLICY "Public read venue reviews" ON commander_venue_reviews
  FOR SELECT USING (is_published = true);

CREATE POLICY "Public read venue followers count" ON commander_venue_followers
  FOR SELECT USING (true);

CREATE POLICY "Public read comments" ON commander_post_comments
  FOR SELECT USING (true);

-- Home posts visible based on group privacy
CREATE POLICY "Read home posts based on visibility" ON commander_home_posts
  FOR SELECT USING (
    is_published = true AND (
      visible_to = 'public' OR
      EXISTS (
        SELECT 1 FROM commander_home_members
        WHERE group_id = commander_home_posts.group_id
...
CREATE POLICY "Users can like posts" ON commander_post_likes
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can comment" ON commander_post_comments
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can review venues" ON commander_venue_reviews
  FOR INSERT WITH CHECK (auth.uid() = reviewer_id);

CREATE POLICY "Users can follow venues" ON commander_venue_followers
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users delete own likes" ON commander_post_likes
  FOR DELETE USING (auth.uid() = user_id);

CREATE POLICY "Users delete own follows" ON commander_venue_followers
  FOR DELETE USING (auth.uid() = user_id);
...
-- ===================
-- UPDATE commander_home_groups with social fields
-- ===================

ALTER TABLE commander_home_groups ADD COLUMN IF NOT EXISTS cover_photo_url TEXT;
ALTER TABLE commander_home_groups ADD COLUMN IF NOT EXISTS profile_photo_url TEXT;
ALTER TABLE commander_home_groups ADD COLUMN IF NOT EXISTS tagline TEXT;
...
CREATE TRIGGER trigger_venue_follower_count
  AFTER INSERT OR DELETE ON commander_venue_followers
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.post_type = 'venue' THEN
      UPDATE commander_venue_posts SET likes_count = likes_count + 1 WHERE id = NEW.post_id;
    ELSE
      UPDATE commander_home_posts SET likes_count = likes_count + 1 WHERE id = NEW.post_id;
    END IF;
  ELSIF TG_OP = 'DELETE' THEN
    IF OLD.post_type = 'venue' THEN
      UPDATE commander_venue_posts SET likes_count = likes_count - 1 WHERE id = OLD.post_id;
    ELSE
      UPDATE commander_home_posts SET likes_count = likes_count - 1 WHERE id = OLD.post_id;
    END IF;
  END IF;
  RETURN NULL;
END;
...
CREATE TRIGGER trigger_post_likes_count
  AFTER INSERT OR DELETE ON commander_post_likes
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.parent_comment_id IS NULL THEN
    IF NEW.post_type = 'venue' THEN
      UPDATE commander_venue_posts SET comments_count = comments_count + 1 WHERE id = NEW.post_id;
    ELSE
      UPDATE commander_home_posts SET comments_count = comments_count + 1 WHERE id = NEW.post_id;
    END IF;
  ELSIF TG_OP = 'DELETE' AND OLD.parent_comment_id IS NULL THEN
    IF OLD.post_type = 'venue' THEN
      UPDATE commander_venue_posts SET comments_count = comments_count - 1 WHERE id = OLD.post_id;
    ELSE
      UPDATE commander_home_posts SET comments_count = comments_count - 1 WHERE id = OLD.post_id;
    END IF;
  END IF;
  RETURN NULL;
END;
...
CREATE TRIGGER trigger_post_comments_count
  AFTER INSERT OR DELETE ON commander_post_comments
