...
import { ALERT_OWNER_ID, routeOperationalPushRows, routeQueuedOperationalPushes } from '../src/lib/push/operational-push-routing.mjs';
...
function fixture(count = 1, recipientId = 'recipient') {
    const now = Date.now();
    const rows = Array.from({ length: count }, (_, i) => ({
        id: `outbox-${i}`, recipient_user_id: recipientId, event: 'accounting_invoice',
    ...
    const state = { rows, prefs: [], legacy: [], runs: [], calls: [], exactCounts: 0,
        subscriptions: [{ id: 'device-1', user_id: recipientId, is_active: true, endpoint: 'fixture-device-1', transport: 'webpush' }],
    ...
    const cron = load('../pages/api/cron/push-dispatch.js', {
        ...
        '../../../src/lib/push/operational-push-routing.mjs': {
            routeOperationalPushRows: (client, rows) => routeOperationalPushRows(client, rows, () => {
                throw new Error('A linked accounting receipt must not enter the operational inbox');
            }),
            routeQueuedOperationalPushes,
        },
...
for (const recipient of [ALERT_OWNER_ID, '00000000-0000-4000-8000-000000000099']) {
    test(`restored operational routing preserves a linked accounting receipt for ${recipient}`, async () => {
        const f = fixture(1, recipient); f.pending();
        const response = await f.run();
        assert.equal(response.code, 200); assert.equal(response.body.sent, 1);
        assert.equal(response.body.digested, 0); assert.equal(f.state.calls.length, 1);
        assert.equal(f.rows[0].status, 'sent');
        assert.equal(f.state.calls[0].payload.data.accountingNotificationId, f.rows[0].accounting_notification_id);
        f.state.slotClaimed = false;
        await f.run();
        assert.equal(f.state.calls.length, 1, 'a completed accounting receipt must not be sent again');
    });
}
