...
test('the engine is generated by the build that actually deploys', () => {
    ...
    // Followed through the indirection, not matched on a name. Both commands
    // call ONE script now, because chaining both copies into buildCommand took
    // it past Vercel's 256-character limit and errored two production deploys.
    const runner = read('scripts/copy-reader-assets.mjs');
    assert.match(runner, /copy-tesseract-assets\.mjs/, 'the runner must actually copy the engine');

    const pkg = JSON.parse(read('package.json'));
    assert.match(pkg.scripts.prebuild, /copy-reader-assets\.mjs/, 'a local build must generate it');

    const vercel = JSON.parse(read('vercel.json'));
    assert.ok(vercel.buildCommand, 'vercel.json sets its own buildCommand');
    assert.match(vercel.buildCommand, /copy-reader-assets\.mjs/, 'and the deployed build must generate it too');
