 ...
 * defects in the server-side JWT fast path:
 *
 ...
 *
 * A test named `server-auth-asymmetric.test.mjs` already existed and ran on
 * every build. It was green throughout. It did not catch this because:
 *   - it imported src/lib/serverAuth.js, NOT the vendored module that 635
 *     files actually use;
 *   - its "token" was the string `'signed-token.'.repeat(4)`, not a JWT;
 *   - it stubbed supabase.auth.getClaims with a mock, so no real signature
 *     verification ever executed.
 *
 * It asserted the plumbing around a fake. THIS file asserts the real thing:
 * genuine ES256 keys, genuine signatures, genuine crypto.subtle, and the
 * genuine vendored modules — with no environment variables set, because
 * that is how production runs.
 *
...
test('decodeSupabaseJWT verifies a real token with no environment configured', async () => {
  ...
  try {
    ...
    assert.ok(
      user,
      'decodeSupabaseJWT returned null for a valid ES256 token. The 635 files importing this module are all falling through to GoTrue.',
    );
    assert.equal(user.id, 'user-123');
  } finally {
