...
test('Preflop Charts has no paid entry gate or signed-in launch reward promise', () => {
  ...
  assert.match(PAGE, /stats:\s*\[\{ label: 'FORMAT', value: '5 Local Rounds' \}[\s\S]*?\{ label: 'SETTLEMENT', value: 'No Account Changes' \}\]/);
});
...
test('authenticated economy and entitlement failures fail closed', () => {
  ...
  assert.match(PAGE, /if \(!userId && getAccessToken\(\)\) \{[\s\S]*?throw new Error\('Authenticated identity is unresolved'\)/);
  assert.match(PAGE, /const proof = await getHeaderStats\(\{ userId, force: true \}\)/);
  assert.match(PAGE, /if \(!proof\?\.success \|\| !proof\.profile \|\| !Number\.isFinite\(Number\(proof\.profile\.diamonds\)\)\) \{[\s\S]*?throw new Error\('Authenticated economy proof unavailable'\)/);
  assert.doesNotMatch(PAGE, /DiamondEngine\.init\(null\)/);
  assert.match(PAGE, /catch \(e\) \{[\s\S]*?setDiamondBalance\(null\);[\s\S]*?setIsVIP\(null\);[\s\S]*?setEconomyReady\(false\)/);
  assert.match(PAGE, /DiamondEngine=\{economyReady \? DiamondEngine : null\}/);
  assert.match(PAGE, /if \(isVIP !== true\) \{[\s\S]*?AI Mode Is Unavailable Until VIP Access Is Verified/);
});

test('the retired Smart Practice state setter cannot crash a successful launch', () => {
  assert.doesNotMatch(PAGE, /setCoachAnalysis\s*\(/);
  assert.match(PAGE, /if \(result\.success && result\.scenario\) \{[\s\S]*?setCurrentScenario\(result\.scenario\);[\s\S]*?setMode\('game'\)/);
});
...
test('Local VS is deterministic session practice, never real ranked matchmaking', () => {
  ...
  assert.match(LOCAL_VS, /Array\.from\([\s\S]*?TOURNAMENT_CHALLENGES\[\(offset \+ index\) % TOURNAMENT_CHALLENGES\.length\]/);
  assert.match(LOCAL_VS, /LOCAL VS PRACTICE/);
  assert.match(LOCAL_VS, /No Player Search Is Running\. This Browser Is Loading A Scripted Practice Coach\./);
  assert.match(LOCAL_VS, /No Live Opponent, Matchmaking, Account Rank, Leaderboard, Or Diamond Settlement/);
  assert.match(LOCAL_VS, /Scores Exist Only In This Run/);
  assert.doesNotMatch(LOCAL_VS, /localStorage|content_authors|from\(['"]horses['"]\)|calculateElo|getRankTier|playerElo|opponent\.elo|startMatchmaking/);
...
test('the prior daily-practice honesty contract remains intact', () => {
  assert.match(PAGE, /This Unsigned Drill Does Not Record Challenge Completion, Streaks, Rankings, Or Diamond Rewards\./);
  assert.match(PAGE, /This Launch Does Not Record Completion, Extend A Streak, Change Rankings, Or Award Diamonds\./);
});
