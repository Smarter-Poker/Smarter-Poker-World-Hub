...
test('invoice tab is a separate explicit selection',()=>{let folder='messages';const tree=widget({open:true,clubId:'a',onFolder:v=>folder=v});elements(tree).find(e=>e.props.role==='tab'&&React.Children.toArray(e.props.children).includes('Invoices')).props.onClick();assert.equal(folder,'invoices');const html=renderToStaticMarkup(widget({open:true,clubId:'a',folder}));assert.match(html,/Weekly Club Statements And Invoice Discussions/);});
...
test('the Club Arena toggle exits without a redundant back button',()=>{let exited=false;const tree=widget({open:true,clubId:'a',onExit:()=>exited=true});elements(tree).find(e=>e.props['aria-expanded']===true).props.onClick();assert.equal(exited,true);assert.doesNotMatch(renderToStaticMarkup(tree),/Back To Social Messages/);});
test('club tabs show independent unread counts and hide zero badges',()=>{const html=renderToStaticMarkup(widget({open:true,clubId:'a',unreadCounts:{messages:2,invoices:0},clubUnread:{a:{messages:2,invoices:0}}}));assert.match(html,/2 Unread/);assert.doesNotMatch(html,/0 Unread/);assert.match(html,/First Club \(2 Unread\)/);});
