...
test('Printful readiness requires a token and the deliberate auto-confirm switch', () => {
    ...
    assert.equal(printful.isPrintfulReady({
...
test('variant mappings override item mappings', () => {
    ...
    assert.deepEqual(
        printful.resolvePrintfulMapping({ external_variant_id: 'tee-black-m' }, null),
...
test('order item builder refuses an unmapped physical line', () => {
    assert.throws(
        () => printful.buildPrintfulItems([{ quantity: 1 }]),
...
test('checkout, webhooks, catalog, and Diamond restrictions stay wired together', async () => {
    const [checkout, stripeWebhook, printfulWebhook, catalog, diamondPurchase, merchStore] = await Promise.all([
    ...
    assert.match(diamondPurchase, /CARD_CHECKOUT_REQUIRED/);
    assert.match(merchStore, /Card Checkout Required For Shipping/);
