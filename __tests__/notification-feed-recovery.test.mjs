...
test('list reports an authoritative personal-view reader failure as unavailable, not an empty success', async () => {
  const { state, request } = await fixture('pages/api/notifications/list.js');
  state.failedTable = 'personal_notifications';
  const result = await request();
  assert.equal(result.statusCode, 503);
  assert.equal(result.payload.success, false);
  assert.equal(result.payload.error, 'Notifications Unavailable');
  assert.equal(result.payload.notifications, undefined);
});
