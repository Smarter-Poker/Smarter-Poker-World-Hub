...
test('mobile Range Lab has a sticky editing command strip with recoverable clear', () => {
  ...
  assert.match(
    CSS,
    /\.preflop-lab-command-strip\s*\{[\s\S]*position:\s*sticky;[\s\S]*top:\s*var\(--sp-header-height/
  );
  assert.match(
    CSS,
    /@media \(min-width: 769px\)[\s\S]*\.preflop-lab-command-strip\s*\{[\s\S]*position:\s*relative/
  );
});
...
test('matrix focus keeps undo, redo, and action-number shortcuts wired', () => {
  ...
  assert.match(
    PAGE,
    /const found = actions\.find[\s\S]*if \(isTextEntry\) return;[\s\S]*handleActionSelect/
  );
  assert.match(
    PAGE,
    /if \(isInteractiveTarget\) return;[\s\S]*if \(key === 'Enter' \|\| key === ' '\)/
  );
});
...
test('VIP checkout sends the server-owned plan key and is idempotent', () => {
  ...
  assert.match(PAGE, /let checkoutRequestId = null/);
  assert.match(PAGE, /checkoutRequestId = getOrCreateCommerceRequestId\(commerceIntent\)/);
