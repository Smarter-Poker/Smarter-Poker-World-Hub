...
// resolution for Node. It never substitutes a module's contents.
registerHooks({
...
function withSourceExtension(absolutePath) {
...
const load = (relativePath) => import(
...
// ── In-memory Supabase client boundary ─────────────────────────────────────
const SOLVER_CANDIDATE_RPC = 'training_solver_spot_candidates_v1';

function createInMemorySupabase({ catalogRows = [], failCatalogRpc = false } = {}) {
...
// ── The real batch-preload route over that boundary ────────────────────────
...
async function loadRealBatchPreloadRoute(db) {
  ...
  const dependencies = {
    '../../../src/lib/serverAuth': {
      getServerUserWithFallback: async () => ({ user: { id: AUDIT_USER_ID }, error: null }),
    },
...
async function requestAttestationCohort({
  ...
  const handler = await loadRealBatchPreloadRoute(db);
  ...
  const previous = {
    audit: process.env.TRAINING_PHASE6_DELIVERY_EXPECTED_AUDIT_USER_ID,
    secret: process.env.TRAINING_GRADING_RECEIPT_SECRET,
  };
  const serverLog = [];
  const originalWarn = console.warn;
  const originalDebug = console.debug;
  process.env.TRAINING_PHASE6_DELIVERY_EXPECTED_AUDIT_USER_ID = AUDIT_USER_ID;
  // A throwaway per-run signing value; never a deployed credential.
  process.env.TRAINING_GRADING_RECEIPT_SECRET = randomBytes(32).toString('hex');
  ...
  try {
    ...
    for (const [name, value] of [
      ['TRAINING_PHASE6_DELIVERY_EXPECTED_AUDIT_USER_ID', previous.audit],
      ['TRAINING_GRADING_RECEIPT_SECRET', previous.secret],
    ]) {
      if (value === undefined) delete process.env[name];
      else process.env[name] = value;
    }
  }
  const diagnostics = serverLog
    .filter((args) => args[0] === cohortDiagnostics.TRAINING_ATTESTATION_COHORT_LOG_PREFIX)
    .map((args) => JSON.parse(args[1]));
  if (response.statusCode >= 500) {
    // Surface the route's own diagnosis; a bare 500 is useless in CI output.
    console.error('[real-geometry] route failed', JSON.stringify(serverLog.map((args) => (
