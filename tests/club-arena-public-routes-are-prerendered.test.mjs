 ...
 * the static HTML the arena build writes for /help and the four legal
 * documents is served only because next.config.js names those files, ahead of
 * the catch-all rewrite. This pins the list to the routes the arena
 * prerenders (its src/lib/seo.ts PUBLIC_ROUTES minus /legal, which it
 * documents as an exception) and keeps the two original rewrites intact.
 */
