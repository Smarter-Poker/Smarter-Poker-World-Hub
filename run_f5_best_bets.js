...
async function run() {
  ...
  const { data, error } = await sb.from('pred_best_bets').select('market').ilike('market', '%f5%').gte('as_of_ts', '2026-06-25T00:00:00Z');
  console.log('pred_best_bets f5 markets:', Array.from(new Set((data || []).map(d => d.market))));
}
run();
