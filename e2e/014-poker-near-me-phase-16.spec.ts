...
test.describe('Poker Near Me phase 16 controller decomposition', () => {
  ...
  test('legacy deep links normalize through the shared route controller', async ({ page }) => {
    ...
    await expect(navigation.getByRole('button', { name: /Events/i })).toHaveAttribute('aria-current', 'true');
  ...
  test('390x844 anchor navigation remains route-synchronized after extraction', async ({ page }) => {
    ...
    const events = navigation.getByRole('button', { name: /Events/i });
    await events.click();
    await expect(events).toHaveAttribute('aria-current', 'true');
    await expect(page).toHaveURL(/\/hub\/poker-near-me\/daily-tournaments(?:\?.*)?$/, {
      timeout: 15_000,
    });
    await expectNoOverflow(page, '390x844 anchor target');
  });
});
