...
> **Current owner instruction (September 17):** Read [AGENTS.md](../../AGENTS.md) before this historical playbook. Authorized tasks independently complete protected submission, required checks, publication, and live verification. The owner removed the human-only `revert-approved` requirement globally; no manual label or extra approval is required. Historical-revert findings are advisory and remain visible. Use the original GitHub/Vercel/Hetzner paths and preserve every unrelated automatic check. Retired local/custom pipelines, automatic merge helpers, watchdogs, repair loops, and external error telemetry remain inactive.
