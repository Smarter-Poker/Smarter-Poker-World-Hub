...
async function run() {
  ...
  const { data, error } = await sb.from('pred_market_output').select('market').gte('as_of_ts', '2026-06-25T00:00:00Z');
  console.log('Unique markets:', Array.from(new Set(data.map(d => d.market))));
}
run();
