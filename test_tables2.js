...
(async () => {
  const { data: tables, error } = await supabase.from('commander_tables').select('*, commander_games(*)').eq('venue_id', 292499).order('table_number');
  if (error) { console.error(error); return; }
  if (!tables || tables.length === 0) { console.log('No tables found for 292499'); return; }
