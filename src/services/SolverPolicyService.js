/**
 * One server-side gateway for solver, chart, curated, heuristic, and missing
 * policy answers. Runtime consumers must not read or interpret the warehouse
...
function boardMatchesRecord(record, board) {
  ...
  const recorded = boardStringVariants(parseBoardFromHash(record?.metadata?.scenario_hash));
...
function flopMatchesRecord(record, board) {
  ...
  const recorded = parseBoardFromHash(record?.metadata?.scenario_hash).slice(0, 3);
...
function keyFromRow(row, overrides = {}) {
  ...
  return createSolverPolicyKey({
    ...
    bounties: [],
    street: row.street,
    board: parseBoardFromHash(row.scenario_hash),
    holding: [],
