 ...
 * Centralized state management for Club Commander
  ...
  persist(
    (set, get) => ({
      ...
      fetchGames: async (venueId) => {
        ...
        try {
          const res = await fetch(`/api/commander/games/venue/${venueId}`);
      ...
      fetchWaitlist: async (venueId) => {
        ...
        try {
          const res = await fetch(`/api/commander/waitlist/venue/${venueId}`);
          const data = await res.json();
          if (data.success) {
...
export const usePlayerCommanderStore = create(
  persist(
    (set, get) => ({
...
export default useCommanderStore;
