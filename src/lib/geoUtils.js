// src/lib/geoUtils.js
// ═══════════════════════════════════════════════
// GLOBAL GEO-LOCATION RESOLVER & CALCULATION ENGINE
// Resolves database string variations (trailing spaces, informal aliases) 
// into absolute GPS precision markers to prevent map-rendering overlap misses.
// ═══════════════════════════════════════════════
...
/**
 * Computes haversine distance between two coordinates in miles.
 */
...
/**
 * Extracts normalized geographic coordinates from dirty venue strings/stops
 * by checking against a known registry of all venue DB entries (allVenues) 
 * or defaulting to major cities.
 */
export function resolveEntityCoordinates(entity, allVenues = []) {
    if (!entity) return null;
    
    const venueName = (entity.venue || entity.name || '').toLowerCase().trim();
    const location = (entity.location || '').toLowerCase().trim();
    const city = (entity.city || '').toLowerCase().trim();
    const state = (entity.state || '').toLowerCase().trim();
