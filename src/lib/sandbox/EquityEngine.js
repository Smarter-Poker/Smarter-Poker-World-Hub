 ...
 */
export function calculateEquity(heroCards, boardCards = [], simulations = 2000, villainRange = null) {
    ...
    for (let i = 0; i < actualSims; i++) {
        ...
        if (useRange) {
            const combo = combos[(Math.random() * combos.length) | 0];
...
function runoutCacheKey(heroCards, boardCards, trials, villainRange) {
    const rangeKey = Array.isArray(villainRange) ? villainRange.join(',') : (villainRange || '');
    return `${heroCards.join('')}|${boardCards.join('')}|${trials}|${rangeKey}`;
}
 ...
 */
export function simulateRunouts(heroCards, boardCards, trials = 300, villainRange = null) {
    ...
    const key = runoutCacheKey(heroCards, boardCards, trials, villainRange);
    ...
    const baseline = calculateEquity(heroCards, boardCards, trials, villainRange).heroEquity;
    ...
    const cardEquities = possibleCards.map(card => {
        ...
        const eq = calculateEquity(heroCards, newBoard, trials, villainRange);
