...
export function formatPokerCards(hand = [], board = []) {
  const handTokens = hand.map(pokerCardToken).filter(Boolean).join('');
  const boardTokens = board.map(pokerCardToken).filter(Boolean).join('');
...
export function parsePokerCards(text = '') {
  ...
  return { hand: cardsFrom(handText), board: cardsFrom(boardText) };
}
