/**
 * Real-time updates hook for Captain dashboard
...
export function useRealtimeUpdates(venueId, onUpdate, enabled = true) {
  ...
  useEffect(() => {
    ...
    const channel = client.channel(`captain:venue:${venueId}`);
    ...
    channel.subscribe((status) => {
      console.log('Captain real-time subscription status:', status);
    });
...
export function useTournamentClock(tournamentId, onTick, enabled = true) {
  ...
  useEffect(() => {
    ...
    const channel = client.channel(`captain:tournament:${tournamentId}`);
