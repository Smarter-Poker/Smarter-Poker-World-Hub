...
function confidenceMult(
  ...
  if (ev > 20) m *= 0.8;
  if (lineupLocked && !vol && pWin != null && pMarket != null && Math.abs(pWin - pMarket) <= 0.06) m *= 1.04;
...
export function betScore(
  ...
  const m = confidenceMult(ev, american, vol, lineupLocked, pWin, pMarket);
...
export function explain(
  ...
  else factors.push({ dir: "down", text: `Negative expected value: ${ev.toFixed(1)}% — the price is worse than the model's real win chance, so it loses money long-run.` });
  factors.push({ dir: "info", text: `Model gives this a ${wc.toFixed(0)}% chance to hit.` });
  ...
  if (pWin != null && pMarket != null) {
    const d = Math.abs(pWin - pMarket) * 100;
    if (d > 15) factors.push({ dir: "down", text: `Model is far above the market (${d.toFixed(0)} pts) — when they disagree this much, the sharp market is usually closer to right.` });
    else if (d > 10) factors.push({ dir: "down", text: `Model is well above the market (${d.toFixed(0)} pts) — some disagreement risk.` });
    else if (d <= 5) factors.push({ dir: "up", text: "Model roughly agrees with the market — you're capturing price value, not betting on a wild disagreement." });
  }
  if (ev > 20) factors.push({ dir: "down", text: "Edge is implausibly large (>20%) — usually a stale line or data quirk, so the score is discounted." });
