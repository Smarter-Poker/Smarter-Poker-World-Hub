...
export async function signupUser(args) {
    ...
    if (!args || typeof args !== 'object') {
        return _err('invalid_email', 'Signup request was malformed.', { args });
    }
    ...
    if (!email || !email.includes('@')) {
        return _err('invalid_email', 'Please enter a valid email address.', { email });
    }
    if (!password || password.length < 10) {
        return _err('weak_password', 'Password must be at least 10 characters.', null);
    }
    ...
    try {
        ...
        return _err('unknown', 'Could not reach the signup service. Please try again.', e);
    }
    ...
    if (result.error) {
        ...
        if (m.includes('rate limit') || m.includes('too many')) {
            return _err('rate_limit', 'Too many signup attempts. Please wait a minute.', result.error, ms);
        }
        if (m.includes('password')) {
            return _err('weak_password', 'Please choose a stronger password.', result.error, ms);
        }
        if (m.includes('email')) {
            return _err('invalid_email', 'That email address was rejected.', result.error, ms);
        }
        return _err('unknown', 'Signup failed. Please try again.', result.error, ms);
    }
...
function _err(code, userMessage, raw, ms = 0) {
    return {
        user: null,
        session: null,
        ms,
