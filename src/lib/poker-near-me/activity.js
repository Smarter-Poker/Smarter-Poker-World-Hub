...
export function capturePokerNearMeEvent(event, properties = {}) {
  const safeProperties = {
    ...
    data_revision: safeText(properties.data_revision, 100) || undefined,
    ...
    candidate_count: Number.isFinite(Number(properties.candidate_count)) ? Math.max(0, Math.round(Number(properties.candidate_count))) : undefined,
    snapshot_age_days: Number.isFinite(Number(properties.snapshot_age_days)) ? Math.max(0, Math.round(Number(properties.snapshot_age_days))) : undefined,
    complete: typeof properties.complete === 'boolean' ? properties.complete : undefined,
