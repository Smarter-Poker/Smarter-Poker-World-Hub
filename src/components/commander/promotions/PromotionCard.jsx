...
export default function PromotionCard({
  ...
  if (compact) {
    return (
      <div
        className="cmd-panel p-3 cursor-pointer hover:border-[#22D3EE] transition-colors"
      ...
      {(onEdit || onViewAwards) && (
        <div className="px-4 py-3 flex items-center gap-2 border-t border-[#4A5E78] bg-[#0B1426]">
          {onEdit && (
            <button
              onClick={() => onEdit(promotion)}
              className="cmd-btn cmd-btn-secondary flex-1 py-2 text-sm"
            >
              Edit
            </button>
          )}
          {onViewAwards && (
            <button
              ...
              className="cmd-btn cmd-btn-primary flex-1 py-2 text-sm"
            >
              View Awards
            </button>
          )}
