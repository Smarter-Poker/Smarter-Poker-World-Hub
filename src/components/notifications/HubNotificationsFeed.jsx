/**
 * HUB NOTIFICATIONS FEED
 * ═══════════════════════════════════════════════════════════════════════════
 * The notifications list. Rendered two ways, from ONE copy of this code:
 *
 *   * as the `/hub/notifications` route (pages/hub/notifications.js), and
 *   * directly inside the full-screen popup (GlobalPageOverlay).
 *
 * WHY IT MOVED HERE, 2026-09-02
 * ───────────────────────────────────────────────────────────────────────────
 * Dan: "IT SHOULD CREATE A 'FULL SCREEN POP UP' SO YOU STAY ON THE PAGE YOU
 * WERE ON." The popup was doing that by FRAMING this page in an iframe, which
 * meant tapping the bell booted a second Next.js application on top of the warm
 * one — fresh document, the Next runtime, _app, hydration, a second Supabase
 * client, a second realtime subscription — all serial, all behind a spinner,
 * before the feed request could even start. Club Arena removed exactly that
 * cost on 2026-08-27 and its popup now paints from cache on frame one; this one
 * still showed a spinner. Rendered as a component there is no second boot at
 * all: it is a render, like any other component.
 *
 * WHAT WAS AND WAS NOT CHANGED IN THE MOVE
 * ───────────────────────────────────────────────────────────────────────────
 ...
 * navigation", and `.agent/workflows/social-feed-protection.md` was followed
 * for this change. The move was made with `sed`, not by retyping, so the
 * fetching, the display, the click handlers and the routing are byte-for-byte
 * what they were. Only the import depth changed (`../../src/` -> `../../`).
 *
 * Four deliberate additions, each one required for the un-framed path and none
 * of them touching a protected behaviour:
 *
 *   1. `embedded` — what `isInIframe` used to detect. When the popup renders
 *      this natively there is no iframe to detect, so the caller says so. The
 *      iframe check is still there and still wins, so the framed path (other
 *      surfaces may still frame this URL) behaves exactly as before.
 *   2. `onNotifCleared` — the badge sync. Framed, this posts SP_NOTIF_CLEARED
 *      up to the parent window; rendered natively there IS no parent, so the
 *      count would never reach the header. Both now fire; the postMessage is
 *      untouched and simply no-ops when not framed.
 *   3. SEOHead is skipped when embedded. A popup must not retitle the tab or
 *      rewrite the canonical URL of the page it is covering.
 *   4. The realtime channel name carries an instance id. Two copies of this
 *      component can be alive at once — a player already on /hub/notifications
 *      who taps the bell — and two subscribers sharing one channel topic means
 *      the loser silently stops receiving inserts. This is the same defect that
...
export default function HubNotificationsFeed({ embedded = false, onNotifCleared } = {}) {
    const router = useRouter();
    // Distinguishes two live copies of this component from each other. See
    ...
    // still frames this URL in an iframe. The iframe check is unchanged; it is
    // now an OR rather than the only signal, because a natively rendered popup
    // has no frame to detect.
    const [isInIframe, setIsInIframe] = useState(embedded);
    useEffect(() => {
        try { setIsInIframe(embedded || window.self !== window.top); } catch (_) { setIsInIframe(true); }
    }, [embedded]);

    /**
     * Publish a fresh unread count to whoever is showing the badge.
     *
     * Framed, that is the parent window over postMessage — the original line,
     * untouched below, which simply does nothing when there is no parent.
     * Rendered natively inside the popup there IS no parent, so the count would
     * never reach the header at all; the callback is how it gets there.
     */
    const publishCount = useCallback((count) => {
        try { onNotifCleared?.(count); } catch (_) { /* a bad listener must not break a read */ }
    }, [onNotifCleared]);
    ...
    useEffect(() => {
        if (!user?.id) return;
        const _ch = supabase
            // The instance id keeps two live copies of this component off one
            // channel topic. See note 4 in the header.
    ...
    if (loading) {
        return (
            <PageTransition>
                {/* Not when embedded: a popup must not retitle the tab or
                    rewrite the canonical URL of the page it is covering. */}
                {!isInIframe && <SEOHead title="Notifications" description="Loading notifications..." canonical="/hub/notifications" noindex={true} />}
                <div style={{ minHeight: '100vh', width: '100%', maxWidth: '100vw', overflowX: 'hidden', boxSizing: 'border-box', background: C.bg, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif' }}>
    ...
    return (
        <PageTransition>
            {/* Not when embedded — see the loading branch above. */}
            {!isInIframe && (
                <SEOHead
                    title="Notifications"
