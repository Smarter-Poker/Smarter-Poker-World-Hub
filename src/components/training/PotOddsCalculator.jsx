    ...
    return (
      <div style={{ background: 'rgba(15,23,42,0.6)', borderRadius: 12, padding: 20, border: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ color: '#64748b', fontSize: 12, marginTop: 2 }}>Calculate Pot Odds, Implied Odds, And Required Equity To Call</div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 16, overflowX: 'auto' }}>
          {PRESETS.map(p => (
            <button key={p.name} onClick={() => { setPotSize(p.pot); setBetSize(p.bet); }} style={{
              ...
              color: potSize === p.pot && betSize === p.bet ? '#3b82f6' : '#94a3b8', fontSize: 12, fontWeight: 600,
            }}>{p.label}</button>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          <div style={{ background: 'rgba(0,0,0,0.15)', borderRadius: 8, padding: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ color: '#64748b', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>Pot Size</span>
          ...
          <div style={{ background: 'rgba(0,0,0,0.15)', borderRadius: 8, padding: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ color: '#64748b', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>Bet To Call</span>
        ...
        <div style={{ background: 'rgba(0,0,0,0.1)', borderRadius: 8, padding: 10, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ color: '#64748b', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>Implied Odds (Extra Value On Future Streets)</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, marginBottom: 16 }}>
          {[
            ...
            <div key={i} style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 6, padding: 10, textAlign: 'center' }}>
              <div style={{ color: '#64748b', fontSize: 12, fontWeight: 600, textTransform: 'uppercase' }}>{r.label}</div>
        ...
        <div style={{ background: 'rgba(0,0,0,0.15)', borderRadius: 8, padding: 10, marginBottom: 16 }}>
          <div style={{ color: '#64748b', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', marginBottom: 6 }}>Pot Breakdown</div>
          <div style={{ display: 'flex', gap: 2, height: 24, borderRadius: 6, overflow: 'hidden' }}>
            <div style={{ width: `${(potSize / (calculations.totalPot + betSize)) * 100}%`, background: '#22c55e', opacity: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ color: '#fff', fontSize: 12, fontWeight: 700 }}>Pot: {potSize}</span>
            </div>
            <div style={{ width: `${(betSize / (calculations.totalPot + betSize)) * 100}%`, background: '#ef4444', opacity: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ color: '#fff', fontSize: 12, fontWeight: 700 }}>Bet: {betSize}</span>
            </div>
            <div style={{ width: `${(betSize / (calculations.totalPot + betSize)) * 100}%`, background: '#3b82f6', opacity: 0.5, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ color: '#fff', fontSize: 12, fontWeight: 700 }}>Call: {betSize}</span>
            </div>
          </div>
          <div style={{ color: '#94a3b8', fontSize: 12, marginTop: 4, textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
          {['flop', 'turn'].map(s => (
            <button key={s} onClick={() => setStreet(s)} style={{
              ...
              color: street === s ? '#3b82f6' : '#64748b', fontSize: 12, fontWeight: 600,
        ...
        <div style={{ background: 'rgba(0,0,0,0.1)', borderRadius: 8, padding: 10 }}>
          <div style={{ color: '#64748b', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', marginBottom: 6 }}>Common Draws - Equity Vs Required {calculations.breakEven.toFixed(1)}%</div>
            ...
            {DRAWS.map(draw => {
              ...
              return (
                <div key={draw.name} role="button" tabIndex={0} aria-pressed={selectedDraw === draw.name} onClick={() => setSelectedDraw(selectedDraw === draw.name ? null : draw.name)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedDraw(selectedDraw === draw.name ? null : draw.name); } }} style={{
                  ...
                  <span style={{ color: '#f1f5f9', fontSize: 12, fontWeight: 600, flex: 1 }}>{draw.name}</span>
                  <span style={{ color: '#64748b', fontSize: 12 }}>{draw.outs > 0 ? `${draw.outs} outs` : ''}</span>
                  <span style={{ color: profitable ? '#22c55e' : '#ef4444', fontSize: 12, fontWeight: 800, width: 40, textAlign: 'right' }}>
                    {equity.toFixed(1)}%
                  </span>
                  <span style={{
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8, justifyContent: 'center' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
              ...
              <span style={{ color: '#94a3b8', fontSize: 12 }}>Profitable Call</span>
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#f59e0b' }} />
              <span style={{ color: '#94a3b8', fontSize: 12 }}>Needs Implied Odds</span>
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#ef4444' }} />
              <span style={{ color: '#94a3b8', fontSize: 12 }}>Not Enough Equity</span>
            </span>
          </div>
        </div>
      </div>
