...
export default function LiveSessionToggle({ currentUser }) {
    ...
    const inputStyle = {
        ...
        background: 'rgba(255,255,255,0.05)', color: T.text, fontSize: 14, width: '100%',
        outline: 'none', boxSizing: 'border-box',
    };
    const btnStyle = (bg, color) => ({
        padding: '10px 16px', borderRadius: 8, border: 'none',
        background: bg, color: color || '#fff', fontSize: 13, fontWeight: 700,
                    ...
                    {!session ? (
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                <button onClick={startSession} disabled={loading || !venueName.trim()}
                        ...
                        <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                                <div>
                                    <div style={{ color: T.text, fontSize: 15, fontWeight: 700 }}>{session.venue_name}</div>
                                    <div style={{ color: T.textSec, fontSize: 12 }}>
                                        {session.stakes} {session.game_type} — {elapsed}
                                        {session.status === 'break' && <span style={{ color: T.gold, marginLeft: 6 }}>ON BREAK</span>}
                                    </div>
                                </div>
                                <div style={{
                                    padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 700,
                                    background: session.status === 'break' ? 'rgba(255,215,0,0.15)' : 'rgba(46,204,113,0.15)',
                            ...
                            <div style={{ marginBottom: 12 }}>
                                <div style={{ color: T.textDim, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>
                                    Current Profit
                                </div>
                                <div style={{ display: 'flex', gap: 8 }}>
                                    <input
                                        ...
                                        style={{ ...inputStyle, flex: 1, fontSize: 18, fontWeight: 700, textAlign: 'center',
                                            color: parseInt(profitInput, 10) >= 0 ? T.green : T.red,
                                        }}
                                    />
                                    <button onClick={updateProfit} disabled={loading}
                                        style={{ ...btnStyle('rgba(79,172,254,0.15)', T.accent), flex: 0, padding: '10px 16px',
                            ...
                            {/* Action buttons */}
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={toggleBreak} disabled={loading}
                                    style={btnStyle(session.status === 'break' ? 'rgba(46,204,113,0.15)' : 'rgba(255,215,0,0.15)',
                                        session.status === 'break' ? T.green : T.gold)}>
                                    {session.status === 'break' ? 'Resume' : 'Take Break'}
                                </button>
                                <button onClick={endSession} disabled={loading}
                                    style={{ ...btnStyle('rgba(231,76,60,0.15)', T.red), border: `1px solid ${T.red}44` }}>
                                    End Session
                                </button>
                            </div>
                        </div>
