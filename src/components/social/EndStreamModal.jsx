...
function playSuccessChime() {
  try {
    ...
    setTimeout(() => {
      if (ctx !== globalAudioCtx) ctx.close();
    }, 600);
  } catch (_) {
...
export function EndStreamModal({
  ...
  useEffect(() => {
    if (resolvedThumbUrl || thumbExtractedRef.current || !videoBlob || !user?.id || !streamId)
      return;
    ...
    vidEl.onseeked = async () => {
      ...
      try {
        ...
        console.warn('[EndStreamModal] thumb extract failed:', e);
      }
      URL.revokeObjectURL(blobUrl);
    };
    vidEl.onerror = () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: C.text }}>Stream Ended</h2>
        ...
        {/* Error Message */}
        {error && <div style={{ padding: '0 16px 16px', color: C.red, fontSize: 14 }}>{error}</div>}
