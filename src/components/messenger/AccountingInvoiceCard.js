...
import { cashierInvoiceDisplay, correctionInvoiceDisplay } from '../../lib/accountingMessage.mjs';
...
export default function AccountingInvoiceCard({ meta, content, theme: C }) {
    ...
    return <article aria-label={correction ? 'Correction Record' : cashier ? 'Cashier Receipt' : weekly ? 'Weekly Club Statement' : 'Accounting Invoice'}
        ...
        <div style={{ fontSize: 12, color: C.textSec }}>{correction ? correction.label : cashier ? cashier.label : weekly ? 'Weekly Club Statement' : 'Invoice'}{!correction && meta.invoice_number ? ` · ${meta.invoice_number}` : ''}</div>
        {!cashier && !correction && (lines.period_start || meta.period_start) && <div style={{ fontSize: 12, marginTop: 6, color: C.textSec }}>
            {String(lines.period_start || meta.period_start).slice(0, 10)} To {String(lines.period_end || meta.period_end || '').slice(0, 10)}
        </div>}
        {!correction && meta.status === 'needs_reconciliation' && <p role="status" style={{ fontSize: 12, lineHeight: 1.5 }}>Needs Reconciliation · These Posted Amounts Are Not A Certified Settlement.</p>}
        <dl style={{ margin: '12px 0' }}>
            {rows.map(([label, value]) => <div key={label} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, padding: '7px 0', borderBottom: `1px solid ${C.border}`, fontSize: 13 }}>
                <dt>{label}</dt><dd style={{ margin: 0, fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{correction ? correction.amount : cashier ? cashier.amount : amount(value)}</dd>
            </div>)}
        </dl>
        <div style={{ fontSize: 12, color: C.textSec }}>Chips · {correction ? correction.status : cashier ? cashier.verified ? cashier.label : 'Details Unavailable' : String(meta.status || 'Status Unavailable').replace(/_/g, ' ')}</div>
        ...
        {!cashier && !correction && <details style={{ marginTop: 12 }}><summary style={{ cursor: 'pointer', color: C.blue, fontSize: 13 }}>{meta.preview ? 'View Statement Details' : 'View Invoice Details'}</summary>
