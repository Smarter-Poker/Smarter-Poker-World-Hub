/* ═══════════════════════════════════════════════════════════════════════════
   ...
   DISABLE    POST /api/auth/mfa/disable → allowed straight through when the
                                           browser already holds a valid MFA
                                           session (the 30-day trusted-device
                                           path). Otherwise the server answers
                                           `requiresMfa` and we ask
                                           /api/auth/mfa/send-code to text one.
...
export default function TwoFactorAuthModal({
    ...
    /* Shared handling for a send-a-code response from setup / disable. */
    ...
    const verify2FA = async () => {
        ...
        if (expectedLen ? cleaned.length !== expectedLen : !/^\d{4,8}$/.test(cleaned)) {
            say('error', `Enter the ${expectedLen || ''}${expectedLen ? '-digit ' : ''}code we texted you.`);
            return;
        }
    ...
    const sendDisableCode = async ({ isResend = false } = {}) => {
        try {
            // /disable only VERIFIES. /send-code is the route that texts.
            // Posting {action:'send'} to /disable just returned the "enter a
            // code" 400 forever, so the disable flow could never get a code.
                ...
                {codes.length > 0 && (
                    <div style={{
                        ...
                        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, marginBottom: 12 }}>
                            Save these somewhere safe. Each works once, and they are the only way in if you lose
                            the phone we text — {String(BACKUP_CODE_LEN)} characters each.
                        </p>
