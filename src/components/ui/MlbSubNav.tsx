...
export default function MlbSubNav() {
  ...
  return (
    <div
      className="bg-[#0a0a15] border-b border-[#3d4f5f] px-4 py-3 flex gap-4 overflow-x-auto whitespace-nowrap shadow-[0_4px_20px_rgba(0,0,0,0.5)] [&::-webkit-scrollbar]:hidden"
      ...
      {links.map((l) => {
        ...
        const isActive =
          l.label === 'Slate'
            ...
            : l.label === 'Teams'
              ? router.pathname.startsWith('/hub/MLB-ANALYTICS/teams') ||
                router.pathname.startsWith('/hub/MLB-ANALYTICS/team/')
              : l.label === 'Model Intel'
                ? router.pathname.startsWith('/hub/MLB-ANALYTICS/model-intel') ||
                  router.pathname.startsWith('/hub/MLB-ANALYTICS/accuracy')
                : router.pathname.startsWith(l.href);
        return (
          <Link
            key={l.label}
            href={l.href}
            className={`pb-1 text-[14px] font-extrabold capitalize tracking-widest transition-all touch-manipulation flex-shrink-0 ${
              isActive
