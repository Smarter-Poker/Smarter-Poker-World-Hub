...
export default function WaitlistCard({ entry, onLeave }) {
  ...
  return (
    <div className={`cap-panel cap-corner-lights p-6 ${isCalled ? 'shadow-[0_0_30px_rgba(16,185,129,0.4)]' : ''}`}>
      {/* Corner glow lights */}
      ...
      {/* Seat Ready Alert */}
      {isCalled && (
        <div className="mb-5 cap-inset p-5 text-center" style={{
          borderColor: '#10B981',
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <div>
              <p className="font-extrabold text-[#10B981] text-lg uppercase tracking-wider cap-text-glow">
                YOUR SEAT IS READY!
              </p>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-extrabold text-xl tracking-wide">
              <span className="text-[#22D3EE] cap-text-glow">{waitlist_entry?.game_type?.toUpperCase()}</span>
        ...
        {onLeave && !isCalled && (
          <button onClick={onLeave} className="cap-btn cap-btn-danger py-3 px-4" title="Leave waitlist">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-10">
        <div className="text-center">
          <div className="cap-stat-value">#{position}</div>
          <div className="cap-stat-label">Position</div>
        </div>
        ...
        <div className="text-center">
          <div className="cap-stat-value flex items-center gap-3">
            <Clock className="w-10 h-10 text-[#64748B]" />
            {estimated_wait || '--'}
          </div>
          <div className="cap-stat-label">Est. Minutes</div>
        </div>
      </div>

      {/* Footer */}
      <div className="cap-divider" style={{ margin: '24px 0 16px 0' }} />
