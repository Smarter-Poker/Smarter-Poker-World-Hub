...
function TooltipCard({ step, currentIndex, totalSteps, position, onNext, onSkip, onDontShow, cardRef }) {
  ...
  const mobileStyles = isMobile ? {
    position: 'fixed',
    bottom: 'calc(var(--active-world-footer-height, 70px) + env(safe-area-inset-bottom, 0px) + 8px)',
    left: 0,
    right: 0,
    margin: '0 auto',
    width: 'min(calc(100vw - 16px), 420px)',
    maxHeight: `min(${Math.min(vh * 0.55, 380)}px, calc(100dvh - var(--active-world-footer-height, 70px) - env(safe-area-inset-bottom, 0px) - 24px))`,
    zIndex: 10003,
    pointerEvents: 'auto',
  } : {
    position: 'fixed',
  ...
  return (
    ...
    >
      <div style={{
        ...
        maxHeight: isMobile ? 'inherit' : 'none',
