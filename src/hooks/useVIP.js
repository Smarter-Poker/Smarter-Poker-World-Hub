...
export default function useVIP() {
    ...
    // Persist authoritative answer for optimistic rendering next load. Done in an effect
    // (not during render) so it's a proper side effect, not an impure render write that
    // fires on every re-render and double-runs under React 18 StrictMode.
    useEffect(() => {
        if (!initializing && typeof window !== 'undefined') {
            try {
