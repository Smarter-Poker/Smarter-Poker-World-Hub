...
export default function PressureCookerGame({ level = 1, onExit, onScoreUpdate, DiamondEngine, userId }) {
    ...
    const startGame = useCallback(() => {
        ...
        setScore(0); setHandsCompleted(0); setStreak(0); setMaxStreak(0); setCorrectCount(0); setUserAnswer(null); setLastAnswerCorrect(null);
        mistakesRef.current = [];
    ...
    const nextHand = useCallback(() => {
        const hand = getRandomHand();
        if (!hand) return;
        setCurrentHand(hand); setGameState('playing'); setUserAnswer(null); setLastAnswerCorrect(null); setEliminatedAction(null);
    }, [getRandomHand]);
    ...
    const handleNext = useCallback(() => {
        ...
        if (handsCompleted < handsRequired) {
            nextHand();
            return;
        }
            ...
            {(gameState === 'playing' || gameState === 'revealed') && currentHand && (
                <>
                    ...
                    <CircularTimer
                        ...
                        isLow={isLowTime}
                    >
                        <div style={{ fontSize: 14, fontFamily: "var(--font-rajdhani), 'Rajdhani'", fontWeight: 700, color: timerColor, marginBottom: 4 }}>{timerSec}s</div>
                        <div style={{
                            width: 140, height: 90,
                            background: isLowTime ? 'linear-gradient(145deg, #3a1a1a, #2e1616)' : 'linear-gradient(145deg, #1a1a2e, #16213e)',
                            border: `3px solid ${gameState === 'revealed' ? (lastAnswerCorrect ? '#00ff88' : '#ff4444') : timerColor}`,
                    ...
                    {gameState === 'revealed' && (
                        <button type="button" onClick={handleNext} style={{ width: '100%', minHeight: 52, marginTop: 18, borderRadius: 0, border: '1px solid #9beeff', background: 'linear-gradient(180deg, #23465b, #07121b)', color: '#fff', fontSize: 15, fontWeight: 900, cursor: 'pointer', boxShadow: 'inset 0 1px rgba(255,255,255,0.26), 0 8px 18px rgba(0,0,0,0.38)' }}>
                            {handsCompleted >= handsRequired ? 'View Results →' : 'Next Hand →'}
                        </button>
                    )}
                </>
            )}
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 10 }}>
                                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 12 }}>
                                    <div style={{ fontFamily: "var(--font-rajdhani), 'Rajdhani'", fontSize: 20, fontWeight: 800, color: '#FFD700' }}>{score.toLocaleString()}</div>
                                    <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: 600 }}>{'SCORE'}</div>
                                </div>
                                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 12 }}>
                                    <div style={{ fontFamily: "var(--font-rajdhani), 'Rajdhani'", fontSize: 20, fontWeight: 800, color: '#00ff88' }}>{timerSec}s</div>
                                    <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: 600 }}>TIME LEFT</div>
                                </div>
                                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 12 }}>
                                    <div style={{ fontFamily: "var(--font-rajdhani), 'Rajdhani'", fontSize: 20, fontWeight: 800, color: '#00d4ff' }}>{maxStreak}</div>
                                    <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', fontWeight: 600 }}>BEST STREAK</div>
                                </div>
                                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 12 }}>
                                    <div style={{ fontFamily: "var(--font-rajdhani), 'Rajdhani'", fontSize: 20, fontWeight: 800, color: '#A78BFA' }}>{correctCount}/{handsCompleted}</div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ marginTop: 20 }}>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 12 }}>
                                ...
                                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 14 }}>
                                    <div style={{ fontFamily: "var(--font-rajdhani), 'Rajdhani'", fontSize: 24, fontWeight: 800, color: '#A78BFA' }}>{handsCompleted}/{handsRequired}</div>
