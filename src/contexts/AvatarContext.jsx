...
export function AvatarProvider({ children }) {
    ...
    async function ensureUserProfile(user, session) {
      ...
      try {
        ...
        if (data.created || data.isBrandNew) {
          console.debug(
            '[ANTIGRAVITY] Profile was missing or brand new - checking welcome modal for:',
          ...
          if (!localStorage.getItem(welcomeKey)) {
            ...
            window.dispatchEvent(
              new CustomEvent('vip-status-changed', { detail: { vipGranted: true } })
            );
    ...
    const {
      ...
      if (event === 'INITIAL_SESSION') {
        if (session?.user) {
          ...
          ensureUserProfile(session.user, session).catch((e) =>
            console.warn('[AvatarContext] ensureUserProfile error:', e)
          );
          ...
          // refresh.
          fetchVipStatus(session.user.id).catch((e) =>
            console.warn('[AvatarContext] fetchVipStatus error:', e?.message || e)
          );
          ...
          if (fallbackUser?.id) {
            console.debug(
              '[AvatarContext] INITIAL_SESSION empty but found user in localStorage fallback:',
              fallbackUser.email
            );
  ...
  async function setActiveAvatar(imageUrl, type = 'custom', presetAvatarId = null, prompt = null) {
    ...
    try {
      ...
      // Sync profiles.arena_avatar_url so Club Arena, training games and the
      ...
      if (imageUrl) {
        const { error: profileError } = await supabase
          .from('profiles')
          .update({ arena_avatar_url: imageUrl })
          .eq('id', user.id);
        if (profileError)
          console.warn('Profile arena avatar sync failed (non-fatal):', profileError.message);
      }
