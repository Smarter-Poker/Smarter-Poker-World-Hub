...
async function run() {
  ...
  const { data, error } = await sb.from('pred_market_output').select('market').ilike('market', 'first_5%').gte('as_of_ts', '2026-06-25T00:00:00Z').limit(50);
  console.log('first_5 markets:', Array.from(new Set(data.map(d => d.market))));
}
run();
