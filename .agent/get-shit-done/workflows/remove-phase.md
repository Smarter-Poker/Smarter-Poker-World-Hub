...
```bash
INIT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" init phase-op "${target}")
...
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
