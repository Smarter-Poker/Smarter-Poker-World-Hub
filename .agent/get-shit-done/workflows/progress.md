...
```bash
INIT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" init progress)
...
- `STATE=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" state-snapshot)`
...
PROGRESS_BAR=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
