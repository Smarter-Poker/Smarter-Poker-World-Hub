...
```bash
TRANSITION=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
