...
All API endpoints are located under `/api/club-commander/`. Authentication is handled via Supabase Auth with JWT tokens.

## Base URL

```
Production: https://smarter.poker/api/club-commander
Development: http://localhost:3000/api/club-commander
```
...
|-------|------|-------------|
| `captain_enabled` | boolean | Filter by Club Commander status |
...
| `VENUE_NOT_CLUB_COMMANDER` | Venue not using Club Commander |
