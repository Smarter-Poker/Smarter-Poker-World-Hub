...
/api/club-commander/
├── venues/
│   ├── GET    /                     # List venues with Club Commander enabled
...
channel: `club-commander:venue:${venueId}`
events:
...
channel: `club-commander:tournament:${tournamentId}`
events:
...
channel: `club-commander:user:${userId}`
events:
...
```javascript
// XP earning events from Club Commander activities
const CLUB_COMMANDER_XP_EVENTS = {
  // Club Activities
  'waitlist.joined': 5,
  'game.seated': 25,
...
const CLUB_COMMANDER_DIAMOND_EVENTS = {
  'session.completed': 5,
...
├── [ ] Set up Supabase schema (Club Commander tables)
...
/pages/club-commander/                 # Staff-facing routes
...
/src/stores/
├── clubCommanderStore.js            # Main Club Commander state
...
```typescript
// /src/lib/club-commander/realtime.ts

import { supabase } from '../supabase';

export class ClubCommanderRealtime {
...
export const clubCommanderRealtime = new ClubCommanderRealtime();
```

### 8.3 Notification Service

```typescript
// /src/lib/club-commander/notifications.ts
  ...
  if (payload.channels.includes('sms') && payload.phone) {
    try {
      results.sms = await twilioClient.messages.create({
        ...
        to: payload.phone,
        statusCallback: `${process.env.NEXT_PUBLIC_URL}/api/club-commander/webhooks/twilio`
      });
    } catch (error) {
...
| Metric | Target |
|--------|--------|
| Venues using Club Commander | 50 |
| Monthly active players | 10,000 |
| Home games hosted | 500/month |
| Smarter.Poker signups from Club Commander | 25,000 |
...
Cost of Club Commander development: ~$100,000 (engineering time)
ROI: 145%
```
...
```env
# Club Commander-specific
TWILIO_ACCOUNT_SID=
TWILIO_AUTH_TOKEN=
TWILIO_PHONE_NUMBER=
FCM_SERVER_KEY=
CLUB_COMMANDER_WEBHOOK_SECRET=
