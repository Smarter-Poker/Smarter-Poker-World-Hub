# Club Commander - User Review Checklist
...
```bash
./scripts/validate-club-commander.sh
```
- If ERRORS > 0, reject and fix

### Test 2: Check for Emojis
```bash
grep -r "[\U0001F300-\U0001F9FF]" src/components/club-commander/ pages/club-commander/
```
- If any results, reject

### Test 3: Check for Wrong Colors
```bash
grep -r "purple\|violet\|pink\|orange" src/components/club-commander/
```
- If any results, reject (unless it's error/warning colors)

### Test 4: Check for Unauthorized Files
```bash
git status --short | grep "club-commander"
```
...
### APIs
- [ ] Test: `curl localhost:3000/api/club-commander/venues` - returns JSON
- [ ] Test: `curl localhost:3000/api/club-commander/games/live` - returns JSON
- [ ] Test: POST to /api/club-commander/waitlist/join - works with auth
- [ ] Verify no extra endpoints were created

### UI
- [ ] Visit /club-commander/login - see PIN login screen
- [ ] Visit /club-commander/dashboard - see staff terminal
- [ ] Visit /hub/club-commander - see player waitlist view
...
10. **Changes to existing non-club-commander code**
...
```
Build Club Commander Phase [X].

REQUIREMENTS:
1. Read .agent/skills/club-commander/AGENT_INSTRUCTIONS.md first
