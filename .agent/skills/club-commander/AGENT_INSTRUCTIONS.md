...
```bash
# Check for existing Club Commander tables
psql $DATABASE_URL -c "SELECT table_name FROM information_schema.tables WHERE table_name LIKE 'captain_%';"

# Check for existing Club Commander API routes
find pages/api/club-commander -name "*.js" 2>/dev/null | head -20

# Check for existing Club Commander components
find src/components/club-commander -name "*.jsx" 2>/dev/null | head -20
```
...
```
feat(club-commander): Phase X Step Y.Z - [Description]
