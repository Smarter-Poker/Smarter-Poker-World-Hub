...
-- Venue & Staff
CREATE INDEX idx_commander_staff_venue ON commander_staff(venue_id);
CREATE INDEX idx_commander_staff_user ON commander_staff(user_id);

-- Tables & Games
CREATE INDEX idx_commander_tables_venue ON commander_tables(venue_id);
CREATE INDEX idx_commander_games_venue_status ON commander_games(venue_id, status);
CREATE INDEX idx_commander_games_table ON commander_games(table_id);
...
CREATE INDEX idx_commander_waitlist_game ON commander_waitlist(game_id);
...
CREATE INDEX idx_commander_tournament_entries_tournament ON commander_tournament_entries(tournament_id);
CREATE INDEX idx_commander_tournament_entries_player ON commander_tournament_entries(player_id);
...
CREATE INDEX idx_commander_home_games_host ON commander_home_games(host_id);

-- Notifications
CREATE INDEX idx_commander_notifications_player ON commander_notifications(player_id, status);
CREATE INDEX idx_commander_notifications_venue ON commander_notifications(venue_id);

-- Analytics & AI
CREATE INDEX idx_commander_analytics_venue_date ON commander_analytics_daily(venue_id, date);
CREATE INDEX idx_wait_predictions_venue ON commander_wait_time_predictions(venue_id, game_type, stakes);

-- Hand History
CREATE INDEX idx_commander_hand_history_game ON commander_hand_history(game_id);
CREATE INDEX idx_commander_hand_history_player ON commander_hand_history USING GIN (player_cards);

-- Incidents
CREATE INDEX idx_commander_incidents_venue ON commander_incidents(venue_id, created_at);

-- Escrow
CREATE INDEX idx_commander_escrow_status ON commander_escrow_transactions(status);

-- Responsible Gaming
CREATE INDEX idx_commander_exclusions_player ON commander_self_exclusions(player_id, expires_at);
...
CREATE POLICY session_player_access ON commander_player_sessions
  FOR SELECT USING (player_id = auth.uid());

-- Players can see their own notifications
CREATE POLICY notification_player_access ON commander_notifications
  FOR SELECT USING (player_id = auth.uid());
...
AFTER INSERT OR UPDATE OR DELETE ON commander_waitlist
...
BEGIN
  ...
  SET
    total_entries = (SELECT COUNT(*) FROM commander_tournament_entries WHERE tournament_id = COALESCE(NEW.tournament_id, OLD.tournament_id)),
    players_remaining = (SELECT COUNT(*) FROM commander_tournament_entries WHERE tournament_id = COALESCE(NEW.tournament_id, OLD.tournament_id) AND status IN ('registered', 'seated'))
...
COMMENT ON SCHEMA public IS 'Club Commander Schema v1.0 - January 2026';
