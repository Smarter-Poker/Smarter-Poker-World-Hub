# Smarter Captain - Mandatory Agent Instructions

## READ THIS FIRST - EVERY TIME

You are building Smarter Captain. This is a STRICT implementation - no creativity allowed.
...
```
SMARTER CAPTAIN BUILD CONFIRMATION
==================================
...
```bash
# Check for existing Captain tables
psql $DATABASE_URL -c "SELECT table_name FROM information_schema.tables WHERE table_name LIKE 'captain_%';"

# Check for existing Captain API routes
find pages/api/captain -name "*.js" 2>/dev/null | head -20

# Check for existing Captain components
find src/components/captain -name "*.jsx" 2>/dev/null | head -20
```
...
3. Create migration file: `supabase/migrations/YYYYMMDD_captain_[description].sql`
4. Include indexes and RLS policies from the schema

**WRONG:**
```sql
-- I think we need a status field
ALTER TABLE captain_waitlist ADD COLUMN my_new_status TEXT;
```
...
```
feat(captain): Phase X Step Y.Z - [Description]
