# Smarter Captain - Complete Build Plan
...
**Smarter Captain** is a cloud-based poker room management platform designed to:
...
| TableCaptain Feature | Smarter Captain | Our Improvement |
|---------------------|-----------------|-----------------|
...
```sql
-- =====================================================
-- SMARTER CAPTAIN DATABASE SCHEMA
-- =====================================================
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  SMARTER CAPTAIN - The Lodge Card Club                    Staff: Mike  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌─ ACTIVE GAMES ──────────────────────────────────────────────────┐   │
│  │                                                                  │   │
...
│                                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│            Powered by Smarter Captain | smarter.poker                   │
└─────────────────────────────────────────────────────────────────────────┘
```
...
/api/captain/
├── venues/
│   ├── GET    /                     # List venues with captain enabled
...
channel: `captain:tournament:${tournamentId}`
events:
...
```javascript
// XP earning events from Captain activities
...
├── [ ] Set up Supabase schema (captain tables)
...
/pages/captain/                 # Staff-facing routes
...
/src/stores/
├── captainStore.js            # Main captain state
...
```typescript
// /src/lib/captain/realtime.ts

import { supabase } from '../supabase';

export class CaptainRealtime {
  ...
  subscribeToTournament(tournamentId: string, callbacks: {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'captain_tournament_entries',
...
export const captainRealtime = new CaptainRealtime();
```
  ...
  if (payload.channels.includes('sms') && payload.phone) {
    try {
      results.sms = await twilioClient.messages.create({
        ...
        to: payload.phone,
        statusCallback: `${process.env.NEXT_PUBLIC_URL}/api/captain/webhooks/twilio`
      });
    } catch (error) {
  ...
  if (payload.channels.includes('push') && payload.userId) {
    try {
      results.push = await sendPushNotification(payload.userId, {
        title: payload.title || 'Smarter Captain',
        body: payload.message,
...
| Aspect | TableCaptain | Smarter Captain | Advantage |
|--------|-------------|-----------------|-----------|
...
| Metric | Target |
|--------|--------|
| Venues using Captain | 50 |
| Monthly active players | 10,000 |
| Home games hosted | 500/month |
| Smarter.Poker signups from Captain | 25,000 |
...
Cost of Captain development: ~$100,000 (engineering time)
ROI: 145%
```
