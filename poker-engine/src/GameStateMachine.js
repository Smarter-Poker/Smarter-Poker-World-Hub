...
class GameStateMachine {
  ...
  _onRoundComplete() {
    ...
    if (actionablePlayers.length <= 1) {
      ...
      // nextStreet was already dealt above, so run out from the street AFTER it
      const afterIdx = STREETS.indexOf(nextStreet) + 1;
      if (afterIdx < STREETS.length) {
        this._runOutBoard(STREETS[afterIdx]);
      } else {
        // All streets dealt, go straight to showdown
        this._handleShowdown();
      }
      return;
    }
