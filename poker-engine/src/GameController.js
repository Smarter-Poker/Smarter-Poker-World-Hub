...
class GameController {
  constructor() {
    ...
    this._bridges = new Map(); // ClubGameBridge instances per table
    this._stats = {
      handsDealt: 0,
      peakPlayers: 0,
      totalActions: 0,
  ...
  async createTable(config) {
    ...
    try {
      ...
      // Attach ClubGameBridge for club tables
      if (clubId && this.supabase) {
        const entry = this.lobby.tables.get(tableId);
        if (entry) {
          const bridge = new ClubGameBridge({
            clubId,
            tableId,
            tableManager: entry.table,
          ...
          bridge.attach();
          this._bridges.set(tableId, bridge);
        }
      }
  ...
  async connectToClubTable(clubTableId) {
    ...
    try {
      ...
      // Attach ClubGameBridge for rake & hand history persistence
      ...
      if (entry) {
        const bridge = new ClubGameBridge({
          ...
          supabase: this.supabase,
          rakePercent: row.rake_percent || 0,
          rakeCapBb: row.rake_cap_bb || 0,
          bigBlind: row.big_blind || 2,
        });
        bridge.attach();
        this._bridges.set(clubTableId, bridge);
      }
  ...
  async closeTable(tableId) {
    ...
    try {
      // Detach bridge
      const bridge = this._bridges.get(tableId);
      if (bridge) {
        bridge.detach();
        this._bridges.delete(tableId);
      }
