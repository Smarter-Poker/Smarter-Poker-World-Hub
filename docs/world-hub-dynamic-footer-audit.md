...
The generated [complete route matrix](./world-hub-footer-route-matrix.md) accounts for all 203 applicable physical routes, including nested and parameterized Pages Router entries. Counts are generated from the page tree and registry, and generation fails if the audited total drifts. `/hub/my-clubs` is excluded because it is a server-side redirect and renders no footer document of its own.
