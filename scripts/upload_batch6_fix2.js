...
// Entities to delete
const idsToDelete = [2251, 1954];

// Logos to update
const uploads = [
  { id: 2612, file: 'media__1775373983654.jpg', ext: 'jpg' }, // Ante Up Poker Tour
  { id: 2249, file: 'media__1775373988902.jpg', ext: 'jpg' }, // Ebro Greyhound Park
  { id: 1947, file: 'media__1775373992745.jpg', ext: 'jpg' }, // Ebro Poker Room
  { id: 2238, file: 'media__1775374006023.png', ext: 'png' }  // Jacksonville Poker Room
];
...
let venues1 = JSON.parse(fs.readFileSync(allVenuesPath1, 'utf8'));
let venues2 = JSON.parse(fs.readFileSync(allVenuesPath2, 'utf8'));

async function main() {
  // First, delete the specific venues
  const initialLength = venues1.venues.length;
  venues1.venues = venues1.venues.filter(v => !idsToDelete.includes(v.id));
  venues2.venues = venues2.venues.filter(v => !idsToDelete.includes(v.id));
  console.log(`Deleted ${initialLength - venues1.venues.length} venues.`);
