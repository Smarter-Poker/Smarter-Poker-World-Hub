...
// Batch 18 is 425-450 (venues #426-450 based on new array length after removal)
const startIndex = 425;
const endIndex = Math.min(450, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 18 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 18 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch18-review.html', html);
console.log("Created public/batch18-review.html");
