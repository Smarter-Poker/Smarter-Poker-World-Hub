...
const uploads = [
  { id: 1896, file: 'media__1775381722700.png', ext: 'png' }, // Northern Quest Casino
  { id: 1893, file: 'media__1775381741601.png', ext: 'png' }, // Muckleshoot Casino
  { id: 2666, file: 'media__1775381744187.jpg', ext: 'jpg' }, // Muckleshoot Poker Series
  { id: 2703, file: 'media__1775381751520.jpg', ext: 'jpg' }  // Northwest Poker Championship
];
...
async function main() {
  ...
  // Remove the specified venues
  const removeIds = [3012];
  const removeNames = ["Buzz Inn Steakhouse Smokey Point"];
  
  venues1.venues = venues1.venues.filter(v => !removeIds.includes(v.id) && !removeNames.includes(v.name));
  venues2.venues = venues2.venues.filter(v => !removeIds.includes(v.id) && !removeNames.includes(v.name));
  ...
  console.log("JSON registries updated! Deleted Buzz Inn and Jamestown Saloon.");
}
