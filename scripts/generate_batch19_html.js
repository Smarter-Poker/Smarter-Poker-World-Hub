...
// Batch 19 is 450-475 (venues #451-475 based on new array length after removal)
const startIndex = 450;
const endIndex = Math.min(475, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 19 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 19 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch19-review.html', html);
console.log("Created public/batch19-review.html");
