...
// Batch 23 is 550-575 (venues #551-575 based on new array length after removal)
const startIndex = 550;
const endIndex = Math.min(575, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 23 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 23 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch23-review.html', html);
console.log("Created public/batch23-review.html");
