...
// Batch 28 is 675-700
const startIndex = 675;
const endIndex = Math.min(700, venues.length);
const batch = venues.slice(startIndex, endIndex);

if (batch.length > 0) {
  let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 28 — Venues #${startIndex + 1}-${endIndex}</title>
  <style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <header>
  <h1>Batch 28 — Venues #${startIndex + 1}-${endIndex}</h1>
  ...
  fs.writeFileSync('public/batch28-review.html', html);
  console.log("Created public/batch28-review.html with " + batch.length + " remaining venues.");
} else {
  console.log("No more venues remaining! End of list.");
}
