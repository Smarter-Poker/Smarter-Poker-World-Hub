...
const PREDECESSOR_JTI_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}(?::[01])?$/i;
const ATTEMPT_SCOPED_JTI_RE = /^training-attempt:([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}):hand:([1-9]\d*):decision:([1-9]\d*)$/;
...
const ADMIN_CLOSEOUT_ACKNOWLEDGEMENT = 'I_ACKNOWLEDGE_THE_ADMIN_CLOSEOUT_EVIDENCE_IS_COMPLETE_AND_ACCESS_CONTROLLED';
...
const ADMIN_COLLECTOR_ACKNOWLEDGEMENT = 'I_ACKNOWLEDGE_PHASE6_ADMIN_COLLECTION_RUNS_ROLLBACK_ONLY_NEGATIVE_PROBES';
const CONTROLLED_REHEARSAL_ACKNOWLEDGEMENT = 'I_ACKNOWLEDGE_CONTROLLED_REHEARSAL_IS_NOT_AUTHENTIC_PRODUCTION_PREDECESSOR_EVIDENCE';
...
const RECEIPT_MATERIAL_RE = /(^|[^A-Za-z0-9_-])([A-Za-z0-9_-]{16,}\.[A-Za-z0-9_-]{43})(?=$|[^A-Za-z0-9_-])/g;
const JWT_MATERIAL_RE = /(^|[^A-Za-z0-9_-])eyJ[A-Za-z0-9_-]{8,}\.eyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{20,}(?=$|[^A-Za-z0-9_-])/g;
const SECRET_PREFIX_RE = /(?:^|[^a-z0-9])(?:sb_(?:secret|publishable)_|sk_(?:live|test)_|gh[opusr]_|xox[baprs]-)[a-z0-9_-]{12,}/i;
const SECRET_PREFIX_RE_GLOBAL = /(^|[^a-z0-9])(?:sb_(?:secret|publishable)_|sk_(?:live|test)_|gh[opusr]_|xox[baprs]-)[a-z0-9_-]{12,}/gi;
...
export function redactReceiptMaterial(value) {
  return String(value ?? '')
    ...
    .replace(CONNECTION_CREDENTIAL_RE, (match) => match.replace(/\/\/.*@/, '//[REDACTED_CREDENTIALS]@'));
}
...
export function validateImmutableDeploymentUrl(value) {
  ...
  try {
    ...
    throw new Error('TRAINING_PHASE6_DELIVERY_BASE_URL must be an absolute immutable Vercel deployment URL.');
  }
...
export function readAttestationConfig(env = process.env) {
  return validateAttestationConfig({
    ...
    authState: env.TRAINING_PHASE6_DELIVERY_AUTH_STATE
...
function readPrivateCredential({ directValue, filePath, label }) {
  ...
  assert.notEqual(hasDirect, hasFile, `${label} requires exactly one direct environment value or out-of-repository credential file`);
  ...
  assert.equal(isPathInsideRepository(absolute), false, `${label} credential file must remain outside the repository`);
...
function validateDatabaseCredential(value, expectedProjectRef = null) {
  ...
  try {
    ...
    throw new Error('Phase 6 administrator database credential is not a PostgreSQL connection URL.');
  }
  assert.ok(
    parsed.protocol === 'postgres:' || parsed.protocol === 'postgresql:',
    'Phase 6 administrator database credential must use postgres:// or postgresql://',
  );
  assert.ok(parsed.hostname && parsed.username && parsed.password, 'Phase 6 administrator database credential is incomplete');
  if (expectedProjectRef) {
    ...
    assert.ok(directHost || poolerHost, 'Phase 6 administrator database credential targets a different Supabase project');
  }
...
export function readMachineCollectorConfig(env = process.env) {
  ...
  assert.ok(publicEvidencePath, 'TRAINING_PHASE6_DELIVERY_EVIDENCE is required for machine collection');
  assert.ok(adminEvidencePath, 'TRAINING_PHASE6_DELIVERY_ADMIN_EVIDENCE is required for machine collection');
  assert.ok(finalEvidencePath, 'TRAINING_PHASE6_DELIVERY_FINAL_EVIDENCE is required for machine collection');
  const paths = [publicEvidencePath, adminEvidencePath, finalEvidencePath].map((path) => resolve(path));
  assert.equal(new Set(paths).size, paths.length, 'public, administrator, and final evidence paths must be distinct');
  assert.equal(existsSync(paths[0]), true, 'immutable public evidence file does not exist');
  assert.equal(existsSync(paths[1]), false, 'refusing to overwrite existing administrator evidence');
  assert.equal(existsSync(paths[2]), false, 'refusing to overwrite existing final evidence');

  const expectedAuditUserId = String(env.TRAINING_PHASE6_DELIVERY_EXPECTED_AUDIT_USER_ID || '').trim();
  assert.match(
    expectedAuditUserId,
    UUID_V4_RE,
    'TRAINING_PHASE6_DELIVERY_EXPECTED_AUDIT_USER_ID must be the designated audit account canonical lowercase UUID v4',
  ...
  const expectedSupabaseProjectRef = String(env.TRAINING_PHASE6_EXPECTED_SUPABASE_PROJECT_REF || '').trim();
  assert.match(expectedSupabaseProjectRef, /^[a-z0-9]{20}$/, 'TRAINING_PHASE6_EXPECTED_SUPABASE_PROJECT_REF is required');
  const databaseUrl = validateDatabaseCredential(readPrivateCredential({
  ...
  assert.ok(vercelToken.length >= 20 && !/\s/.test(vercelToken), 'Phase 6 Vercel credential has an invalid shape');
  ...
  if (predecessorMode === 'controlled_rehearsal') {
    ...
    assert.equal(isPathInsideRepository(artifactPath), false, 'predecessor artifact must remain outside the repository');
    assert.equal(isPathInsideRepository(publicKeyPath), false, 'predecessor public key must remain outside the repository');
    ...
    assert.equal(statSync(artifactPath).mode & 0o077, 0, 'authentic predecessor artifact must not be group/world accessible');
    const trustedKeySha256 = String(env.TRAINING_PHASE6_PREDECESSOR_PUBLIC_KEY_SHA256 || '').trim().toLowerCase();
...
export function decodeReceiptObservation(receipt) {
  ...
  assert.ok(
    ...
    'grading receipt signature has the wrong shape',
  );
  let payload;
  try {
  ...
  assert.ok(payload && typeof payload === 'object' && !Array.isArray(payload), 'grading receipt payload is invalid');
  ...
  if (receiptJti.startsWith('training-attempt:')) {
    assert.ok(attemptScopedMatch, 'attempt-scoped receipt jti must use the canonical UUID v4 and ordinal format');
  }
  if (attemptScopedMatch) {
    assert.match(String(payload.attemptId || ''), UUID_V4_RE, 'attempt-scoped receipt omitted a canonical UUID v4 attempt');
    assert.ok(
    ...
    assert.equal(Number(attemptScopedMatch[2]), Number(payload.handOrdinal), 'attempt-scoped receipt jti embeds a different hand');
    assert.equal(Number(attemptScopedMatch[3]), Number(payload.decisionOrdinal), 'attempt-scoped receipt jti embeds a different decision');
    assert.equal(
      receiptJti,
...
export function validateFullAttemptDelivery(payload, {
  sessionId,
  gameId = GAME_ID,
  level = LEVEL,
  ...
  assert.equal(String(payload?.sessionId || ''), String(sessionId || ''), 'initial batch returned the wrong session');
  assert.match(String(payload?.attemptId || ''), UUID_V4_RE, 'initial batch omitted a canonical UUID v4 attempt id');
  assert.equal(payload?.sessionKind, 'campaign', 'initial batch was not a campaign');
  assert.equal(payload?.targetHands, TARGET_HANDS, `initial batch targetHands must be exactly ${TARGET_HANDS}`);
  assert.ok(Array.isArray(payload?.questions), 'initial batch omitted questions');
  assert.equal(payload.questions.length, payload.targetHands, 'initial batch did not deliver the complete attempt');
  ...
  const observations = payload.questions.map((question, index) => {
    ...
    assert.ok(SHA256_RE.test(String(question?.policyChecksum || '')), `question ${index + 1} omitted its policy checksum`);
    ...
    assert.equal(context.sessionTargetHands, payload.targetHands, `question ${index + 1} target mismatch`);
    ...
    assert.equal(context.countsTowardCompletion, true, `question ${index + 1} must count toward completion`);
    assert.equal(context.practiceOnly, false, `question ${index + 1} must not be practice-only`);
    assert.equal(context.difficultyMode, difficultyMode, `question ${index + 1} returned the wrong difficulty mode`);
    assert.ok(Array.isArray(question.options) && question.options.length >= 2, `question ${index + 1} has no choices`);
    ...
    assert.equal(observation.jti, context.submissionId, `question ${index + 1} submission mismatch`);
    assert.equal(observation.attemptId, payload.attemptId, `question ${index + 1} receipt attempt mismatch`);
    assert.equal(observation.snapshotKey, context.snapshotKey, `question ${index + 1} receipt snapshot mismatch`);
    assert.equal(observation.questionId, String(question.id), `question ${index + 1} receipt question mismatch`);
    assert.equal(observation.sessionId, sessionId, `question ${index + 1} receipt session mismatch`);
    assert.equal(observation.sessionKind, 'campaign', `question ${index + 1} receipt session kind mismatch`);
    assert.equal(observation.sessionTargetHands, TARGET_HANDS, `question ${index + 1} receipt target mismatch`);
    ...
    assert.equal(observation.countsTowardCompletion, true, `question ${index + 1} receipt completion binding mismatch`);
    assert.equal(observation.practiceOnly, false, `question ${index + 1} receipt practice binding mismatch`);
    assert.equal(observation.difficultyMode, difficultyMode, `question ${index + 1} receipt difficulty mismatch`);
    assert.ok(Number.isFinite(observation.issuedAt), `question ${index + 1} receipt issue time is invalid`);
    assert.ok(observation.expiresAt > observation.issuedAt, `question ${index + 1} receipt expiry is invalid`);
    assert.equal(observation.format, 'attempt_scoped_serve', `question ${index + 1} did not use an attempt-scoped serve key`);
    assert.equal(UUID_V4_RE.test(observation.jti), false, 'new delivery unexpectedly used a predecessor UUID receipt id');
    assert.equal(questionIds.has(String(question.id)), false, `duplicate question id ${question.id}`);
    assert.equal(snapshotKeys.has(String(context.snapshotKey)), false, `duplicate snapshot key ${context.snapshotKey}`);
    assert.equal(submissionIds.has(String(context.submissionId)), false, `duplicate submission id ${context.submissionId}`);
...
export function compareReissuedManifest(initial, reissued) {
  ...
  assert.equal(reissued?.recoveredExistingAttempt, true, 'reissue did not recover the existing attempt');
  ...
  assert.equal(reissued?.questions?.length, initial.questions.length, 'reissue changed manifest length');
  return reissued.questions.map((question, index) => {
    ...
    assert.equal(publicQuestionFingerprint(question), publicQuestionFingerprint(before), `reissue changed public question ${index + 1}`);
    for (const field of ['submissionId', 'sessionId', 'attemptId', 'snapshotKey', 'sessionKind', 'sessionTargetHands', 'handOrdinal', 'decisionOrdinal', 'countsTowardCompletion', 'practiceOnly', 'difficultyMode', 'rngRolls', 'solverEvidenceAvailable']) {
      assert.deepEqual(afterContext?.[field], beforeContext?.[field], `reissue changed ${field} for hand ${index + 1}`);
    }
    ...
    assert.equal(observation.format, 'attempt_scoped_serve', `reissued hand ${index + 1} receipt format mismatch`);
    assert.equal(observation.jti, afterContext.submissionId, `reissued hand ${index + 1} submission mismatch`);
    assert.equal(observation.attemptId, afterContext.attemptId, `reissued hand ${index + 1} attempt mismatch`);
    assert.equal(observation.snapshotKey, afterContext.snapshotKey, `reissued hand ${index + 1} snapshot mismatch`);
    assert.equal(observation.questionId, String(question.id), `reissued hand ${index + 1} question mismatch`);
    assert.equal(observation.sessionId, afterContext.sessionId, `reissued hand ${index + 1} session mismatch`);
    assert.equal(observation.sessionKind, afterContext.sessionKind, `reissued hand ${index + 1} session kind mismatch`);
    assert.equal(observation.sessionTargetHands, afterContext.sessionTargetHands, `reissued hand ${index + 1} target mismatch`);
    assert.equal(observation.handOrdinal, afterContext.handOrdinal, `reissued hand ${index + 1} hand ordinal mismatch`);
    assert.equal(observation.decisionOrdinal, afterContext.decisionOrdinal, `reissued hand ${index + 1} decision ordinal mismatch`);
    assert.equal(
    ...
    assert.equal(observation.difficultyMode, afterContext.difficultyMode, `reissued hand ${index + 1} difficulty mismatch`);
    return {
...
export function buildAnswerRequest(question, selectedAnswer) {
  const context = question?._gradingContext;
  assert.ok(context?.receipt && context?.submissionId && context?.attemptId && context?.snapshotKey, 'question lacks signed grading context');
  assert.ok(SHA256_RE.test(String(question?.policyChecksum || '')), 'question lacks canonical policy checksum');
  assert.ok(question.options?.some((option) => String(option?.id ?? option) === String(selectedAnswer)), 'selected answer is not a public option');
  return {
    gameId: GAME_ID,
...
export function immutableAnswerEvidence(payload) {
  ...
  assert.ok(payload?.feedback && typeof payload?.evidence?.isCorrect === 'boolean', 'answer omitted authoritative feedback');
  ...
  assert.match(String(payload?.attemptId || ''), UUID_V4_RE, 'answer omitted canonical UUID v4 attemptId');
  assert.match(String(payload?.snapshotKey || ''), SHA256_RE, 'answer omitted snapshotKey');
  assert.ok(Number.isInteger(payload?.handOrdinal) && payload.handOrdinal >= 1, 'answer omitted handOrdinal');
  assert.ok(Number.isInteger(payload?.decisionOrdinal) && payload.decisionOrdinal >= 1, 'answer omitted decisionOrdinal');
  assert.equal(typeof payload?.countsTowardCompletion, 'boolean', 'answer omitted completion binding');
...
export function validateAnswerBinding(payload, question, label = 'answer') {
  ...
  for (const field of [
    'submissionId', 'sessionId', 'attemptId', 'snapshotKey', 'handOrdinal',
    'decisionOrdinal', 'countsTowardCompletion', 'practiceOnly',
  ]) {
...
export function assertExactReplay(first, replay, label = 'answer') {
  ...
  assert.deepEqual(immutableAnswerEvidence(replay), immutableAnswerEvidence(first), `${label} replay changed immutable evidence`);
}
...
export function validateContinuation(parentQuestion, parentAnswer, first, replay) {
  ...
  assert.ok(first?.question && child?.receipt, 'next-street response omitted a signed child question');
  ...
  assert.equal(first.decisionOrdinal, parent.decisionOrdinal + 1, 'continuation did not advance exactly one decision');
  ...
  assert.equal(child.sessionTargetHands, parent.sessionTargetHands, 'child receipt changed target hands');
  assert.equal(child.handOrdinal, parent.handOrdinal, 'child receipt changed hand');
  assert.equal(child.decisionOrdinal, parent.decisionOrdinal + 1, 'child receipt decision mismatch');
  ...
  assert.equal(child.difficultyMode, parent.difficultyMode, 'child receipt changed difficulty mode');
  assert.notEqual(child.snapshotKey, parent.snapshotKey, 'continuation reused the parent snapshot');
  assert.notEqual(child.submissionId, parent.submissionId, 'continuation reused the parent submission');
  assert.ok(SHA256_RE.test(String(first.question.policyChecksum || '')), 'continuation omitted its policy checksum');
  ...
  assert.equal(childReceipt.format, 'attempt_scoped_serve', 'continuation receipt was not attempt-scoped');
  ...
  assert.equal(childReceipt.snapshotKey, child.snapshotKey, 'continuation receipt snapshot mismatch');
  assert.equal(childReceipt.questionId, String(first.question.id), 'continuation receipt question mismatch');
  assert.equal(childReceipt.sessionId, child.sessionId, 'continuation receipt session mismatch');
  assert.equal(childReceipt.sessionKind, child.sessionKind, 'continuation receipt session kind mismatch');
  assert.equal(childReceipt.sessionTargetHands, child.sessionTargetHands, 'continuation receipt target mismatch');
  assert.equal(childReceipt.handOrdinal, child.handOrdinal, 'continuation receipt hand mismatch');
  assert.equal(childReceipt.decisionOrdinal, child.decisionOrdinal, 'continuation receipt decision mismatch');
  assert.equal(
    childReceipt.countsTowardCompletion,
    child.countsTowardCompletion,
    'continuation receipt completion binding mismatch',
  );
  assert.equal(childReceipt.practiceOnly, child.practiceOnly, 'continuation receipt practice binding mismatch');
  assert.equal(childReceipt.difficultyMode, child.difficultyMode, 'continuation receipt difficulty mismatch');
  assert.equal(parentAnswer?.feedback?.continuation?.actionId, parentAnswer?.selectedAnswer, 'saved parent answer did not follow the canonical continuation branch');
  assert.equal(replay?.success, true, 'continuation replay did not succeed');
  assert.equal(replay?.recoveredExistingContinuation, true, 'continuation replay did not recover the registered child');
  for (const field of ['attemptId', 'sessionId', 'handOrdinal', 'decisionOrdinal', 'snapshotKey', 'submissionId']) {
  ...
  assert.ok(
    sha256Bytes(String(replayContext?.receipt ?? ''))
      === sha256Bytes(String(child.receipt ?? '')),
    'continuation replay changed receipt bytes',
  );
  for (const field of [
    'submissionId', 'sessionId', 'attemptId', 'snapshotKey', 'sessionKind',
    'sessionTargetHands', 'handOrdinal', 'decisionOrdinal',
    'countsTowardCompletion', 'practiceOnly', 'difficultyMode',
  ]) {
  ...
  for (const field of [
    'version', 'jti', 'attemptId', 'snapshotKey', 'questionId', 'sessionId',
    'sessionKind', 'sessionTargetHands', 'handOrdinal', 'decisionOrdinal',
    'countsTowardCompletion', 'practiceOnly', 'difficultyMode', 'format',
  ]) {
    assert.deepEqual(replayReceipt[field], childReceipt[field], `continuation replay receipt changed ${field}`);
  }
  assert.equal(publicQuestionFingerprint(replay.question), publicQuestionFingerprint(first.question), 'continuation replay changed the child question');
...
export function receiptFormatCensus(observations) {
  ...
  return {
    ...
    limitation: counts.predecessorUuidV4 > 0
      ? null
...
export function createSlidingWindowRequestPacer({
  ...
  assert.ok(Number.isInteger(maxRequests) && maxRequests >= 1, 'request pacer maxRequests must be a positive integer');
  assert.ok(Number.isFinite(windowMs) && windowMs > 0, 'request pacer windowMs must be positive');
  assert.ok(Number.isFinite(safetyMs) && safetyMs >= 0, 'request pacer safetyMs must not be negative');
  const issuedAt = [];
...
function assertEvidenceContainsNoSecrets(value, path = 'admin evidence') {
  ...
  if (typeof value === 'string') {
    ...
    assert.equal(RECEIPT_MATERIAL_RE.test(value), false, `${path} contains grading-receipt material`);
    assert.equal(JWT_MATERIAL_RE.test(value), false, `${path} contains JWT material`);
    assert.equal(SECRET_PREFIX_RE.test(value), false, `${path} contains secret-like credential material`);
    return;
  }
...
function assertExactObjectKeys(value, expected, label) {
  ...
  assert.deepEqual(Object.keys(value).sort(), [...expected].sort(), `${label} fields do not match the closeout schema`);
}
...
function assertExactStringSet(actual, expected, label) {
  assert.deepEqual(exactUniqueStrings(actual, label), [...expected].sort(), `${label} does not match the public evidence`);
}
...
function isoTimestamp(value, label) {
  ...
  assert.equal(new Date(timestamp).toISOString(), text, `${label} must use canonical UTC ISO-8601 milliseconds`);
  return timestamp;
}

function canonicalAttemptEventKey({ attemptId, handOrdinal, decisionOrdinal }, label) {
  assert.match(String(attemptId || ''), UUID_V4_RE, `${label} attemptId must be a canonical lowercase UUID v4`);
  assert.ok(Number.isSafeInteger(handOrdinal) && handOrdinal >= 1, `${label} handOrdinal must be a positive safe integer`);
  assert.ok(Number.isSafeInteger(decisionOrdinal) && decisionOrdinal >= 1, `${label} decisionOrdinal must be a positive safe integer`);
  return `training-attempt:${attemptId}:hand:${handOrdinal}:decision:${decisionOrdinal}`;
}

function validatePublicCorrelationRecord(record, {
  attemptId,
  expectedHandOrdinal,
  ...
  if (expectedDecisionOrdinal !== undefined) {
    assert.equal(record.decisionOrdinal, expectedDecisionOrdinal, `${label} decision ordinal mismatch`);
  }
  const eventKey = canonicalAttemptEventKey(record, label);
  assert.equal(record.eventKey, eventKey, `${label} event key is not canonical or does not match its slot`);
  assert.equal(record.submissionId, eventKey, `${label} submissionId must equal its canonical event key`);
  assert.match(String(record.snapshotKey || ''), SHA256_RE, `${label} snapshotKey is invalid`);
  assert.match(String(record.policyChecksum || ''), SHA256_RE, `${label} policyChecksum is invalid`);
...
function validatePublicAnswerEvidence(value, correlation, label, expectedCountsTowardCompletion) {
  assert.ok(value && typeof value === 'object' && !Array.isArray(value), `${label} immutable evidence is missing`);
  for (const field of ['submissionId', 'attemptId', 'snapshotKey', 'handOrdinal', 'decisionOrdinal']) {
  ...
  assert.equal(value.countsTowardCompletion, expectedCountsTowardCompletion, `${label} completion binding mismatch`);
  assert.equal(value.practiceOnly, false, `${label} practice binding mismatch`);
  assert.equal(typeof value.evidence?.isCorrect, 'boolean', `${label} authoritative correctness is missing`);
  assert.ok(value.feedback && typeof value.feedback === 'object', `${label} authoritative feedback is missing`);
}
...
export function validateCompletePublicAttestation(publicEvidence) {
  ...
  assert.equal(publicEvidence?.success, false, 'public evidence was unexpectedly already finalized');
  assert.equal(publicEvidence?.releaseGateReady, false, 'public evidence was unexpectedly already release-ready');
  assert.equal(
    publicEvidence?.status,
    'public_api_verified_admin_correlation_pending',
    'public evidence is not awaiting administrator closeout',
  );
  assert.match(String(publicEvidence?.expectedBuild || ''), SHA40_RE, 'public evidence build is invalid');
  assert.equal(publicEvidence.expectedBuild, publicEvidence.expectedBuild.toLowerCase(), 'public evidence build must be lowercase');

  const deployment = publicEvidence?.deployment;
  assert.ok(deployment && typeof deployment === 'object', 'public evidence deployment identity is missing');
  assertExactObjectKeys(deployment, ['commitSha', 'version', 'deploymentUrl', 'deploymentId'], 'public deployment identity');
  assert.equal(deployment.commitSha, publicEvidence.expectedBuild, 'public deployment commit mismatch');
  assert.equal(deployment.version, publicEvidence.expectedBuild, 'public deployment version mismatch');
  ...
  assert.ok(
    typeof deployment.deploymentId === 'string' && deployment.deploymentId.trim() === deployment.deploymentId
      && deployment.deploymentId.length >= 3 && deployment.deploymentId.length <= 200,
  ...
  assertExactObjectKeys(writeScope, ['acknowledged', 'auditUserId', 'gameId', 'level', 'effects'], 'public write scope');
  ...
  assertExactStringSet(writeScope?.effects, [
  ...
  assert.ok(Array.isArray(parents) && parents.length === TARGET_HANDS, 'public evidence does not contain all parent attempts');
  const expectedEventKeys = parents.map((record, index) => {
    ...
    assert.ok(String(record.selectedAnswer || '').length > 0, `${label} selected answer is missing`);
    assert.equal(typeof record.followedContinuationBranch, 'boolean', `${label} continuation selection is missing`);
    assert.equal(record.responseLossRecovered, index === 0, `${label} response-loss result mismatch`);
    assert.equal(record.exactReplay, true, `${label} exact replay did not pass`);
    assert.equal(record.initialReceiptServerVerified, true, `${label} initial receipt was not server verified`);
    assert.equal(record.reissuedReceiptServerVerified, true, `${label} reissued receipt was not server verified`);
    assert.equal(typeof record.receiptBytesIdentical, 'boolean', `${label} receipt-byte comparison is missing`);
    assert.ok(
      ['same_bytes_replayed_through_record_question', 'refreshed_receipt_replayed_through_record_question']
        .includes(record.reissuedReceiptVerification),
      `${label} reissued receipt verification is invalid`,
    );
    assert.equal(record.recordQuestionRequests, index === 0 ? 3 : 2, `${label} request count is invalid`);
    return eventKey;
  });
  assert.equal(new Set(expectedEventKeys).size, TARGET_HANDS, 'public parent event keys are not unique');

  const reissue = api?.reissue;
  assert.equal(reissue?.manifestIdentityExact, true, 'public reissue manifest identity did not pass');
  assert.equal(reissue?.recoveredExistingAttempt, true, 'public reissue did not recover the attempt');
  assert.equal(typeof reissue?.receiptBytesIdentical, 'boolean', 'public reissue receipt-byte aggregate is missing');
  assert.ok(Array.isArray(reissue?.hands) && reissue.hands.length === TARGET_HANDS, 'public reissue omitted hands');
  reissue.hands.forEach((hand, index) => {
    const parent = parents[index];
    assert.equal(hand.questionId, parent.questionId, `public reissue hand ${index + 1} question mismatch`);
    assert.equal(hand.handOrdinal, index + 1, `public reissue hand ${index + 1} ordinal mismatch`);
    assert.equal(hand.snapshotKey, parent.snapshotKey, `public reissue hand ${index + 1} snapshot mismatch`);
    assert.equal(hand.submissionId, parent.submissionId, `public reissue hand ${index + 1} submission mismatch`);
    assert.match(String(hand.publicQuestionDigest || ''), SHA256_RE, `public reissue hand ${index + 1} digest is invalid`);
    assert.equal(typeof hand.receiptBytesIdentical, 'boolean', `public reissue hand ${index + 1} byte comparison is missing`);
    assert.equal(hand.receiptBytesIdentical, parent.receiptBytesIdentical, `public reissue hand ${index + 1} byte comparison mismatch`);
  });
  assert.equal(
  ...
  assert.deepEqual(api?.responseLossRecovery, {
  ...
  for (const [label, section] of [['initial', receiptVerification?.initial], ['reissued', receiptVerification?.reissued]]) {
  ...
  assert.equal(rate?.maxRequests, RECORD_QUESTION_WINDOW_MAX, 'record-question attestation pacing maximum mismatch');
  assert.equal(rate?.windowMs, RECORD_QUESTION_WINDOW_MS, 'record-question attestation pacing window mismatch');
  assert.equal(rate?.requestsIssued, 45, 'record-question request census mismatch');
  assert.ok(Number.isFinite(rate?.totalWaitMs) && rate.totalWaitMs >= 0, 'record-question pacing wait census is invalid');

  assert.deepEqual(api?.conflictingReplayRefusals, {
  ...
  const continuationParent = parents.find(({ eventKey }) => eventKey === continuation.parentEventKey);
  assert.ok(continuationParent, 'public continuation parent event key is not one of the parent events');
  assert.equal(continuation.handOrdinal, continuationParent.handOrdinal, 'public continuation changed the parent hand');
  assert.equal(continuation.decisionOrdinal, continuationParent.decisionOrdinal + 1, 'public continuation is not the next decision');
  assert.equal(continuation.recoveredExistingContinuation, true, 'public continuation replay did not recover the child');
  assert.equal(continuation.parentBindingVerifiedFromPublicReceipts, true, 'public continuation parent binding did not pass');
  assert.match(String(continuation.childPublicQuestionDigest || ''), SHA256_RE, 'public continuation digest is invalid');
  const selectedParentEventKey = validatePublicCorrelationRecord(api?.parent, {
    attemptId,
    label: 'public selected parent',
  });
  assert.equal(selectedParentEventKey, continuation.parentEventKey, 'public selected parent does not bind the continuation');
  for (const field of ['snapshotKey', 'questionId', 'policyChecksum', 'submissionId', 'selectedAnswer']) {
    assert.deepEqual(api.parent[field], continuationParent[field], `public selected parent changed ${field}`);
  }
  assert.equal(api.parent.followedContinuationBranch, true, 'public selected parent did not follow its continuation branch');
  assert.equal(api.parent.exactReplay, true, 'public selected parent exact replay did not pass');
  assert.ok(String(api.parent.selectedAnswer || '').length > 0, 'public selected parent answer is missing');
  validatePublicAnswerEvidence(api.parent.immutableEvidence, api.parent, 'public selected parent', true);
  assert.equal(api.parent.immutableEvidence.sessionId, initial.sessionId, 'public selected parent immutable session mismatch');
  assert.equal(api?.childAnswer?.exactReplay, true, 'public child answer exact replay did not pass');
  assert.ok(String(api?.childAnswer?.selectedAnswer || '').length > 0, 'public child answer selection is missing');
  validatePublicAnswerEvidence(api.childAnswer.immutableEvidence, continuation, 'public child answer', false);
  assert.equal(api.childAnswer.immutableEvidence.sessionId, initial.sessionId, 'public child answer immutable session mismatch');

  const census = api?.receiptFormatCensus;
  assert.equal(census?.observedReceipts, 42, 'public receipt census did not cover every observed receipt');
  assert.deepEqual(census?.counts, { attemptScopedServe: 42, predecessorUuidV4: 0, unknown: 0 }, 'public receipt census is invalid');
  assert.equal(census?.predecessorFormatObserved, false, 'public receipt census falsely claimed predecessor coverage');
  assert.equal(census?.predecessorStatus, 'not_observed', 'public predecessor receipt status is invalid');
  assert.ok(String(census?.limitation || '').includes('did not fabricate'), 'public predecessor limitation is missing');

  const startedAt = isoTimestamp(publicEvidence?.startedAt, 'public evidence startedAt');
  const apiCompletedAt = isoTimestamp(publicEvidence?.apiCompletedAt, 'public evidence apiCompletedAt');
  ...
  assert.equal(settle?.windowStart, publicEvidence.apiCompletedAt, 'public error-settle start mismatch');
  assert.equal(settle?.windowEnd, publicEvidence.completedAt, 'public error-settle end mismatch');
  assert.ok(settle?.observedMs >= ERROR_SETTLE_WINDOW_MS, 'public error-settle window was too short');
  assert.equal(settle?.publicClientErrorCount, 0, 'public client error review found errors');
  assert.equal(completedAt - apiCompletedAt, settle.observedMs, 'public error-settle duration is internally inconsistent');
...
export function validateAdministratorCloseout(
  ...
  assert.match(String(publicEvidenceSha256 || ''), SHA256_RE, 'public evidence checksum is invalid');
  assertExactObjectKeys(adminEvidence, [
    'schemaVersion',
    'evidenceKind',
    'publicEvidenceSha256',
    'expectedBuild',
  ...
  assert.equal(adminEvidence?.evidenceKind, ADMIN_EVIDENCE_KIND, 'administrator evidence kind mismatch');
  assert.equal(adminEvidence?.publicEvidenceSha256, publicEvidenceSha256, 'administrator evidence binds a different public file');
  assert.equal(adminEvidence?.expectedBuild, publicEvidence.expectedBuild, 'administrator evidence build mismatch');
  assert.equal(adminEvidence?.auditUserId, publicContract.auditUserId, 'administrator evidence audit account mismatch');

  assert.deepEqual(adminEvidence?.deployment, {
  ...
  assertExactObjectKeys(collector, [
    'kind',
    'status',
    'generatedAt',
    'queryMode',
  ...
  assert.equal(collector?.queryMode, 'read_only_plus_explicit_rolled_back_probes', 'administrator collector query mode mismatch');
  assert.ok(String(collector?.currentUser || '').length > 0, 'administrator collector database user is missing');
  assert.ok(String(collector?.currentDatabase || '').length > 0, 'administrator collector database name is missing');
  assert.match(String(collector?.serverVersionNum || ''), /^\d{5,6}$/, 'administrator collector server version is invalid');
  assert.match(String(collector?.supabaseProjectRef || ''), /^[a-z0-9]{20}$/, 'administrator collector Supabase project ref is invalid');
  assert.equal(collector?.publicEvidenceSha256, publicEvidenceSha256, 'administrator collector binds a different public file');
  assert.equal(collector?.correlationReadOnlyVerified, true, 'administrator correlation was not collected in a read-only transaction');
  assert.equal(collector?.correlationTransactionRolledBack, true, 'administrator correlation transaction was not rolled back');
  assert.ok(
  ...
  const collectorGeneratedAt = isoTimestamp(collector?.generatedAt, 'administrator collector generatedAt');

  const { attemptId } = publicContract;
  assert.equal(adminEvidence?.attemptId, attemptId, 'administrator evidence attempt mismatch');
  const publicCompletedAt = isoTimestamp(publicEvidence?.completedAt, 'public evidence completedAt');
  ...
  assert.ok(collectorGeneratedAt >= publicCompletedAt, 'administrator collector predates public completion');
  assert.ok(verifiedAt >= collectorGeneratedAt, 'administrator verification predates collector completion');
  ...
  assert.equal(new Set(expectedEventKeys).size, TARGET_HANDS + 1, 'public evidence event keys are not unique');
  const correlation = adminEvidence?.correlation;
  assertExactObjectKeys(correlation, [
    'status',
    'attemptId',
    'auditUserId',
  ...
  assert.equal(correlation?.auditUserId, publicContract.auditUserId, 'administrator correlation audit account mismatch');
  assert.equal(correlation?.attemptRowBindingExact, true, 'administrator attempt row binding did not pass');
  assert.equal(correlation?.servedEventCount, TARGET_HANDS + 1, 'administrator served-event count mismatch');
  assert.equal(correlation?.answerCount, TARGET_HANDS + 1, 'administrator answer count mismatch');
  assertExactStringSet(correlation?.servedEventKeys, expectedEventKeys, 'correlated served event keys');
  assert.equal(correlation?.allServedFieldBindingsExact, true, 'administrator served-event field binding did not pass');
  assertExactStringSet(correlation?.answerSubmissionIds, expectedSubmissionIds, 'correlated answer submission ids');
  assert.equal(correlation?.allAnswerFieldBindingsExact, true, 'administrator answer field binding did not pass');
  assert.equal(
    correlation?.continuationParentEventKey,
    continuation.parentEventKey,
    'administrator continuation parent binding mismatch',
  );
  assert.equal(correlation?.continuationSlotBindingExact, true, 'administrator continuation-slot binding did not pass');

  const privateAttestation = adminEvidence?.privateAttemptScopedServeAttestation;
  assertExactObjectKeys(privateAttestation, [
    'status',
    'rowCount',
    'contractVersion',
    'evidenceKind',
  ...
  assert.equal(privateAttestation?.status, 'passed', 'private attempt-scoped attestation did not pass');
  assert.equal(privateAttestation?.rowCount, 1, 'private attempt-scoped attestation must be exactly one row');
  assert.equal(
    privateAttestation?.contractVersion,
    'training-attempt-decision-authority-v1',
    'private attestation contract version mismatch',
  );
  assert.equal(privateAttestation?.evidenceKind, 'attempt_scoped_serve', 'private attestation kind mismatch');
  assert.match(String(privateAttestation?.evidenceEventKey || ''), ATTEMPT_SCOPED_JTI_RE, 'private attestation event key is not canonical attempt-scoped evidence');
  assert.equal(privateAttestation?.evidenceEventExists, true, 'private attestation evidence event does not exist');
  assert.equal(privateAttestation?.evidenceOwnerPresent, true, 'private attestation evidence event has no owner');

  const refusalMatrix = adminEvidence?.negativeRefusalMatrix;
  assertExactObjectKeys(refusalMatrix, ['status', 'probeCount', 'transactionRolledBack', 'probes'], 'negative refusal matrix');
  assert.equal(refusalMatrix?.status, 'passed', 'negative refusal matrix did not pass');
  assert.equal(refusalMatrix?.probeCount, ADMIN_NEGATIVE_PROBES.length, 'negative refusal probe count mismatch');
  ...
  assertExactObjectKeys(predecessor, [
    'status',
    'method',
    'provenance',
    'artifactSha256',
  ...
  if (predecessor.method === 'authentic_saved_receipt') {
    assert.equal(predecessor?.provenance, 'signature_verified_authentic_artifact', 'authentic predecessor provenance mismatch');
    assert.match(String(predecessor?.artifactSha256 || ''), SHA256_RE, 'authentic predecessor artifact digest is invalid');
    assert.match(String(predecessor?.trustedKeySha256 || ''), SHA256_RE, 'authentic predecessor trust-key digest is invalid');
    assert.equal(predecessor?.verifierSha256, null, 'authentic predecessor evidence must not claim rehearsal provenance');
  } else {
    assert.equal(predecessor?.provenance, 'controlled_rehearsal_not_authentic', 'controlled rehearsal provenance mismatch');
    assert.equal(predecessor?.artifactSha256, null, 'controlled rehearsal must not claim an authentic artifact');
    assert.equal(predecessor?.trustedKeySha256, null, 'controlled rehearsal must not claim a trusted artifact key');
    assert.match(String(predecessor?.verifierSha256 || ''), SHA256_RE, 'controlled rehearsal verifier digest is invalid');
  }
  assert.equal(predecessor?.transactionRolledBack, true, 'predecessor rehearsal was not rolled back');
  assert.equal(predecessor?.initialWrite, 'passed', 'predecessor initial write did not pass');
  assert.equal(predecessor?.continuationWrite, 'passed', 'predecessor continuation write did not pass');

  const errorReview = adminEvidence?.productionErrorStreamReview;
  assertExactObjectKeys(errorReview, [
    'status',
    'expectedBuild',
    'deploymentId',
    'relevantErrorCount',
  ...
  assert.equal(errorReview?.expectedBuild, publicEvidence.expectedBuild, 'production error review build mismatch');
  assert.equal(errorReview?.deploymentId, publicContract.deploymentId, 'production error review deployment mismatch');
  assert.equal(errorReview?.relevantErrorCount, 0, 'production error-stream review found relevant errors');
  assert.equal(errorReview?.settledWindowCovered, true, 'production error review omitted the settled window');
  assert.equal(errorReview?.source, 'vercel_cli_runtime_logs', 'production error review source is invalid');
  assert.equal(errorReview?.deploymentUrl, publicContract.deploymentUrl, 'production error review deployment URL mismatch');
  assert.equal(errorReview?.queryComplete, true, 'production error log query was incomplete');
  assert.deepEqual(errorReview?.levelsReviewed, ['error', 'fatal'], 'production error review did not cover error and fatal levels');
  ...
  assert.equal(errorReview?.windowStart, publicEvidence.startedAt, 'production error review must start at the exact public attestation start');
  assert.equal(errorReview?.windowEnd, publicEvidence.completedAt, 'production error review must end at the exact public attestation completion');
...
export function finalizeProductionDeliveryAttestation({
  ...
  assert.ok(typeof publicEvidencePath === 'string' && publicEvidencePath.length > 0, 'public evidence path is required');
  assert.ok(typeof adminEvidencePath === 'string' && adminEvidencePath.length > 0, 'administrator evidence path is required');
  assert.ok(typeof outputPath === 'string' && outputPath.length > 0, 'final evidence path is required');
  const publicPath = resolve(publicEvidencePath);
  const adminPath = resolve(adminEvidencePath);
  const finalPath = resolve(outputPath);
  assert.notEqual(publicPath, adminPath, 'public and administrator evidence must be separate files');
  assert.notEqual(finalPath, publicPath, 'final evidence must not overwrite the immutable public evidence');
  assert.notEqual(finalPath, adminPath, 'final evidence must not overwrite the administrator evidence');
...
export function readAdministratorCloseoutConfig(env = process.env) {
  ...
  assert.ok(publicEvidencePath, 'TRAINING_PHASE6_DELIVERY_EVIDENCE is required for administrator closeout');
  assert.ok(adminEvidencePath, 'TRAINING_PHASE6_DELIVERY_ADMIN_EVIDENCE is required for administrator closeout');
  assert.ok(outputPath, 'TRAINING_PHASE6_DELIVERY_FINAL_EVIDENCE is required for administrator closeout');
  return {
    publicEvidencePath,
    adminEvidencePath,
    outputPath,
...
async function rollbackAndVerify(database, label, postRollbackCheck = null) {
  ...
  const state = rowsFromQuery(await database.query(
  ...
  assert.equal(state[0].transactionIdle, true, `${label} transaction did not return to an unassigned state`);
...
async function collectReadOnlyDatabaseCorrelation(database, publicEvidence, publicContract) {
  ...
  try {
    ...
    const modeRows = rowsFromQuery(await database.query(
    ...
    assert.equal(modeRows[0]?.transactionReadOnly, 'on', 'administrator correlation transaction is not read-only');

    const attemptRows = rowsFromQuery(await database.query(
    ...
    assert.equal(attemptRows.length, 1, 'administrator correlation requires exactly one attempt row');
    assert.deepEqual(attemptRows[0], {
    ...
    const servedRows = rowsFromQuery(await database.query(
    ...
    assert.equal(servedRows.length, TARGET_HANDS + 1, 'administrator served-event row count mismatch');
    const publicDecisions = publicEvidence.publicApi.parentCandidateAttempts.concat(publicEvidence.publicApi.continuation);
    const publicDecisionByKey = new Map(publicDecisions.map((decision) => [decision.eventKey, decision]));
    for (const row of servedRows) {
      ...
      assert.equal(row.difficultyMode, PERSISTED_DIFFICULTY_MODE, 'served-event difficulty mismatch');
      assert.ok(row.rngRolls && typeof row.rngRolls === 'object', 'served-event RNG authority is missing');
    }

    const answerRows = rowsFromQuery(await database.query(
    ...
    const expectedAnswers = new Map(publicEvidence.publicApi.parentCandidateAttempts.map((parent) => [
      parent.submissionId,
    ...
    for (const row of answerRows) {
      ...
      assert.equal(row.sessionId, publicEvidence.publicApi.initialAttempt.sessionId, 'answer session mismatch');
    ...
    const slotRows = rowsFromQuery(await database.query(
      ...
      [publicContract.attemptId, publicContract.continuation.handOrdinal, publicContract.continuation.decisionOrdinal],
    ...
    const attestationRows = rowsFromQuery(await database.query(
    ...
    assert.equal(attestationRows.length, 1, 'private delivery attestation must contain exactly one authority row');
...
function probeInputFor(name, publicEvidence, publicContract, databaseEvidence, nowEpochSeconds) {
  ...
  const servedParent = databaseEvidence.servedRows.find(({ eventKey }) => eventKey === parent.eventKey);
  ...
  switch (name) {
    case 'answeredSlotPromotion':
      return {
        ...
        params: [publicContract.attemptId, publicContract.auditUserId, JSON.stringify([{
...
async function verifyProbeRollback(database, name, publicEvidence, publicContract, databaseEvidence) {
  const parent = publicEvidence.publicApi.parentCandidateAttempts[0];
  const continuation = publicEvidence.publicApi.continuation;
  if (name === 'changedAnswerReplay') {
    const rows = rowsFromQuery(await database.query(
    ...
    assert.deepEqual(rows, [{ answerId: parent.selectedAnswer }], 'changed-answer probe was not rolled back');
    ...
    assert.deepEqual(rows, [{ snapshotKey: databaseEvidence.continuationSlot.snapshotKey }], 'changed-slot probe was not rolled back');
    ...
    assert.deepEqual(rows, [{ count: 1 }], 'never-served probe did not restore the served event on rollback');
  }
}
...
async function runOneRollbackNegativeProbe(
  ...
  assert.ok(ADMIN_NEGATIVE_PROBES.includes(name), `Unreviewed Phase 6 negative probe refused: ${name}`);
  const expectation = ADMIN_NEGATIVE_PROBE_EXPECTATIONS[name];
  const input = probeInputFor(name, publicEvidence, publicContract, databaseEvidence, nowEpochSeconds);
  let transactionStarted = false;
  let rolledBack = false;
  let matched = false;
  ...
  try {
    ...
    for (const setup of input.setup || []) {
      ...
      if (setup.expectedRowCount !== undefined) {
        assert.equal(result.rowCount, setup.expectedRowCount, `${name} setup did not touch exactly the reviewed row`);
      }
    }
    try {
      ...
      if (expectation.outcome === 'error') {
        matched = String(error?.code || '') === expectation.code
...
async function collectNegativeRefusalMatrix(database, publicEvidence, publicContract, databaseEvidence, nowEpochSeconds) {
  assert.deepEqual(
...
export function createVercelCliRuntimeLogTransport({
  ...
  assert.ok(typeof token === 'string' && token.length >= 20 && !/\s/.test(token), 'Vercel log adapter credential is invalid');
  assert.ok(typeof project === 'string' && project.trim().length > 0, 'Vercel log adapter project is required');
  ...
  return {
    async collect({ deploymentId, deploymentUrl, expectedBuild, windowStart, windowEnd }) {
      assert.ok(String(deploymentId || '').length > 0, 'Vercel log adapter requires an exact deployment ID');
      validateImmutableDeploymentUrl(deploymentUrl);
      assert.match(String(expectedBuild || ''), SHA40_RE, 'Vercel log adapter requires an exact build SHA');
      isoTimestamp(windowStart, 'Vercel log window start');
      isoTimestamp(windowEnd, 'Vercel log window end');
      assert.ok(Date.parse(windowEnd) >= Date.parse(windowStart), 'Vercel log window ends before it starts');
      const records = [];
      for (const level of ['error', 'fatal']) {
        ...
        assert.ok(levelRows.length < ADMIN_LOG_LIMIT, `TRAINING_PHASE6_VERCEL_LOG_QUERY_TRUNCATED:${level}`);
        for (const row of levelRows) {
          const observedDeploymentId = row.deploymentId || row.deployment?.id || null;
          if (observedDeploymentId !== null) {
            assert.equal(observedDeploymentId, deploymentId, 'Vercel log adapter returned a cross-deployment record');
          }
    ...
    async collect() {
      const result = spawnSyncFn(process.execPath, [verifierPath], {
        ...
        env: {
          ...
          ...(process.env.PHASE6_POSTGRES_BIN ? { PHASE6_POSTGRES_BIN: process.env.PHASE6_POSTGRES_BIN } : {}),
        },
      ...
      if (!evidenceLine) throw new Error('TRAINING_PHASE6_CONTROLLED_PREDECESSOR_REHEARSAL_OUTPUT_INVALID');
      let evidence;
      try {
      ...
      for (const gate of [
        ...
        assert.equal(evidence?.[gate], true, `controlled predecessor rehearsal did not prove ${gate}`);
      }
      return {
        status: 'passed',
...
export function verifyAuthenticPredecessorArtifact(config, publicContract) {
  ...
  assert.equal(artifact.evidenceKind, AUTHENTIC_PREDECESSOR_KIND, 'authentic predecessor artifact kind mismatch');
  assert.equal(artifact.signatureAlgorithm, AUTHENTIC_PREDECESSOR_ALGORITHM, 'authentic predecessor signature algorithm mismatch');
  assert.ok(typeof artifact.keyId === 'string' && artifact.keyId.length >= 3 && artifact.keyId.length <= 120, 'authentic predecessor key ID is invalid');
  assertExactObjectKeys(artifact.payload, ['auditUserId', 'attemptId', 'initial', 'continuation'], 'authentic predecessor payload');
  assert.equal(artifact.payload.auditUserId, publicContract.auditUserId, 'authentic predecessor artifact belongs to a different audit account');
  assert.match(String(artifact.payload.attemptId || ''), UUID_V4_RE, 'authentic predecessor artifact attempt is invalid');
  for (const [label, decision, expectedDecisionOrdinal] of [
    ...
    assertExactObjectKeys(decision, ['eventKey', 'receiptId', 'handOrdinal', 'decisionOrdinal'], `authentic predecessor ${label}`);
    assert.match(String(decision.receiptId || ''), UUID_V4_RE, `authentic predecessor ${label} receipt ID is invalid`);
    assert.equal(decision.decisionOrdinal, expectedDecisionOrdinal, `authentic predecessor ${label} decision ordinal is invalid`);
    assert.ok(Number.isSafeInteger(decision.handOrdinal) && decision.handOrdinal >= 1, `authentic predecessor ${label} hand ordinal is invalid`);
    assert.equal(
      decision.eventKey,
  ...
  assert.equal(artifact.payload.continuation.handOrdinal, artifact.payload.initial.handOrdinal, 'authentic predecessor continuation changed hand');
  assert.notEqual(artifact.payload.initial.receiptId, artifact.payload.continuation.receiptId, 'authentic predecessor receipt IDs must be distinct');
  assert.match(String(artifact.signature || ''), /^[A-Za-z0-9+/]+={0,2}$/, 'authentic predecessor signature encoding is invalid');
  const signatureBytes = Buffer.from(artifact.signature, 'base64');
  assert.equal(signatureBytes.length, 64, 'authentic predecessor Ed25519 signature length is invalid');
  assert.equal(signatureBytes.toString('base64'), artifact.signature, 'authentic predecessor signature encoding is not canonical');
  ...
  assert.equal(trustedKeySha256, config.trustedKeySha256, 'authentic predecessor public key fingerprint mismatch');
  let key;
  try {
...
async function collectAuthenticPredecessorCompatibility(database, config, publicContract) {
  ...
  try {
    ...
    assert.equal(rows.length, 2, 'authentic predecessor artifact did not correlate to both immutable production events');
    for (const [index, expected] of [artifact.payload.initial, artifact.payload.continuation].entries()) {
      assert.equal(rows[index].eventKey, expected.eventKey, 'authentic predecessor event key mismatch');
      assert.equal(rows[index].auditUserId, artifact.payload.auditUserId, 'authentic predecessor owner mismatch');
      assert.equal(rows[index].attemptId, artifact.payload.attemptId, 'authentic predecessor attempt mismatch');
      assert.equal(rows[index].handOrdinal, expected.handOrdinal, 'authentic predecessor hand mismatch');
      assert.equal(rows[index].decisionOrdinal, expected.decisionOrdinal, 'authentic predecessor decision mismatch');
      assert.equal(rows[index].receiptId, expected.receiptId, 'authentic predecessor receipt ID mismatch');
      assert.ok(
...
function validateMachineCollectorCoreConfig(config, runtime) {
  ...
  assert.equal(existsSync(publicEvidencePath), true, 'immutable public evidence file does not exist');
  assert.match(String(config.expectedAuditUserId || ''), UUID_V4_RE, 'machine collector expected audit account is invalid');
  assert.match(String(config.expectedSupabaseProjectRef || ''), /^[a-z0-9]{20}$/, 'machine collector expected Supabase project ref is invalid');
  assert.equal(config.acknowledgement, ADMIN_CLOSEOUT_ACKNOWLEDGEMENT, 'machine collector closeout acknowledgement is invalid');
  assert.ok(
    config.predecessor?.mode === 'authentic_artifact' || config.predecessor?.mode === 'controlled_rehearsal',
    'machine collector predecessor mode is invalid',
  );
  if (config.predecessor.mode === 'controlled_rehearsal') {
    assert.equal(config.predecessor.rehearsalAcknowledged, true, 'controlled rehearsal was not explicitly acknowledged as non-authentic');
  }
  if (!runtime.databaseTransport) validateDatabaseCredential(config.databaseUrl, config.expectedSupabaseProjectRef);
  if (!runtime.logTransport) {
    assert.ok(typeof config.vercelToken === 'string' && config.vercelToken.length >= 20, 'machine collector Vercel credential is missing');
    assert.ok(typeof config.vercelProject === 'string' && config.vercelProject.length > 0, 'machine collector Vercel project is missing');
  }
...
export async function collectMachineAdministratorEvidenceCore(config, runtime = {}) {
  ...
  assert.equal(publicContract.auditUserId, config.expectedAuditUserId, 'public evidence belongs to a different designated audit account');
  ...
  assert.deepEqual(initialDeployment, publicParsed.value.deployment, 'live deployment health does not match immutable public evidence');

  const database = runtime.databaseTransport || await createPostgresMachineCollectorTransport({
    connectionString: config.databaseUrl,
  });
  const logTransport = runtime.logTransport || createVercelCliRuntimeLogTransport({
  ...
  try {
    ...
    if (config.predecessor.mode === 'authentic_artifact') {
      predecessorRollbackCompatibility = runtime.predecessorTransport
        ...
        : await collectAuthenticPredecessorCompatibility(database, config.predecessor, publicContract);
    }
  } finally {
    await database.close();
  }
  if (config.predecessor.mode === 'controlled_rehearsal') {
    const rehearsal = runtime.predecessorTransport || createControlledPredecessorRehearsalTransport();
  ...
  assert.deepEqual(finalDeployment, initialDeployment, 'deployment identity changed during administrator collection');
  const currentPublic = parseJsonEvidence(config.publicEvidencePath, 'public evidence');
  assert.equal(currentPublic.digest, publicParsed.digest, 'immutable public evidence changed during administrator collection');

  const verifiedAt = now().toISOString();
  assert.ok(Date.parse(verifiedAt) >= publicContract.completedAt, 'machine collector verification predates public completion');
  const identity = databaseEvidence.databaseIdentity;
  assert.ok(String(identity.currentUser || '').length > 0, 'administrator database current_user is missing');
  assert.ok(String(identity.currentDatabase || '').length > 0, 'administrator database current_database is missing');
  assert.match(String(identity.serverVersionNum || ''), /^\d{5,6}$/, 'administrator database server version is invalid');
...
async function runMachineAdministratorCollector(config = readMachineCollectorConfig()) {
  ...
  try {
    ...
    assert.equal(attestationExitCode(finalized), 0, 'machine-collected final evidence did not satisfy release exit semantics');
    assert.equal(
    ...
    writeOutputAtomic(resolve(config.adminEvidencePath), collected.adminEvidence, { overwrite: false });
    writeOutputAtomic(resolve(config.finalEvidencePath), finalized, { overwrite: false });
    assert.equal(statSync(resolve(config.adminEvidencePath)).mode & 0o777, 0o600, 'administrator evidence mode is not 0600');
    assert.equal(statSync(resolve(config.finalEvidencePath)).mode & 0o777, 0o600, 'final evidence mode is not 0600');
    return finalized;
  } finally {
...
export function originScopedAuthState(state, targetOrigin) {
  const trustedSources = (state?.origins || []).filter(({ origin }) => (
  ...
  assert.equal(candidates.length, 1, 'authenticated storage state must contain exactly one trusted smarter-poker-auth value');
...
function decodeJwtPayload(accessToken) {
  ...
  assert.ok(payload && typeof payload === 'object' && !Array.isArray(payload), 'saved access token JWT payload is invalid');
  return payload;
}
...
export function validateDesignatedAuditAuthState(state, targetOrigin, expectedAuditUserId) {
  ...
  assert.equal(tokenPayload.sub, expectedAuditUserId, 'saved access token subject does not match the designated audit account');
  assert.equal(session?.user?.id, expectedAuditUserId, 'saved auth session user does not match the designated audit account');
...
export async function readDeploymentIdentity(
  ...
  assert.equal(String(health?.commitSha || '').toLowerCase(), expectedBuild, 'deployment health commitSha mismatch');
  assert.equal(String(health?.version || '').toLowerCase(), expectedBuild, 'deployment health version mismatch');
  ...
  assert.equal(validateImmutableDeploymentUrl(healthDeploymentUrl), baseUrl, 'health named a different immutable deployment');
  assert.ok(
    typeof health?.deploymentId === 'string' && health.deploymentId.trim() === health.deploymentId
      && health.deploymentId.length >= 3 && health.deploymentId.length <= 200,
  ...
  assert.equal(health?.checks?.trainingGradingReceipt?.status, 'ok', 'deployment grading-receipt health was not ok');
  return {
...
async function browserRequest(page, path, { method = 'GET', body = null, discardBody = false } = {}) {
  return page.evaluate(async ({ requestPath, requestMethod, requestBody, shouldDiscard }) => {
    let session;
    try { session = JSON.parse(localStorage.getItem('smarter-poker-auth') || 'null'); } catch { session = null; }
    ...
    try {
      ...
      let payload = null;
      try { payload = text ? JSON.parse(text) : null; } catch { /* asserted by caller */ }
      return { status: response.status, payload, responseBytes: new TextEncoder().encode(text).length };
    } finally {
...
async function expectApi(page, path, options, label) {
  ...
  assert.equal(result.status, 200, `${label} returned HTTP ${result.status}: ${result.payload?.code || result.payload?.error || 'no JSON error'}`);
...
async function expectRecordQuestion(page, pacer, body, label, { discardBody = false } = {}) {
  ...
  return expectApi(page, '/api/training/record-question', {
    method: 'POST',
    body,
    discardBody,
  }, label);
...
async function expectRecordQuestionRefusal(
  page,
  pacer,
  body,
  label,
  ...
  assert.equal(result.status, status, `${label} returned HTTP ${result.status} instead of ${status}`);
...
function publicContinuationCandidates(question) {
  ...
  const betOptions = options.filter((option) => /^(?:b\d+|bet(?:[_-].*)?|raise(?:[_-].*)?)$/i.test(option));
  return betOptions.length > 0 ? betOptions : options;
}

export async function runProductionDeliveryAttestation(config = readAttestationConfig(), runtime = {}) {
  ...
  const now = runtime.now || (() => new Date());
  const sleep = runtime.sleep || ((delayMs) => new Promise((resolveSleep) => setTimeout(resolveSleep, delayMs)));
  ...
  try {
    ...
    page.on('pageerror', (error) => {
      publicClientErrors.push({ kind: 'pageerror', message: redactReceiptMaterial(error?.message || String(error)) });
    });
    page.on('console', (message) => {
      if (message.type() === 'error') {
        publicClientErrors.push({ kind: 'console', message: redactReceiptMaterial(message.text()) });
      }
    });
    const pageResponse = await page.goto(`${config.baseUrl}/hub/training?revision=phase6-delivery-attestation`, {
      waitUntil: 'domcontentloaded',
      timeout: 60_000,
    });
    assert.ok(pageResponse && pageResponse.status() < 400, `authenticated bootstrap returned HTTP ${pageResponse?.status() || 'none'}`);
    assert.equal(new URL(pageResponse.url()).origin, config.baseUrl, 'authenticated bootstrap response escaped the immutable deployment origin');
    assert.equal(new URL(page.url()).origin, config.baseUrl, 'authenticated page escaped the immutable deployment origin');

    const authProbe = await expectApi(page, '/api/training/get-sessions?limit=1', {}, 'authenticated session probe');
    ...
    const initialResult = await expectApi(page, `/api/training/batch-preload?${params}`, {}, 'fresh full attempt');
    ...
    const reissueResult = await expectApi(page, '/api/training/reissue-questions', {
      method: 'POST',
      body: {
        gameId: GAME_ID,
    ...
    observations.push(...reissued.questions.map((question) => decodeReceiptObservation(question._gradingContext.receipt)));
    evidence.publicApi.reissue = {
      manifestIdentityExact: true,
      recoveredExistingAttempt: true,
      receiptBytesIdentical: reissueManifest.every(({ receiptBytesIdentical }) => receiptBytesIdentical),
    ...
    for (let index = 0; index < Math.min(MAX_PARENT_CANDIDATES, reissued.questions.length); index += 1) {
      ...
      if (index === 0) {
        ...
        assert.equal(lostResponse.discardedResponseBody, true, 'response-loss probe unexpectedly retained a response body');
        recorded = (await expectRecordQuestion(
          page,
          recordQuestionPacer,
          initialBody,
          'response-loss recovery',
        )).payload;
        assert.equal(recorded.idempotentReplay, true, 'the committed response loss did not recover idempotently');
        validateAnswerBinding(recorded, initialQuestion, 'response-loss answer');
        initialReceiptServerVerified = true;
        const exactReplay = (await expectRecordQuestion(
          page,
          recordQuestionPacer,
          reissuedBody,
        ...
        assert.equal(recorded.idempotentReplay, false, `candidate ${index + 1} was unexpectedly already answered`);
        validateAnswerBinding(recorded, initialQuestion, `fresh initial-receipt candidate ${index + 1}`);
        initialReceiptServerVerified = true;
        const reissuedReplay = (await expectRecordQuestion(
          page,
          recordQuestionPacer,
          reissuedBody,
      ...
      const selectsFirstContinuation = continuationAction === selectedAnswer && selectedParent === null;
    ...
    assert.ok(selectedParent, `no real continuation branch was reached in ${parentAttempts.length} bounded parent candidates`);
    assert.equal(parentAttempts.length, initial.questions.length, 'not every initial receipt reached record-question');
    assert.ok(parentAttempts.every(({ initialReceiptServerVerified }) => initialReceiptServerVerified), 'an initial receipt was not server-verified');
    assert.ok(parentAttempts.every(({ reissuedReceiptServerVerified }) => reissuedReceiptServerVerified), 'a reissued receipt was not server-verified');

    const selectedParentBody = buildAnswerRequest(selectedParent.question, selectedParent.selectedAnswer);
    ...
    assert.ok(conflictingAnswer, 'selected parent did not expose a distinct answer for the replay-conflict probe');
    ...
    const differentSubmissionId = reissued.questions
      .find((candidate) => candidate._gradingContext.submissionId !== selectedParent.question._gradingContext.submissionId)
      ?._gradingContext.submissionId;
    assert.ok(differentSubmissionId, 'submission-binding refusal probe could not select another signed slot');
    ...
    const firstContinuation = (await expectApi(page, '/api/training/next-street', {
      method: 'POST',
      body: { gradingReceipt: selectedParent.question._gradingContext.receipt },
    }, 'real next-street continuation')).payload;
    const continuationReplay = (await expectApi(page, '/api/training/next-street', {
      method: 'POST',
      body: { gradingReceipt: selectedParent.question._gradingContext.receipt },
    }, 'next-street child replay')).payload;
    ...
    const childAnswer = (await expectRecordQuestion(
      page,
      recordQuestionPacer,
      childBody,
      'child answer',
    )).payload;
    assert.equal(childAnswer.idempotentReplay, false, 'fresh child answer was unexpectedly a replay');
    validateAnswerBinding(childAnswer, childQuestion, 'child answer');
    const childAnswerReplay = (await expectRecordQuestion(
      page,
      recordQuestionPacer,
      childBody,
      'child answer replay',
    ...
    evidence.publicApi = {
      ...
      receiptServerVerification: {
        initial: {
          ...
          verified: parentAttempts.filter(({ initialReceiptServerVerified }) => initialReceiptServerVerified).length,
          allAccepted: parentAttempts.every(({ initialReceiptServerVerified }) => initialReceiptServerVerified),
        },
        reissued: {
          expected: reissued.questions.length,
          verified: parentAttempts.filter(({ reissuedReceiptServerVerified }) => reissuedReceiptServerVerified).length,
          allAccepted: parentAttempts.every(({ reissuedReceiptServerVerified }) => reissuedReceiptServerVerified),
        },
        receiptBytesIdentical: parentAttempts.filter(({ receiptBytesIdentical }) => receiptBytesIdentical).length,
        refreshedReceiptBytes: parentAttempts.filter(({ receiptBytesIdentical }) => !receiptBytesIdentical).length,
    ...
    assert.ok(observedSettleMs >= ERROR_SETTLE_WINDOW_MS, 'post-run error-settle window was shorter than required');
    assert.deepEqual(publicClientErrors, [], 'public client emitted errors during the attestation window');
...
if (isMain) {
  const run = process.env.TRAINING_PHASE6_ADMIN_COLLECT === '1'
    ? runMachineAdministratorCollector(readMachineCollectorConfig())
    : process.env.TRAINING_PHASE6_DELIVERY_ADMIN_EVIDENCE
      ? Promise.resolve().then(() => finalizeProductionDeliveryAttestation(readAdministratorCloseoutConfig()))
