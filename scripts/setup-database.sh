...
CREATE OR REPLACE FUNCTION unlock_free_avatars(p_user_id UUID) RETURNS void AS \$\$ BEGIN INSERT INTO avatar_unlocks (user_id, avatar_id, unlock_method) SELECT p_user_id, 'free_' || unnest(ARRAY['rockstar', 'chef', 'cyborg', 'detective', 'business', 'teacher', 'musician', 'pirate', 'shark', 'penguin', 'fox', 'owl', 'lion', 'rabbit', 'ninja', 'knight', 'samurai', 'android', 'shiba', 'wizard', 'space_commander', 'viking', 'aztec', 'geisha', 'cowboy']), 'default' ON CONFLICT (user_id, avatar_id) DO NOTHING; END; \$\$ LANGUAGE plpgsql SECURITY DEFINER;
