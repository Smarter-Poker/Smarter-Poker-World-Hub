...
// Batch 21 is 500-525 (venues #501-525 based on new array length after removal)
const startIndex = 500;
const endIndex = Math.min(525, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 21 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 21 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch21-review.html', html);
console.log("Created public/batch21-review.html");
