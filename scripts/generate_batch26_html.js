...
// Batch 26 is 625-650 (venues #626-650 based on new array length after removal)
const startIndex = 625;
const endIndex = Math.min(650, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 26 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 26 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch26-review.html', html);
console.log("Created public/batch26-review.html");
