...
// Batch 27 is 650-675 (venues #651-675 based on new array length after removal)
const startIndex = 650;
const endIndex = Math.min(675, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 27 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 27 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch27-review.html', html);
console.log("Created public/batch27-review.html");
