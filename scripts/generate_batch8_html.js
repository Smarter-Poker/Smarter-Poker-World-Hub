...
// Batch 8 is 175-200 (venues #176-200 based on new array length)
const startIndex = 175;
const endIndex = Math.min(200, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 8 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 8 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch8-review.html', html);
console.log("Created public/batch8-review.html");
