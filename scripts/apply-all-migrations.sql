-- ═══════════════════════════════════════════════════════════════════════════
-- MASTER MIGRATION SCRIPT FOR SUPABASE
-- Run this in the Supabase SQL Editor to apply all pending migrations
-- ═══════════════════════════════════════════════════════════════════════════

-- ═══════════════════════════════════════════════════════════════════════════
-- STEP 1: CLEAN UP ALL SOCIAL DATA (Delete anonymous posts & seed data)
-- ═══════════════════════════════════════════════════════════════════════════

-- Delete all social interactions first (foreign key dependencies)
DELETE FROM social_post_reactions WHERE true;
DELETE FROM social_comments WHERE true;
DELETE FROM social_message_reads WHERE true;
DELETE FROM social_messages WHERE true;
DELETE FROM social_conversation_participants WHERE true;
DELETE FROM social_conversations WHERE true;

-- Delete all social posts
DELETE FROM social_posts WHERE true;

-- Delete any social media uploads
DELETE FROM social_media WHERE true;

-- Confirm cleanup
SELECT 'Social data cleanup complete!' as status;

-- ═══════════════════════════════════════════════════════════════════════════
-- STEP 2: SOCIAL MESSAGING SYSTEM - Complete Schema  
-- ═══════════════════════════════════════════════════════════════════════════
...
DROP POLICY IF EXISTS "Users can view their conversations" ON social_conversations;
DROP POLICY IF EXISTS "Users can view conversation participants" ON social_conversation_participants;
...
DROP POLICY IF EXISTS "Users can view read receipts" ON social_message_reads;
DROP POLICY IF EXISTS "Users can mark messages read" ON social_message_reads;
DROP POLICY IF EXISTS "Users can manage their settings" ON social_messaging_settings;
...
-- Enable realtime for messages (safely check if already added)
DO $$ 
BEGIN
...
SELECT 'Migration complete! All social tables and policies created.' as status;
