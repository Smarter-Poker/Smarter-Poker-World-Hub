...
// Batch 16 is 375-400 (venues #376-400 based on new array length)
const startIndex = 375;
const endIndex = Math.min(400, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 16 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 16 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch16-review.html', html);
console.log("Created public/batch16-review.html");
