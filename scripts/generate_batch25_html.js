...
// Batch 25 is 600-625 (venues #601-625 based on new array length after removal)
const startIndex = 600;
const endIndex = Math.min(625, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 25 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 25 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch25-review.html', html);
console.log("Created public/batch25-review.html");
