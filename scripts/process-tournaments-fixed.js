/**
 * Tournament Series Processor - Fixed Schema
...
class TournamentProcessor {
    ...
    transformSeries(series) {
        ...
        if (nameLower.includes('wsop') || nameLower.includes('wpt') || nameLower.includes('lapc') || nameLower.includes('world series')) {
            seriesType = 'major';
        } else if (nameLower.includes('circuit') || nameLower.includes('mspt') || nameLower.includes('rgps') || nameLower.includes('hpt')) {
            seriesType = 'circuit';
        }
        ...
        if (nameLower.includes('borgata')) shortName = 'BORGATA';
        if (nameLower.includes('venetian')) shortName = 'VENETIAN';
        ...
        return {
            ...
            total_events: series.event_count ? Math.floor(series.event_count) : null,
    ...
    async processTournaments(jsonPath) {
        ...
        for (let i = 0; i < upcomingSeries.length; i += BATCH_SIZE) {
            ...
            console.log(`📦 Batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(upcomingSeries.length / BATCH_SIZE)}:`);
