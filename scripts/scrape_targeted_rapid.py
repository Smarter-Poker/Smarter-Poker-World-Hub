import sys, os, time, uuid, json, urllib.request, urllib.parse, re
...
if __name__ == "__main__":
    missing = get_missing()
    print(f"Found {len(missing)} missing venues to process rapidly.")
    
    batch_id = str(uuid.uuid4())
    session = StealthySession(headless=True, solve_cloudflare=True)
    session.start()
    print("Fetching Henderson/Cardplayer maps...")
    hm_map = scraper.fetch_hendonmob(session)
    cp_map = scraper.fetch_cardplayer(session)
    session.close()

    print(f"Starting {min(15, len(missing))} parallel workers...")
    successes = []
    fails = []
    ...
    print(f"Fails (no records found or error): {len(fails)}")
