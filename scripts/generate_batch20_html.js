...
// Batch 20 is 475-500 (venues #476-500 based on new array length after removal)
const startIndex = 475;
const endIndex = Math.min(500, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 20 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 20 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch20-review.html', html);
console.log("Created public/batch20-review.html");
