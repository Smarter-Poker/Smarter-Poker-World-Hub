...
// Batch 29 is 700-725
const startIndex = 700;
const endIndex = Math.min(725, venues.length);
const batch = venues.slice(startIndex, endIndex);

if (batch.length > 0) {
  let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 29 — Venues #${startIndex + 1}-${endIndex}</title>
  <style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <header>
  <h1>Batch 29 — Venues #${startIndex + 1}-${endIndex}</h1>
  ...
  fs.writeFileSync('public/batch29-review.html', html);
  console.log("Created public/batch29-review.html with " + batch.length + " remaining venues.");
} else {
