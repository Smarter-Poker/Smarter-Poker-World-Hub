...
// Batch 17 is 400-425 (venues #401-425 based on new array length after removal)
const startIndex = 400;
const endIndex = Math.min(425, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 17 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 17 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch17-review.html', html);
console.log("Created public/batch17-review.html");
