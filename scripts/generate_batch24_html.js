...
// Batch 24 is 575-600 (venues #576-600 based on new array length after removal)
const startIndex = 575;
const endIndex = Math.min(600, venues.length);
const batch = venues.slice(startIndex, endIndex);

let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Batch 24 — Venues #${startIndex + 1}-${endIndex}</title>
<style>
...
<header>
<h1>Batch 24 — Venues #${startIndex + 1}-${endIndex}</h1>
...
fs.writeFileSync('public/batch24-review.html', html);
console.log("Created public/batch24-review.html");
