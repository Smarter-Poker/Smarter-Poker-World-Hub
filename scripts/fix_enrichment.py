...
def get_html(url):
    ...
    if fc % 15 == 0:
        ...
        session = StealthyFetcher(auto_match=True)
    try:
        r = session.fetch(url, timeout=12000)
...
for idx, (series_url, db_events) in enumerate(needs_enrichment.items(), 1):
    ...
    print(f"\n[{idx}/{len(needs_enrichment)}] {series_url[:60]} -> {len(links)} event links.")
    ...
    for link in links:
        ...
        # Full Late Reg Text Extraction
        late_m = re.search(r'(?:Late Registration|Late Reg|Registration closes)(?:[^.!?\n]{1,50}(?:Level\s*\d+|[\d]{1,2}:\d{2}\s*(?:AM|PM|am|pm)|hours|mins?))', text, re.I)
        if late_m:
            scraped_latereg = late_m.group(0)[:80].strip()
    ...
    print(f"  --> Cumulative: +{stats['stack']} Stacks, +{stats['gtd']} GTDs, +{stats['late']} LateRegs in {stats['updated']} updates")
