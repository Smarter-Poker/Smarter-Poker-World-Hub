...
async function persist(rows) {
  ...
  const { data, error } = await db.from('training_question_cache')
  ...
  if (error) throw new Error(`cache_upsert:${error.message}`);
  if ((data || []).length !== rows.length) throw new Error('cache_upsert_receipt_count_mismatch');
  ...
  if (releaseError) throw new Error(`quarantine_release:${releaseError.message}`);
  if (Number(released) !== ids.length) throw new Error(`quarantine_release_count:${released}/${ids.length}`);
  counters.restored += rows.length;
  counters.released += Number(released);
}
