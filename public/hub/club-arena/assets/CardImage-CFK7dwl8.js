import{s as m}from"./index-ChtNWGYj.js";import{j as h}from"./vendor-react-BBaYhDCi.js";class y{async getHand(e){const{data:a,error:s}=await m.from("hands").select(`
                *,
                hand_players (
                    seat,
                    user_id,
            ...
            `).eq("id",e).maybeSingle();if(s||!a)return null;const i=(a.hand_players||[]).map(t=>t.user_id),o=await this.fetchProfileMap(i);return this.mapHandRecord(a,o)}async getPlayerHands(e,a=50){const{data:s,error:i}=await m.from("hand_players").select(`
                hands (
                    *,
                    hand_players (
                        seat,
            ...
            `).eq("table_id",e).order("created_at",{ascending:!1}).limit(a);if(i||!s)return[];const o=s.flatMap(r=>(r.hand_players||[]).map(d=>d.user_id)),t=await this.fetchProfileMap(o);return s.map(r=>this.mapHandRecord(r,t)).filter(r=>r!==null)}async getRecentWinningHands(e,a=10){const{data:s,error:i}=await m.from("hand_players").select(`
                hands (
                    *,
                    hand_players (
                        seat,
